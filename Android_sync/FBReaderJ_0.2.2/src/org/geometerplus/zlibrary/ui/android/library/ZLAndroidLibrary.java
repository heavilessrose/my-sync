...
public final class ZLAndroidLibrary extends ZLibrary {
	...
	ZLAndroidLibrary(ZLAndroidActivity activity) {
		...
		try {
			ZLApplication application = (ZLApplication) getApplicationClass()
					.newInstance();
	...
	public ZLAndroidWidget getWidget() {
		if (myWidget == null) {
			myWidget = (ZLAndroidWidget) myActivity
					.findViewById(R.id.zlandroidactivity);
		}
		return myWidget;
	}
	...
	protected InputStream getResourceInputStream(String fileName) {
		final String fieldName = fileName.replace("/", "__").replace(".", "_")
