...
public final class ZLSQLiteConfig extends ZLConfig {
	...
	private SQLiteDatabase database() {
		if (myDatabase == null) {
			myDatabase = myContext.openOrCreateDatabase(myName + ".db",
					Context.MODE_PRIVATE, null);
			try {
				myDatabase
						.execSQL("CREATE TABLE config (groupName VARCHAR, name VARCHAR, value VARCHAR, PRIMARY KEY(groupName, name) )");
			} catch (Exception e) {
			}
		}
		return myDatabase;
	...
	public String getValue(String group, String name, String defaultValue) {
		Cursor cursor = database().query(true, ourTableName, ourColumns,
				"groupName='" + group + "' AND name='" + name + "'",
				ourSelectionArgs, null, null, null, null);
	...
	public void unsetValue(String group, String name) {
		database().execSQL(
				"DELETE FROM config WHERE groupName='" + group + "' AND name='"
						+ name + "'");
	}
}
