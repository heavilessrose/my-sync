...
class UserDatabase extends SQLiteOpenHelper implements BaseColumns {
	...
	public void onCreate(SQLiteDatabase db) {
		db.execSQL("CREATE TABLE users (" + "_id INTEGER PRIMARY KEY, "
				+ "username TEXT, " + "realname TEXT, " + "nsid TEXT, "
				+ "buddy_icon BLOB," + "last_update INTEGER);");
	...
	private void addUser(SQLiteDatabase db, String userName, String realName,
		...
		final Bitmap bitmap = BitmapFactory.decodeResource(mContext
				.getResources(), icon);
	...
	static void writeBitmap(ContentValues values, String name, Bitmap bitmap) {
		if (bitmap != null) {
			...
			// serialized to avoid unnecessary allocations/copies during the
			// write.
