...
class Flickr {
	...
	/**
	 * A Flickr user, in the strictest sense, is only defined by its NSID. The
	 * NSID is usually obtained by {@link Flickr#findByUserName(String) looking
	 * up a user by its user name}.
	 * 
	 * To obtain more information about a given user, refer to the UserInfo
	 * class.
	 * 
	...
	static class User implements Parcelable {
		...
		/**
		 * Returns the Flickr NSDID of the user. The NSID is used to identify
		 * the user with any operation performed on Flickr.
		 * 
	...
	/**
	 * A set of information for a given Flickr user. The information exposed
	 * include: - The user's NSDID - The user's name - The user's real name -
	 * The user's location - The URL to the user's photos - The URL to the
	 * user's profile - The URL to the user's mobile web site - Whether the user
	 * has a pro account
	 */
	static class UserInfo implements Parcelable {
		...
		/**
		 * Returns the user's name. This is the name that the user authenticates
		 * with, and the name that Flickr uses in the URLs (for instance,
		 * http://flickr.com/photos/romainguy, where romainguy is the user
		 * name.)
		 * 
		...
		/**
		 * Returns the user's real name. The real name is chosen by the user
		 * when creating his account and might not reflect his civil name.
		 * 
		...
		/**
		 * Loads the user's buddy icon as a Bitmap. The user's buddy icon is
		 * loaded from the URL returned by {@link #getBuddyIconUrl()}. The buddy
		 * icon is not cached locally.
		 * 
		 * @return A 48x48 bitmap if the icon was loaded successfully or null
		 *         otherwise.
		 */
		Bitmap loadBuddyIcon() {
			...
			try {
				in = new BufferedInputStream(new URL(getBuddyIconUrl())
				...
				android.util.Log.e(Flickr.LOG_TAG,
						"Could not load buddy icon: " + this, e);
			} finally {
				closeStream(in);
				closeStream(out);
			}
	...
	/**
	 * A photo is represented by a title, the date at which it was taken and a
	 * URL. The URL depends on the desired
	 * {@link com.google.android.photostream.Flickr.PhotoSize}.
	 */
	static class Photo implements Parcelable {
		 ...
		 * 
		 * @return The title of the photo. The returned value can be empty or
		 *         null.
		 */
		String getTitle() {
			return mTitle;
		...
		/**
		 * Returns the date at which the photo was taken, formatted in the
		 * current locale with the following pattern: MMMM d, yyyy.
		 * 
		 * @return The title of the photo. The returned value can be empty or
		 *         null.
		 */
		...
		String getUrl(PhotoSize photoSize) {
			return String.format(PHOTO_IMAGE_URL, mFarm, mServer, mId, mSecret,
					photoSize.size());
		}

		/**
		 * Loads a Bitmap representing the photo for the specified size. The
		 * Bitmap is loaded from the URL returned by
		 * {@link #getUrl(com.google.android.photostream.Flickr.PhotoSize)}.
		 * 
		 * @param size
		 *            The size of the photo to load.
		 * 
		 * @return A Bitmap whose longest size is the same as the longest side
		 *         of the specified
		 *         {@link com.google.android.photostream.Flickr.PhotoSize}, or
		...
		Bitmap loadPhotoBitmap(PhotoSize size) {
			...
			try {
				in = new BufferedInputStream(
						new URL(getUrl(size)).openStream(), IO_BUFFER_SIZE);
				...
				android.util.Log.e(Flickr.LOG_TAG, "Could not load photo: "
						+ this, e);
			} finally {
				closeStream(in);
				closeStream(out);
	...
	/**
	 * A list of {@link com.google.android.photostream.Flickr.Photo photos}. A
	 * list represents a series of photo on a page from the user's photostream,
	 * a list is therefore associated with a page index and a page count. The
	 * page index and the page count both depend on the number of photos per
	 * page.
	 */
	 ...
	 * 
	 * The returned User contains only the user's NSID. To retrieve more
	 ...
	 * 
	 * @return A User instance with a valid NSID, or null if the user cannot be
	 *         found.
	 * 
	...
	/**
	 * Retrives a list of photos for the specified user. The list contains at
	 * most the number of photos specified by <code>perPage</code>. The photos
	 * are retrieved starting a the specified page index. For instance, if a
	 * user has 10 photos in his photostream, calling getPublicPhotos(user, 5,
	 * 2) will return the last 5 photos of the photo stream.
	 * 
	 * The page index starts at 1, not 0.
	 * 
	 * @param user
	 *            The user to retrieve photos from.
	 * @param perPage
	 *            The maximum number of photos to retrieve.
	 * @param page
	 *            The index (starting at 1) of the page in the photostream.
	 * 
	 ...
	 *      com.google.android.photostream.Flickr.PhotoSize,
	...
	PhotoList getPublicPhotos(User user, int perPage, int page) {
		...
		try {
			...
			android.util.Log.e(LOG_TAG, "Could not find photos for user: "
					+ user);
		}
	 ...
	 * 
	 * @param photo
	 *            The photo to get the location of.
	 * 
	 * @return The geo location of the photo, or null if the photo has no
	 *         geodata or the photo cannot be found.
	 * 
	...
	Location getLocation(Flickr.Photo photo) {
		...
		try {
			...
			android.util.Log.e(LOG_TAG, "Could not find location for photo: "
					+ photo);
		}
	 ...
	 * 
	 * @param user
	 *            The user whose feed must be checked.
	 * @param reference
	 *            The date after which to check for updates.
	 * 
	 * @return True if any update occured after the reference date, false
	 *         otherwise.
	 */
	boolean hasUpdates(User user, final Calendar reference) {
		...
		try {
			...
			android.util.Log
					.e(LOG_TAG, "Could not find feed for user: " + user);
		}
	 ...
	 */
	void downloadPhoto(Photo photo, PhotoSize size, OutputStream destination)
			throws IOException {
	...
	private boolean parseUpdated(XmlPullParser parser, Calendar reference)
			...
			if (RESPONSE_TAG_UPDATED.equals(name)) {
				if (parser.next() == XmlPullParser.TEXT) {
					final SimpleDateFormat format = new SimpleDateFormat(
							"yyyy-MM-dd HH:mm:ss");
					try {
						final String text = parser.getText().replace('T', ' ')
								.replace('Z', ' ');
	...
	private void parsePhotos(XmlPullParser parser, PhotoList photos)
		...
		while (((type = parser.next()) != XmlPullParser.END_TAG || parser
			...
			if (RESPONSE_TAG_PHOTOS.equals(name)) {
				photos.mPage = Integer.parseInt(parser.getAttributeValue(null,
						RESPONSE_ATTR_PAGE));
				photos.mPageCount = Integer.parseInt(parser.getAttributeValue(
				...
				photo.mSecret = parser.getAttributeValue(null,
						RESPONSE_ATTR_SECRET);
				photo.mServer = parser.getAttributeValue(null,
						RESPONSE_ATTR_SERVER);
				photo.mFarm = parser
						.getAttributeValue(null, RESPONSE_ATTR_FARM);
				photo.mTitle = parser.getAttributeValue(null,
						RESPONSE_ATTR_TITLE);
				photo.mDate = parser.getAttributeValue(null,
				...
				try {
					photo.mDate = outputFormat.format(parseFormat
							.parse(photo.mDate));
				} catch (ParseException e) {
					android.util.Log
							.w(LOG_TAG, "Could not parse photo date", e);
				}
	...
	private void parsePhotoLocation(XmlPullParser parser, Location location)
		...
		while (((type = parser.next()) != XmlPullParser.END_TAG || parser
			...
			if (RESPONSE_TAG_LOCATION.equals(name)) {
				try {
					location.mLatitude = Float.parseFloat(parser
							.getAttributeValue(null, RESPONSE_ATTR_LATITUDE));
					location.mLongitude = Float.parseFloat(parser
							.getAttributeValue(null, RESPONSE_ATTR_LONGITUDE));
				} catch (NumberFormatException e) {
					throw new XmlPullParserException("Could not parse lat/lon",
							parser, e);
				}
			}
		}
	...
	private void parseUserInfo(XmlPullParser parser, UserInfo info)
		...
		while (((type = parser.next()) != XmlPullParser.END_TAG || parser
			...
			if (RESPONSE_TAG_PERSON.equals(name)) {
				info.mIsPro = "1".equals(parser.getAttributeValue(null,
						RESPONSE_ATTR_ISPRO));
				info.mIconServer = parser.getAttributeValue(null,
						RESPONSE_ATTR_ICONSERVER);
				info.mIconFarm = parser.getAttributeValue(null,
						RESPONSE_ATTR_ICONFARM);
	 ...
	 * {@link Flickr#executeRequest(org.apache.http.client.methods.HttpGet, com.google.android.photostream.Flickr.ResponseHandler)}
	...
	/**
	 * Response parser used with
	 * {@link Flickr#parseResponse(java.io.InputStream, com.google.android.photostream.Flickr.ResponseParser)}
	 * . When Flickr returns a valid response, this parser is invoked to process
	 * the XML response.
	 */
