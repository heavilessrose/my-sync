...
/**
 * Activity used to login the user. The activity asks for the user name and then
 * add the user to the users list upong successful login. If the login is
 ...
 * sucessful login returns a shortcut Intent to Home instead of proceeding to
 * PhotostreamActivity.
 * 
 * The shortcut Intent contains the real name of the user, his buddy icon, the
 * action {@link android.content.Intent#ACTION_VIEW} and the URI
 * flickr://photos/nsid.
 */
public class LoginActivity extends Activity implements View.OnKeyListener,
	...
	private void schedule() {
		...
		// preferences.edit().putBoolean(Preferences.KEY_ALARM_SCHEDULED,
		// true).commit();
		// }
	}
	...
	private Cursor initializeCursor() {
		Cursor cursor = mDatabase.query(UserDatabase.TABLE_USERS, new String[] {
				UserDatabase._ID, UserDatabase.COLUMN_REALNAME,
	...
	public void onItemClick(AdapterView<?> parent, View view, int position,
			long id) {
		final Flickr.User user = Flickr.User.fromId(((UserDescription) view
				.getTag()).nsid);
	...
	public boolean onContextItemSelected(MenuItem item) {
		final AdapterView.AdapterContextMenuInfo info = (AdapterView.AdapterContextMenuInfo) item
				.getMenuInfo();
		final UserDescription description = (UserDescription) info.targetView
				.getTag();
	 ...
	 * 
	 * @param user
	 *            The user to create a shortcut for.
	 */
	private void onCreateShortcut(Flickr.User user) {
		final Cursor cursor = mDatabase.query(UserDatabase.TABLE_USERS,
				new String[] { UserDatabase.COLUMN_REALNAME,
						UserDatabase.COLUMN_USERNAME,
						UserDatabase.COLUMN_BUDDY_ICON },
				UserDatabase.COLUMN_NSID + "=?", new String[] { user.getId() },
				null, null, UserDatabase.SORT_DEFAULT);
		cursor.moveToFirst();
		...
		String name = cursor.getString(cursor
				.getColumnIndexOrThrow(UserDatabase.COLUMN_REALNAME));
		if (name == null || name.length() == 0) {
			name = cursor.getString(cursor
					.getColumnIndexOrThrow(UserDatabase.COLUMN_USERNAME));
		}
		...
		byte[] data = cursor.getBlob(cursor
				.getColumnIndexOrThrow(UserDatabase.COLUMN_BUDDY_ICON));
	...
	/**
	 * Background task used to load the user's NSID. The task begins by showing
	 * the progress bar, then loads the user NSID from the network and finally
	 * open PhotostreamActivity.
	 */
	private class FindUserTask extends UserTask<String, Void, Flickr.User> {
		...
		public Flickr.User doInBackground(String... params) {
			...
			values.put(UserDatabase.COLUMN_LAST_UPDATE, System
					.currentTimeMillis());
