...
final class ZLOwnHtmlParser {
	...
	private static String convertToString(HashMap strings,
			ZLMutableString contatiner) {
	...
	/*
	 * private boolean isCorrectHtmlIdentificatorSymbol(char s) { return !( (s
	 * != '?') && (s != ' ') && (s != '\t') && (s != '\n') && (s != ' ') && (s
	 * != '\\') && (s != '/') && (s != '+') ); }
	 */

	public ZLOwnHtmlParser(ZLHtmlReader htmlReader, InputStream stream)
			throws IOException {
		myReader = htmlReader;
		...
		// if (len < 384) {
		StringBuffer description = new StringBuffer(new String(buffer, 0,
				observeChars));
		...
		if (index > 0) {
			int startIndex;
			for (startIndex = description.indexOf("=", index) + 1; !CharacterUtil
					.isLetter(description.charAt(startIndex)); startIndex++) {
			}
			if (startIndex > 0) {
				int endIndex = Math.min(description.indexOf("\"",
						startIndex + 1), Math.min(description.indexOf(" ",
						startIndex + 1), description.indexOf(">",
						startIndex + 1)));
				if (endIndex > 0) {
	...
	public void doIt() throws IOException {
		...
		final boolean dontCacheAttributeValues = htmlReader
				.dontCacheAttributeValues();
			...
			try {
				for (int i = -1;;) {
						...
						while (true) {
							...
							case '>':
								...
								tagName.append(buffer, startPosition, i
										- startPosition);
								{
									String stringTagName = convertToString(
											strings, tagName);
									processStartTag(htmlReader, stringTagName,
											attributes);
									if ("script".equals(stringTagName
											.toLowerCase())) {
										scriptOpened = true;
										state = SCRIPT;
										break mainSwitchLabel;
									}
									if ("html".equals(stringTagName
											.toLowerCase())) {
										html = true;
									}
								}
					...
					case WS_AFTER_START_TAG_NAME:
						switch (buffer[++i]) {
						case '>': {
							String stringTagName = convertToString(strings,
									tagName);
							processStartTag(htmlReader, stringTagName,
									attributes);
						...
						while (true) {
							...
							if ((buffer[i] == ' ') || (buffer[i] == '\'')
								...
								if (dontCacheAttributeValues) {
									attributes.put(convertToString(strings,
											attributeName), attributeValue
											.toString());
									attributeValue.clear();
								} else {
									attributes.put(convertToString(strings,
											attributeName), convertToString(
							...
							case '>':
								...
								processStartTag(htmlReader, stringTagName,
										attributes);
								if ("script"
										.equals(stringTagName.toLowerCase())) {
									scriptOpened = true;
									state = SCRIPT;
									break mainSwitchLabel;
								}
						...
						while (i - savePosition < 6) {
							...
							case ';':
								...
								if (value != null) {
									...
									case S_ATTRIBUTE_VALUE:
									case D_ATTRIBUTE_VALUE:
										attributeValue.append(value, 0,
												value.length);
										break;
									case ATTRIBUTE_NAME:
										attributeName.append(value, 0,
												value.length);
										break;
									case START_TAG:
									case END_TAG:
										...
										break;
									case TEXT:
										htmlReader.characterDataHandler(value,
												0, value.length);
										break;
									}
								}
					...
					case SLASH:
						while (true) {
							...
							case ' ':
								break;
							case '>':
								processFullTag(htmlReader, convertToString(
										strings, tagName), attributes);
								state = TEXT;
								startPosition = i + 1;
								break mainSwitchLabel;
							default:
					...
					case TEXT:
						while (true) {
							...
							case '<':
								if (i > startPosition) {
									htmlReader.characterDataHandlerFinal(
											buffer, startPosition, i
													- startPosition);
								}
								state = LANGLE;
								break mainSwitchLabel;
							case '&':
								if (i > startPosition) {
									htmlReader.characterDataHandler(buffer,
				...
				case END_TAG:
					tagName
							.append(buffer, startPosition, count
									- startPosition);
					break;
				case ATTRIBUTE_NAME:
				...
				case ENTITY_REF:
					...
					break;
				case TEXT:
					htmlReader.characterDataHandler(buffer, startPosition,
							count - startPosition);
					break;
				}
			}
	...
	private static void processFullTag(ZLHtmlReader htmlReader, String tagName,
			ZLStringMap attributes) {
	...
	private static void processStartTag(ZLHtmlReader htmlReader,
			String tagName, ZLStringMap attributes) {
		htmlReader.startElementHandler(tagName, attributes);
		attributes.clear();
	}
