...
public class PhotostreamActivity extends Activity implements
	 ...
	 * 
	 * @return The user whose photos will be displayed, or null if no user was
	 *         found.
	 */
	...
	public Object onRetainNonConfigurationInstance() {
		...
		for (int i = 0; i < count; i++) {
			...
			list[i] = new LoadedPhoto(((BitmapDrawable) v.getDrawable())
					.getBitmap(), (Flickr.Photo) v.getTag());
		}
	...
	private class LoadPhotosTask extends
			UserTask<Flickr.PhotoList, LoadedPhoto, Flickr.PhotoList> {
		...
		public Flickr.PhotoList doInBackground(Flickr.PhotoList... params) {
			...
			for (int i = 0; i < count; i++) {
				...
				Bitmap bitmap = photo
						.loadPhotoBitmap(Flickr.PhotoSize.THUMBNAIL);
				if (!isCancelled()) {
					if (bitmap == null) {
		 ...
		 * {@link com.google.android.photostream.Flickr.Photo} it was loaded
		 * from.
		 * 
		 * @param value
	 ...
	 * LoadPhotosTask.
	 */
	private class GetPhotoListTask extends
			UserTask<Integer, Void, Flickr.PhotoList> {
		public Flickr.PhotoList doInBackground(Integer... params) {
			if (mUsername != null) {
				mUser = Flickr.get().findByUserName(mUsername);
				mUsername = null;
			}
			return Flickr.get().getPublicPhotos(mUser, PHOTOS_COUNT_PER_PAGE,
					params[0]);
		}
