...
import cn.sda.pinyin.SelectPointForm;
import cn.sda.pinyin.SelectSymbolForm;
 ...
 */
public class MainMenuUI extends BaseControl {

    //背景图
    private Image backImage = null;
    //左右菜单
    private PopMenuUI leftMemu = null;
    private PopMenuUI rightMemu = null;
    //左右主菜单项名称
    ...
    //输入法菜单
    private PopMenuUI inputMethodMemu = null;
    //选择的当前菜单
    ...
    public MainMenuUI() {
        ...
        inputMethodMemu = new PopMenuUI();
        inputMethodMemu.setBackColor(ConstsUI.clWhite);
        ...
        inputMethodMemu.setOnClick(new PopMenuOnClickEvent() {
            ...
            public void Event(MenuItemUI item, String itemName, String itemCaption) {
                //菜单设置
                if (form == null) {
                    return;
                }
                ...
                if (itemName.equals("ABC")) {
                    form.Application.inputPanel.setImeType(InputPanel.imUpperCase);
                }
                if (itemName.equals("abc")) {
                    form.Application.inputPanel.setImeType(InputPanel.imLowerCase);
                }
                if (itemName.equals("123")) {
                    form.Application.inputPanel.setImeType(InputPanel.imDigit);
                }
                if (itemName.equals("拼音")) {
                    form.Application.inputPanel.setImeType(InputPanel.imPinYin);
                }
                if (itemName.equals("标点")) {
                    form.Application.inputPanel.setImeType(InputPanel.imPoint);
                    SelectPointForm.getInstance().open(form.focusControl);
                    return;
                }
                if (itemName.equals("符号")) {
                    form.Application.inputPanel.setImeType(InputPanel.imSymbol);
                    SelectSymbolForm.getInstance().open(form.focusControl);
                    return;
                }
    ...
    private void getMenuItemName() {
        if (form.Application.inputPanel.ImeType == InputPanel.imDigit) {
            menuItemName = "123";
        }
        if (form.Application.inputPanel.ImeType == InputPanel.imLowerCase) {
            menuItemName = "abc";
        }
        if (form.Application.inputPanel.ImeType == InputPanel.imPinYin) {
            menuItemName = "拼音";
        }
        if (form.Application.inputPanel.ImeType == InputPanel.imPoint) {
            menuItemName = "标点";
        }
        if (form.Application.inputPanel.ImeType == InputPanel.imSymbol) {
            menuItemName = "符号";
        }
        if (form.Application.inputPanel.ImeType == InputPanel.imUpperCase) {
            menuItemName = "ABC";
        }
        MenuItemUI item = inputMethodMemu.getMenuItem(inputMethodMemu, menuItemName);
    ...
    public void paint() {
        ...
        //注册键盘信息
        if (!reg) {
            form.registerHotKey(ConstsUI.KEY_SOFT1, hotKeyEvent);
            form.registerHotKey(ConstsUI.KEY_SOFT2, hotKeyEvent);
            reg = true;
        }
        //判断是否显示输入法（触摸屏的显示）
        SetClip(g);
    ...
    private void paintMainMenu(Graphics g) {
        ...
        if (backImage != null) {
            Image image = null;
            if (stretchImage) {
                image = ImageUtils.processImage(backImage, width, height, ImageUtils.MODE_STRETCH);
            } else {
                image = backImage;
            }
    ...
    //设置左右的菜单项目
    public PopMenuUI getLeftMemu() {
        return leftMemu;
    }

    public void setLeftMemu(PopMenuUI leftMemu) {
        this.leftMemu = leftMemu;
    }

    public PopMenuUI getRightMemu() {
        return rightMemu;
    }

    public void setRightMemu(PopMenuUI rightMemu) {
    ...
    public void setDock(int dock) {
        super.setDock(ConstsUI.dsBottom);
    }
    ...
    private void doPointerPressed(int x, int y) {
        ...
        if (InClientRect(posx, posy, 0, 0, 2 * width / 5, height)) {
            doHotKeyPressed(ConstsUI.KEY_SOFT1);
        }
        if (InClientRect(posx, posy, 3 * width / 5, 0, 2 * width / 5, height)) {
            doHotKeyPressed(ConstsUI.KEY_SOFT2);
        }
    }
    //键盘处理
    private void doHotKeyPressed(String keyName) {
        //处理热键
        if (keyName.equals(ConstsUI.KEY_SOFT1)) {
            //左键
            if (leftMemu != null) {
        ...
        if (keyName.equals(ConstsUI.KEY_SOFT2)) {
            if (rightMemu != null) {
