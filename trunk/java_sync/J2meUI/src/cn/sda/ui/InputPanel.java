 ...
 */
public class InputPanel extends BaseControl {
	...
	// 选中颜色
	private int selectedBackColor = ConstsUI.clBlack;
	...
	// 输入内容显示和选择框
	protected class inputCharsPanel extends BaseControl {

		// 光标在屏幕中的绝对位置
		protected int oraX = 0, oraY = 0;
		private int pageScrollBarColor = ConstsUI.clBtnFace;
		private int pageScrollBarFontColor = ConstsUI.clBlack;
		private int inputBarColor = ConstsUI.clWhite;
		// 选择
		...
		public inputCharsPanel() {
			...
			setWidth(60);
			setHeight(80);
			setBackColor(ConstsUI.clWhite);
			foreColor = ConstsUI.clBlack;
		...
		public void getCursorPosInForm() {
			...
			if (form != null) {
				if (form.focusControl != null) {
					if (form.focusControl instanceof BaseEdit) {
						control = (BaseEdit) form.focusControl;
