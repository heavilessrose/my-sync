 ...
 */
public class ChartUI extends BaseControl {

	private int borderColor = ConstsUI.clBlack;
	private int scaleLineColor = ConstsUI.clFocusShadow;
	private String titleText = "";
	private int titleTextAlign = ConstsUI.alignCenter;
	private boolean showTitleText = false;
	private String footText = "";
	private int footTextAlign = ConstsUI.alignCenter;
	private boolean showFootText = false;
	private String axisText = "";
	private int axisTextAlign = ConstsUI.alignCenter;
	private boolean showAxisText = false;
	// 柱图宽度
	private int barChartWidth = 8;
	// 参考线颜色
	private int refLineColor = ConstsUI.clFocusShadow;
	...
	public void paint() {
		...
		for (int i = 0; i < seriesList.size(); i++) {
			if (((ChartSeriesUI) seriesList.elementAt(i)).getChartType() == ChartSeriesUI.ctLine) {
				paintLineSeries(g, (ChartSeriesUI) seriesList.elementAt(i));
			}
			if (((ChartSeriesUI) seriesList.elementAt(i)).getChartType() == ChartSeriesUI.ctBar) {
				paintBarSeries(g, (ChartSeriesUI) seriesList.elementAt(i));
			}
			if (((ChartSeriesUI) seriesList.elementAt(i)).getChartType() == ChartSeriesUI.ctPie) {
				paintPieSeries(g, (ChartSeriesUI) seriesList.elementAt(i));
			}
			if (((ChartSeriesUI) seriesList.elementAt(i)).getChartType() == ChartSeriesUI.ctPoint) {
				paintPointSeries(g, (ChartSeriesUI) seriesList.elementAt(i));
			}
		}
		PaintChilds();
	}
	...
	private void sortValue() {
		// 各自Series排序(对x方向升序)
		ChartSeriesUI series = null;
		ChartSeriesUI.PointData pd = null;
		ChartSeriesUI.PointData pd1 = null;
		int num = 0;
		for (int i = 0; i < seriesList.size(); i++) {
			series = (ChartSeriesUI) seriesList.elementAt(i);
			num = series.getDataCount();
			for (int j = 0; j < num; j++) {
				pd = (ChartSeriesUI.PointData) series.getData(j);
				for (int k = j + 1; k < num; k++) {
					pd1 = (ChartSeriesUI.PointData) series.getData(k);
		...
		for (int i = 0; i < xValueList.size(); i++) {
			pd = (ChartSeriesUI.PointData) xValueList.elementAt(i);
			for (int j = i + 1; j < xValueList.size(); j++) {
				pd1 = (ChartSeriesUI.PointData) xValueList.elementAt(j);
				if (pd.xValue > pd1.xValue) {
		...
		for (int i = 0; i < yValueList.size(); i++) {
			pd = (ChartSeriesUI.PointData) yValueList.elementAt(i);
			for (int j = i + 1; j < yValueList.size(); j++) {
				pd1 = (ChartSeriesUI.PointData) yValueList.elementAt(j);
	...
	private double calYScale() {
		Font ft = getFont();
		// 计算显示刻度(排序后)
		double maxValue = yValueList.size() > 0 ? ((ChartSeriesUI.PointData) yValueList
				.elementAt(yValueList.size() - 1)).yValue
				: 0;
		double minValue = yValueList.size() > 0 ? ((ChartSeriesUI.PointData) yValueList
				.elementAt(0)).yValue
				: 0;
		if (maxValue < 0) {
			maxValue = 0;
	...
	private double calXScale() {
		Font ft = getFont();
		// 计算显示刻度(排序后)
		double maxValue = xValueList.size() > 0 ? ((ChartSeriesUI.PointData) xValueList
				.elementAt(xValueList.size() - 1)).xValue
				: 0;
		double minValue = xValueList.size() > 0 ? ((ChartSeriesUI.PointData) xValueList
				.elementAt(0)).xValue
				: 0;
		// 判断是否可以容纳的下文字(注意两边的空间)
	...
	// 画series线
	private void paintLineSeries(Graphics g, ChartSeriesUI series) {
		ChartSeriesUI.PointData pd = null;
		ChartSeriesUI.PointData pd1 = null;
		Font ft = getFont();
		int fontHeight = ft.getHeight();
		...
		for (int i = 0; i < series.getDataCount() - 1; i++) {
			...
			if (series.isShowMarks()) {
				if (type == ChartSeriesUI.svNone) {
					value = "";
				}
				if (type == ChartSeriesUI.svValue) {
					value = String.valueOf(pd.yValue);
				}
				if (type == ChartSeriesUI.svXValue) {
					value = String.valueOf(pd.xValue);
				}
				if (type == ChartSeriesUI.svPercent) {
					value = pd.percent;
				}
				if (type == ChartSeriesUI.svXYValue) {
					value = pd.xValue + "/" + pd.yValue;
				}
				if (type == ChartSeriesUI.svXValuePercent) {
					value = pd.xValue + "/" + pd.percent;
				}
				if (type == ChartSeriesUI.svLabel) {
					value = pd.label;
				}
				if (type == ChartSeriesUI.svLabelValue) {
					value = pd.label + "/" + pd.yValue;
				}
				if (type == ChartSeriesUI.svLabelPercent) {
	...
	// 散点图
	private void paintPointSeries(Graphics g, ChartSeriesUI series) {
		ChartSeriesUI.PointData pd = null;
		Font ft = getFont();
		int fontHeight = ft.getHeight();
		int x1 = 0, y1 = 0;
	...
	// 柱图
	private void paintBarSeries(Graphics g, ChartSeriesUI series) {
		ChartSeriesUI.PointData pd = null;
		Font ft = getFont();
		int fontHeight = ft.getHeight();
	...
	// 饼图
	private void paintPieSeries(Graphics g, ChartSeriesUI series) {
		ChartSeriesUI.PointData pd = null;
		Font ft = getFont();
		int fontHeight = ft.getHeight();
		int x1 = 0, y1 = 0;
	...
	// 图表增加删除
	public void addSeries(ChartSeriesUI series) {
	...
	public void removeSeries(ChartSeriesUI series) {
		if (seriesList.contains(series)) {
