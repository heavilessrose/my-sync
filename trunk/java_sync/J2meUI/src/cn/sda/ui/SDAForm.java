...
public class SDAForm extends SDABaseControl {
	...
	private final void DrawForm() {
		...
		if (Application.getSkins() == null) {
			...
			skin = SDAImageUtils.processImage(skin, this.width - 2,
					this.height - 1, SDAImageUtils.MODE_STRETCH);
	...
	protected final void keyReleased(int keyCode) {
		...
		if (popControl != null) {
			...
			if (focusControl == null
					|| (keyName.equals(SDAConsts.KEY_UP) && focusControl
							.canUpTabPrior())
					|| (keyName.equals(SDAConsts.KEY_LEFT) && focusControl
							.canLeftTabPrior())) {
				this.PriorTabOrder();
				operateControl = null;
			}
			if (focusControl == null
					|| (keyName.equals(SDAConsts.KEY_DOWN) && focusControl
							.canDownTabNext())
					|| (keyName.equals(SDAConsts.KEY_RIGHT) && focusControl
							.canRightTabNext())) {
				this.NextTabOrder();
				operateControl = null;
			}
