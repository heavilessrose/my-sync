 ...
 * @version 1.0
 */
public class SDAScrollBox extends SDABaseControl {
	// 边框
	...
	// 子组件参照组件
	private SDABaseControl refLeftControl = null;
	private SDABaseControl refTopControl = null;
	// 容器
	private SDABevel backPanel = null;

	public SDAScrollBox() {
		super();
		backColor = SDAConsts.clWhite;
		backPanel = new SDABevel();
		backPanel.setShape(SDAConsts.bvSpacer);
		backPanel.setStyle(SDAConsts.pbNone);
	...
	private void setChildsPos() {
		...
		for (int i = 0; i < backPanel.ctrlList.size(); i++) {
			ctrl = (SDABaseControl) backPanel.ctrlList.elementAt(i);
			if (!ctrl.visible) {
				continue;
			}
	...
	public final void AddControl(SDABaseControl control) {
