...
public class Widget {
	...
	protected class BindInstruction implements LinkedListItem {
		 ...
		 * @param pattern
		 */
		private BindInstruction(String attribute, String[] bindedProperties,
				String pattern) {
		...
		/**
		 * @param property
		 * @return <code>true</code> if given <code>property</code> is binded by
		 *         the instruction.
		 */
			...
			if (dataProvider != null) {
				if (pattern != null) {
					if (!setAttribute(
							attribute,
							Kuix
									.processI18nPattern(processBindPropertyVariables(pattern)))) {
						throw new IllegalArgumentException(attribute);
					}
				} else if (bindedProperties != null
						&& bindedProperties.length == 1) {
					if (!setObjectAttribute(attribute, dataProvider
							.getValue(bindedProperties[0]))) {
						throw new IllegalArgumentException(attribute);
					}
				}
			}
		...
		/**
		 * Process the bind property variables and replace them by their values.
		 * <p>
		 * Syntax : <code>@{varName[|Null text]}</code>
		 * </p>
		 * <p>
		 * Example with var1="Hello" and var2=null
		 * </p>
		 * <p>
		 * <code>@{var1}</code> is transform to <code>Hello</code>
		 * </p>
		 * <p>
		 * <code>@{var1|Nothing}</code> is transform to <code>Hello</code>
		 * </p>
		 * <p>
		 * <code>@{var2|Nothing}</code> is transform to <code>Nothing</code>
		 * </p>
		 * 
		 * @param rawData
		...
		private String processBindPropertyVariables(String rawData) {
			int posStart = rawData
					.indexOf(KuixConstants.BIND_PROPERTY_START_PATTERN);
			if (posStart != -1) {
				int posEnd = rawData.indexOf(
						KuixConstants.PROPERTY_END_PATTERN, posStart);
				if (posEnd != -1) {
					StringBuffer buffer = new StringBuffer(rawData.substring(0,
							posStart));
					String variable = rawData.substring(posStart + 2, posEnd);
					String variableValue = null;
					int posPipe = variable
							.indexOf(KuixConstants.PROPERTY_ALTERNATIVE_SEPRATOR_PATTERN);
					if (posPipe != -1) {
						if (dataProvider != null) {
							variableValue = dataProvider
									.getStringValue(variable.substring(0,
											posPipe));
						}
					...
					return buffer.append(
							processBindPropertyVariables(rawData
	...
	/**
	 * Set the <code>value</code> to the specified attribute representing by the
	 * <code>name</code>
	 * 
	 * @param name
	 * @param value
	...
	public boolean setAttribute(String name, String value) {
		...
		if (KuixConstants.SHORTCUTS_ATTRIBUTE.equals(name)
				|| KuixConstants.PRESSED_SHORTCUTS_ATTRIBUTE.equals(name)) {
	...
	/**
	 * Set the Object <code>value</code> to the specified attribute representing
	 * by the <code>name</code>.
	 * 
	 * @param name
	 * @param value
	 * @return
	 ...
	 * 
	 * @param name
	 * @return <code>true</code> if the given attribute require an Object value
	 */
	...
	/**
	 * Return the inner width of this widget (exluding margin, border and
	 * padding).
	 * 
	...
	/**
	 * Return the inner height of this widget (exluding margin, border and
	 * padding).
	 * 
	...
	public void setBounds(int x, int y, int width, int height) {
		if (invalidated || this.x != x || this.y != y || this.width != width
				|| this.height != height) {
	 ...
	 * 
	 * @param authorStyle
	 *            the authorStyle to parse
	 */
	public void parseAuthorStyle(String rawAuthorStyle) {
		Style[] styles = Kuix.getConverter().convertStyleSheets(getTag(),
				rawAuthorStyle);
		if (styles.length > 0) {
	...
	/**
	 * @return <code>true</code> if one or more data bind instructions are
	 *         defined
	 */
	 ...
	 * @param pattern
	 */
	public void setAttributeBindInstruction(String attribute,
			String[] properties, String pattern) {
		...
		bindInstructions
				.add(new BindInstruction(attribute, properties, pattern));
	}
	 ...
	 * 
	 * @param rawShortcuts
	 *            a string that contains keys and / or action.
	 * @param eventType
	 *            the event type. <code>KuixConstants.KEY_PRESSED_EVENT_TYPE,
	 ...
	 * This method override all previous shortcuts and shortcut's actions
	 * defined by <code>setShortcut</code> method.
	 * 
	...
	/**
	 * @return <code>true</code> if shortcuts are set for all most one event
	 *         type.
	 */
	public boolean hasShortcuts() {
		return (pressedShortcutActions != null
				|| repeatedShortcutActions != null || releasedShortcutActions != null);
	}
	 ...
	 *            KuixConstants.KEY_REPEATED_EVENT_TYPE or
	 *            KuixConstants.KEY_RELEASED_EVENT_TYPE</code>
	 * @return <code>true</code> if shortcuts are set for a specified event type
	 */
	...
	/**
	 * Check if a <code>kuixKeyCode</code> is compatible with widget's
	 * shortcuts.
	 * 
	 * @param kuixKeyCode
	 * @param eventType
	...
	public Layout getLayout() {
		if ((validCachedFlags & VALID_CACHED_FLAG_LAYOUT) != VALID_CACHED_FLAG_LAYOUT) {
			cachedLayout = (Layout) getStylePropertyValue(
	...
	public LayoutData getLayoutData() {
		if ((validCachedFlags & VALID_CACHED_FLAG_LAYOUT_DATA) != VALID_CACHED_FLAG_LAYOUT_DATA) {
			cachedLayoutData = (LayoutData) getStylePropertyValue(
					KuixConstants.LAYOUT_DATA_STYLE_PROPERTY, false);
	...
	public Metrics getMinSize() {
		if ((validCachedFlags & VALID_CACHED_FLAG_MIN_SIZE) != VALID_CACHED_FLAG_MIN_SIZE) {
			cachedMinSize = (Metrics) getStylePropertyValue(
					KuixConstants.MIN_SIZE_STYLE_PROPERTY, false);
	...
	/**
	 * Returns the color value. By default the value is <code>null</code>.
	 * 
	 * @return the color
	 */
	public Color getColor() {
		if ((validCachedFlags & VALID_CACHED_FLAG_COLOR) != VALID_CACHED_FLAG_COLOR) {
			Object colorValue = getStylePropertyValue(
					KuixConstants.COLOR_STYLE_PROPERTY, true);
			if (colorValue != null) {
	...
	/**
	 * Returns the border color array. The array length is 4. By default the
	...
	public Color[] getBorderColor() {
		if ((validCachedFlags & VALID_CACHED_FLAG_BORDER_COLOR) != VALID_CACHED_FLAG_BORDER_COLOR) {
			Object borderColorValue = getStylePropertyValue(
					KuixConstants.BORDER_COLOR_STYLE_PROPERTY, false);
	...
	public int getBorderStroke() {
		if ((validCachedFlags & VALID_CACHED_FLAG_BORDER_STROKE) != VALID_CACHED_FLAG_BORDER_STROKE) {
			Object borderStrokeValue = getStylePropertyValue(
					KuixConstants.BORDER_STROKE_STYLE_PROPERTY, false);
	...
	public Image[] getBorderImage() {
		if ((validCachedFlags & VALID_CACHED_FLAG_BORDER_IMAGE) != VALID_CACHED_FLAG_BORDER_IMAGE) {
			Object borderImageValue = getStylePropertyValue(
					KuixConstants.BORDER_IMAGE_STYLE_PROPERTY, false);
	...
	/**
	 * Returns the border alignments array. The array length is 8. By default
	...
	public Alignment[] getBorderAlign() {
		if ((validCachedFlags & VALID_CACHED_FLAG_BORDER_ALIGN) != VALID_CACHED_FLAG_BORDER_ALIGN) {
			Object borderAlignValue = getStylePropertyValue(
					KuixConstants.BORDER_ALIGN_STYLE_PROPERTY, false);
	...
	public Color getBackgroundColor() {
		if ((validCachedFlags & VALID_CACHED_FLAG_BACKGROUND_COLOR) != VALID_CACHED_FLAG_BACKGROUND_COLOR) {
			Object backgroundColorValue = getStylePropertyValue(
					KuixConstants.BACKGROUND_COLOR_STYLE_PROPERTY, false);
	...
	public Image[] getBackgroundImage() {
		if ((validCachedFlags & VALID_CACHED_FLAG_BACKGROUND_IMAGE) != VALID_CACHED_FLAG_BACKGROUND_IMAGE) {
			Object backgroundImageValue = getStylePropertyValue(
					KuixConstants.BACKGROUND_IMAGE_STYLE_PROPERTY, false);
	...
	/**
	 * Returns the backroundAlign or alignment list if multi alignments are
	 * defined.
	 * 
	...
	public Alignment[] getBackgroundAlign() {
		if ((validCachedFlags & VALID_CACHED_FLAG_BACKGROUNG_ALIGN) != VALID_CACHED_FLAG_BACKGROUNG_ALIGN) {
			Object backgroundAlignValue = getStylePropertyValue(
					KuixConstants.BACKGROUND_ALIGN_STYLE_PROPERTY, false);
	...
	public Repeat[] getBackgroundRepeat() {
		if ((validCachedFlags & VALID_CACHED_FLAG_BACKGROUND_REPEAT) != VALID_CACHED_FLAG_BACKGROUND_REPEAT) {
			Object backgroundRepeatValue = getStylePropertyValue(
					KuixConstants.BACKGROUND_REPEAT_STYLE_PROPERTY, false);
	...
	public Color getGrayedColor() {
		if ((validCachedFlags & VALID_CACHED_FLAG_GRAYED_COLOR) != VALID_CACHED_FLAG_GRAYED_COLOR) {
			Object grayedColorValue = getStylePropertyValue(
					KuixConstants.GRAYED_COLOR_STYLE_PROPERTY, true);
	...
	public Gap getGap() {
		if ((validCachedFlags & VALID_CACHED_FLAG_GAP) != VALID_CACHED_FLAG_GAP) {
			cachedGap = (Gap) getStylePropertyValue(
	...
	public Span getSpan() {
		if ((validCachedFlags & VALID_CACHED_FLAG_SPAN) != VALID_CACHED_FLAG_SPAN) {
			cachedSpan = (Span) getStylePropertyValue(
					KuixConstants.SPAN_STYLE_PROPERTY, false);
	...
	public Weight getWeight() {
		if ((validCachedFlags & VALID_CACHED_FLAG_WEIGHT) != VALID_CACHED_FLAG_WEIGHT) {
			cachedWeight = (Weight) getStylePropertyValue(
					KuixConstants.WEIGHT_STYLE_PROPERTY, false);
	...
	public Widget getWidgetAt(int mx, int my) {
		...
		return getWidgetAt(mx, my, x + margin.left, y + margin.top, width
				- margin.left - margin.right, height - margin.top
				- margin.bottom);
	}
	 ...
	 * @return the child {@link Widget} under mx, my point
	 */
	public Widget getWidgetAt(int mx, int my, int x, int y, int width,
			int height) {
		...
		if (visible && (mx >= x) && (my >= y) && (mx < x + width)
				&& (my < y + height)) {
	 ...
	 * 
	 * @return <code>true</code> if this {@link Widget} is a child of a focused
	 *         widget
	 */
	 ...
	 * <code>referenceWidget</code> child. The referenceWidget need to be a
	...
	public void bringNear(Widget widget, Widget referenceWidget, boolean after) {
		if (widget == null || referenceWidget == null || widget.parent != this
				|| referenceWidget.parent != this || after
				&& referenceWidget.next == widget || !after
				&& referenceWidget.previous == widget) {
			return;
		}
	...
	/**
	 * Remove the current {@link Widget} from its parent.<br/>
	 * <b>Caution</b> : this method do NOT cleanUp the widgets. It only remove
	 * widget from widget tree.
	 */
	public void remove() {
		if (parent == null) {
			return;
	 ...
	 * 
	 * @param fromWidget
	 *            the Widget responsible of the invalidation.
	 */
	...
	protected void invalidateAppearanceRegion(int x, int y, int width,
			int height) {
		if (parent != null) {
			parent.invalidateAppearanceRegion(x + this.x, y + this.y, width,
					height);
		}
	}
	...
	public void paintBackground(Graphics g) {
		...
		int width = getWidth() - margin.left - border.left - border.right
				- margin.right;
		int height = getHeight() - margin.top - border.top - border.bottom
				- margin.bottom;
		...
		if (images != null) {
			...
			int backgroundCount = Math.max(images.length, Math.max(
					alignments.length, repeats.length));
			Repeat repeat;
	...
	public void paintBorder(Graphics g) {
		...
		if (borderImages != null) {
			...
			if (borderImages[0] != null) {
				paintMosaicImage(g, borderImages[0], x + border.left, y, width
			...
			if (borderImages[1] != null) {
				paintMosaicImage(g, borderImages[1], x + width - border.right,
						y, border.right, border.top, extractBorderAlignment(1,
								alignments), Integer.MAX_VALUE,
						Integer.MAX_VALUE);
			}

			// Right
			if (borderImages[2] != null) {
				paintMosaicImage(g, borderImages[2], x + width - border.right,
						y + border.top, border.right, height - border.top
								- border.bottom, extractBorderAlignment(2,
								alignments), Integer.MAX_VALUE,
						Integer.MAX_VALUE);
			}
			...
			if (borderImages[3] != null) {
				paintMosaicImage(g, borderImages[3], x + width - border.right,
						y + height - border.bottom, border.right,
						border.bottom, extractBorderAlignment(3, alignments),
						Integer.MAX_VALUE, Integer.MAX_VALUE);
			}

			// Bottom
			if (borderImages[4] != null) {
				paintMosaicImage(g, borderImages[4], x + border.left, y
						+ height - border.bottom, width - border.left
						- border.right, border.bottom, extractBorderAlignment(
						4, alignments), Integer.MAX_VALUE, Integer.MAX_VALUE);
			}

			// Bottom left
			if (borderImages[5] != null) {
				paintMosaicImage(g, borderImages[5], x, y + height
			...
			if (borderImages[6] != null) {
				paintMosaicImage(g, borderImages[6], x, y + border.top,
						border.left, height - border.top - border.bottom,
			...
			if (borderImages[7] != null) {
				paintMosaicImage(g, borderImages[7], x, y, border.left,
						border.top, extractBorderAlignment(7, alignments),
						Integer.MAX_VALUE, Integer.MAX_VALUE);
			}
	 ...
	 */
	private Alignment extractBorderAlignment(int borderIndex,
	 ...
	 * @param repeatY
	 */
	protected void paintMosaicImage(Graphics g, Image image, int x, int y,
			int width, int height, Alignment alignment, int repeatX, int repeatY) {
		...
		int imax = Math.min(repeatX, MathFP.toInt(MathFP.ceil(MathFP.div(width,
				imageWidth))));
		int jmax = Math.min(repeatY, MathFP.toInt(MathFP.ceil(MathFP.div(
				height, imageHeight))));
		byte dx = 1;
		byte dy = 1;
		...
		for (int i = 0; i < imax; ++i) {
			for (int j = 0; j < jmax; ++j) {
				g.drawImage(image, x + i * imageWidth * dx, y + j * imageHeight
						* dy, Graphics.SOLID);
			}
		}

		// Restore widget clip
		g.setClip(currentClipX1, currentClipY1, currentClipX2 - currentClipX1,
	...
	public void paintImpl(Graphics g) {
		...
		if ((width > 0) && (height > 0)) {
			if ((clipX + clipWidth <= x) || (clipX >= x + width)
					|| (clipY + clipHeight <= y) || (clipY >= y + height)) {
		...
		// Paint children
		Insets insets = getInsets();
		g.clipRect(insets.left, insets.top, width - insets.left - insets.right,
	...
	protected Object getStylePropertyValue(String name, boolean inherited) {
		...
		if (inherited && parent != null) {
			Object parentStyleProperty = parent.getStylePropertyValue(name,
					inherited);
	...
	/**
	 * @param style
	 * @return <code>true</code> if the {@link Style} is compatible with this
	 *         widget
	 */
	private boolean checkStyleCompatibility(Style style) {
		...
		for (StyleSelector selector = style.getSelector(); selector != null; selector = selector.parent) {
			widget = getCompatibleWidget(selector, widget, selector != style
					.getSelector());
			if (widget == null) {
				return false;
			}
		...
		for (; widget != null; widget = checkParents ? widget.parent : null) {
			...
			if (selector.hasClass()) {
				...
				if (widgetStyleClasses != null) {
					...
					for (; i >= 0; --i) {
						if (selector.getStyleClass().equals(
								widgetStyleClasses[i])) {
							isCompatible = true;
							break;
						}
					}
	...
	/**
	 * Requests the focus of the backward focusable {@link Widget}. Starts from
	 * this.
	 */
	...
	/**
	 * Requests the focus of the forward focusable {@link Widget}. Starts from
	 * this.
	 */
	 ...
	 */
	public Widget getOtherFocus(Widget root, Widget focusedWidget,
			Widget nearestFocusableWidget, boolean forward,
			Alignment direction, boolean checkItself, boolean checkChild,
			boolean checkParent) {
		boolean isVisible = isVisible();
		if (checkItself && root != this && !isFocused() && isFocusable()
				&& isVisible) {
			...
			if (childWidget != null) {
				nearestFocusableWidget = childWidget.getOtherFocus(root,
						focusedWidget, nearestFocusableWidget, forward,
						direction, true, true, false);
				if ((focusedWidget == null || direction == null)
						&& nearestFocusableWidget != null) {
		...
		if (root != this) {
			...
			if (otherWidget != null) {
				nearestFocusableWidget = otherWidget.getOtherFocus(root,
						focusedWidget, nearestFocusableWidget, forward,
						direction, true, true, false);
				if ((focusedWidget == null || direction == null)
						&& nearestFocusableWidget != null) {
			...
			if (checkParent && parent != null && isVisible) {
				nearestFocusableWidget = parent.getOtherFocus(root,
						focusedWidget, nearestFocusableWidget, forward,
	 ...
	 *         <code>originWidget</code> than <code>bestWidget</code> according
	 *         to <code>direction</code>.
	 */
	private boolean isNearest(Widget originWidget, Widget nearestWidget,
			Alignment direction) {
		if (originWidget != null && direction != null) {
			...
			if (nearestWidget != null) {
				int nearestDx = nearestWidget.getDisplayX()
						+ nearestWidget.visualCenterX - origX;
				int nearestDy = nearestWidget.getDisplayY()
						+ nearestWidget.visualCenterY - origY;
				if (direction.isTop() || direction.isBottom()) {
					if (Math.abs(dy) <= Math.abs(nearestDy)
						...
						if (Math.abs(dx) <= Math.abs(nearestDx)) {
							return Math.abs(dx) + Math.abs(dy) < Math
									.abs(nearestDx)
									+ Math.abs(nearestDy);
						}
					}
					...
					if (Math.abs(dx) <= Math.abs(nearestDx)
							&& dx != 0
							&& (direction.isRight() && dx > 0 || direction
									.isLeft()
									&& dx < 0)) {
						if (Math.abs(dy) <= Math.abs(nearestDy)) {
							return Math.abs(dx) + Math.abs(dy) < Math
		...
		if (isShortcutKeyCodeCompatible(kuixKeyCode, type)) {
			...
			if (shortcutActions != null && shortcutActions.length > 4) {
				...
				while (index < shortcutActions.length) {
					...
					index += 4;
					int actionLength = NumberUtil.toShort(shortcutActions,
							index);
					index += 2;
					String action = new String(shortcutActions, index,
							actionLength);
	...
	/**
	 * Event called when the <code>widjet</code> is removed from
	 * <code>parent</code>.
	 */
	 ...
	 * 
	 * @param widget
	 *            The widget child witch is added
	 */
