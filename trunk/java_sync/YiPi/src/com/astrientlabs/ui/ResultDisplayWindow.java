...
public class ResultDisplayWindow extends Window implements Runnable,
	...
	protected void sizeChanged(int w, int h) {
		...
		if (resultDisplayWidget != null) {
			resultDisplayWidget.widgetHeight = getHeight() - footerHeight
					- headerHeight;
		}
	}
	...
	protected void moveItem(boolean forward) {
		if (!running) {
			int index = Math.max(0, this.searchQuery.getResults().indexOf(
					searchResult))
					+ ((forward) ? 1 : -1);
			if (index > -1 && index < this.searchQuery.getResults().size()) {
				searchResult = (SearchResult) searchQuery.getResults()
						.elementAt(index);
			} else {
				searchResult = null;
			}
	...
	protected void handleKeyPressed(int keyCode) {
		...
		if (action == LEFT) {
			...
			if (searchResult != null) {
				JobRunner.instance
						.run(new DownloadImageJob(this, searchResult));
			}
		} else if (action == UP) {
	...
	public void run() {
		if (!running) {
			...
			try {
				...
				if (searchResult != null
						&& searchResult.getUrl().startsWith("http")) {

					Image image = (Image) imageCache.get(searchResult.getUrl());

					if (image == null) {
						AdWindow.instance.setMessage("fetching "
								+ searchResult.getTitle());
						navigateTo(AdWindow.instance);
						image = searchResult.getImage(
								resultDisplayWidget.widgetWidth,
								resultDisplayWidget.widgetHeight);
						imageCache.put(searchResult.getUrl(), image);
						Network.postOp();
					}

					if (running) {
						resultDisplayWidget.setImage(image, searchResult
								.getTitle());
