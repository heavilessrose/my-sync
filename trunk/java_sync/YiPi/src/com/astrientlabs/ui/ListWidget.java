...
{       
    ...
    {
        //TODO: check doc
        if ( model.size() > size ) model.setSize(size);
    }
    ...
    public void determineWindowStart()
    {
        if ( positionChanged )
        {
            ...
            for ( int i = windowStart, stop = model.size(); i < position && i < stop; i++ )
            {
                postitionStartY += ((Widget)model.elementAt(i)).widgetHeight;
            }
            
            positionEndY = postitionStartY + ((Widget)model.elementAt(position)).widgetHeight;
            ...
            {
                //move down
                for ( int i = windowStart, stop = model.size(); positionEndY > (startY+widgetHeight) && i <= position && i < stop; i++ )
                {
                    windowStart++;
    ...
    public void paint(Graphics g, int x, int y, int maxX, int maxY, boolean hasFocus)
	{
        ...
        if ( !model.isEmpty() )
        {
            ...
            for ( int i = windowStart; i < count && y < (startY+widgetHeight-padding-padding); i++ )
            {
                ...
                widget.paint(g,x+padding,y,widgetWidth-scrollbarWidth-padding-padding-1,startY+widgetHeight,hasFocus && (position==i));
