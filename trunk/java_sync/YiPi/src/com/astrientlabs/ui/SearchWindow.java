...
public class SearchWindow extends Window implements ProgressListener,
		TextInputListener {
	...
	public SearchWindow() {
		...
		moreResultsWidget = new CannedSearchResultWidget(this,
				Images.MORE_RESULTS, TextMap.instance.get("text.more"), "");
		thumbnailFetcher = new ThumbnailFetcher(2);

		logs = new LogWindow(this, (int) (getWidth() * .90),
				(int) (getHeight() * .60), Colors.POPUP_BG, 15);
		Logger.instance.setListener(logs);

		about = new PopupWidget(this, (int) (getWidth() * .90),
				(int) (getHeight() * .60), Colors.POPUP_BG);
		about.add(new LabelWidget(this, "version: "
				+ Yipi.instance.getAppProperty("MIDlet-Version")));
		about.add(new LabelWidget(this, "platform: "
				+ System.getProperty("microedition.platform")));
		about.add(new LabelWidget(this, "build date: "
				+ Yipi.instance.getAppProperty("astrientlabs-build-date")));
		about.add(new LabelWidget(this, "total mem: "
				+ Strings.getSizeString(Runtime.getRuntime().totalMemory())));
		...
		this.listWidget = new ListWidget(this, getHeight() - footerHeight
				- headerHeight - textInputWidget.widgetHeight,
				Colors.BACKGROUND);
	...
	public void widgetSelected(Widget who, Object arg) {
		if (who == moreResultsWidget) {
			...
			CannedResult cannedResult = (CannedResult) ((CannedSearchResultWidget) who)
					.getValue();
			if (cannedResult.getType() == CannedResult.TYPE_POPUP) {
				...
				boolean filtering = !Yipi.instance.systemPrefs.getBoolean(
						"filter", false);
				Yipi.instance.systemPrefs.put("filter", String
						.valueOf(filtering));
			...
			ResultDisplayWindow.instance.setSearchResult(searchResultWidget
					.getSearchResult());
	...
	public void search(String queryString) {
		if (queryString != null) {
			if (queryString.trim().length() > 0) {
				...
				query.setFilter(Yipi.instance.systemPrefs.getBoolean("filter",
						false));
				query.setTerm(queryString);
				query.setPageSize(Yipi.instance.systemPrefs.getInt(
						"maxresults", 20));
	...
	public void update(Object who, long position) {
		if (who == query) {
			SearchResult tempResult = (SearchResult) ((SearchQuery) who)
					.getResults().lastElement();
			...
			if (tempResult instanceof CannedResult) {
				listWidget
						.add(new CannedSearchResultWidget(this,
								Images.CANNED_RESULT, tempResult.getTitle(),
								tempResult));
			} else {
				...
				thumbnailFetcher.run(new FetchThumbnailJob(searchResultWidget,
						listWidget));
			}
