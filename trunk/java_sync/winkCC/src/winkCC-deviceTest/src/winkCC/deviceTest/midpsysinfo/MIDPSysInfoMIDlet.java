...
public class MIDPSysInfoMIDlet extends MIDlet implements CommandListener {
	/*
	 * private final Command displayInfoCmd = new
	 * Command(Locale.get("cmd.display"), Command.SCREEN, 1); private final
	 * Command soundInfoCmd = new Command(Locale.get("cmd.multimedia"),
	 * Command.SCREEN, 2); private final Command systemInfoCmd = new
	 * Command(Locale.get("cmd.system"), Command.SCREEN, 3); private final
	 * Command gameControlsInfoCmd = new Command(Locale.get("cmd.keys"),
	 * Command.SCREEN, 4); private final Command arithmeticBenchmarkCmd = new
	 * Command(Locale.get("cmd.arithmeticBenchmark"), Command.SCREEN, 6);
	 * private final Command creditsCmd = new Command(Locale.get("cmd.credits"),
	 * Command.SCREEN, 6);
	 */
	private final Command exitCmd = new Command(Locale.get("cmd.exit"),
			Command.EXIT, 10);
	private final Command backCmd = new Command(Locale.get("cmd.back"),
			Command.BACK, 1);
	...
	private void setCommands(Displayable disp) {
		/*
		 * disp.addCommand( this.displayInfoCmd ); disp.addCommand(
		 * this.soundInfoCmd ); disp.addCommand( this.systemInfoCmd );
		 * disp.addCommand( this.gameControlsInfoCmd ); disp.addCommand(
		 * this.arithmeticBenchmarkCmd ); disp.addCommand( this.creditsCmd );
		 */
