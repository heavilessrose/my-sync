...
public class DefaultTCPSession extends AbstractSession {
	...
	public DefaultTCPSession(SocketChannel sc, SelectionKey sk,
		...
		this.in = ByteBuffer.allocate(readRecvBufferSize);
	...
	protected void readFromBuffer() {
		updateTimeStamp();
		if (!in.hasRemaining()) { // have no room for contain new data
			in = ByteBufferUtils.increaseBufferCapatity(in);
		}
		if (closed)
			return;
		int n = -1;
		int readCount = 0;
		try {
			while ((n = ((ReadableByteChannel) selectableChannel).read(in)) > 0) {
				readCount += n;
			}
			if (readCount > 0) {
				in.flip();
	...
	int blockingRead() throws ClosedChannelException, IOException {
		...
		try {
			...
			if (code > 0) {
				do {
					n = ((ReadableByteChannel) selectableChannel).read(in);
	...
	public void decode() {
		...
		while (in.hasRemaining()) {
			try {
				pkt = this.decoder.decode(in);
				if (pkt == null) {
					break;
				} else {
					statistics.statisticsRead(size - in.remaining());
					size = in.remaining();
				}
