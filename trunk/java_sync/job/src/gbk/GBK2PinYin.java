...
public class GBK2PinYin {

	private String[] ALL_VALUE = { "zuo", "zun", "zui", "zuan", "zu", "zou",
	...
	private int[] ALL_CODE = { -10254, -10256, -10260, -10262, -10270, -10274,
	...
	/** 得到字符串的拼音 */
	public String getAllPinyin(String GBK) {
		if (null == GBK || "".equals(GBK.trim())) {
			return GBK;
		}
		char[] chars = GBK.toCharArray();
		StringBuffer retuBuf = new StringBuffer();
		for (int i = 0, Len = chars.length; i < Len; i++) {
			retuBuf.append(getAllPinyin(chars[i]));
		} // end of for
		return retuBuf.toString();
	}

	/** 得到单个字符的拼音 */
	public String getAllPinyin(char GBK) {
		int ascii = getChsAscii(GBK);
		if (ascii == 0) { // 取ascii时出错
			return String.valueOf(GBK);
		} else {
	...
	/** 得到汉字的ASCII编码, 未处理多音字(多音字的处理: 如何使多个汉字对应一个拼音? ) */
	private int getChsAscii(char cn) {
		byte[] bytes = null;
		try {
	...
	// }
	// }
	// // System.out.println("pinYin len: " + pinYin.length);
	// return pinYin[ind];
	// }

	public char getFirstPinYin(char ch) {
		if (ch >= 0 && ch <= 0x7F) {
			return ch;
		}
		int gb = 0;
	...
	public String getFirstPinYin(String src) {
		StringBuffer sb = new StringBuffer();
		int len = src.length();
		int i;
		for (i = 0; i < len; i++) {
			sb.append(getFirstPinYin(src.charAt(i)));
		}
		return sb.toString();
	}
	...
	public static void main(String[] args) {
		...
		System.out.println(new GBK2PinYin().getFirstPinYin("单字符"));
	}
}
