...
/**
 * MenuResource is a class that contain all menu message. It also
 * internatinalize menu content following specify locale.
 * 
 * @author Ken
 * @version %I%,%G%
 */
public class MenuResource {
	...
	/**
	 * Load language definition from specify locale if the specify locale not
	 * correct, default locale will be loaded instead
	 */
	public static void loadMenuResouce(Locale locale) {
		try {
			menuResource = ResourceBundle.getBundle(MSG_RESOURCE, locale);
		} catch (Exception e) {
			menuResource = ResourceBundle.getBundle(MSG_RESOURCE, new Locale(
					DEFAULT_LOCALE));
		}
	}
