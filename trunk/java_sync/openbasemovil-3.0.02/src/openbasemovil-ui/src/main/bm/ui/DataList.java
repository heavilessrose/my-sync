    ...
    implements EventListener
{
    ...
    RowSet          rowSet;
    Font            font        = Font.getFont( Font.FACE_PROPORTIONAL, Font.STYLE_PLAIN, Ui.getFontSize() );
     ...
     */
    public synchronized void setRowSet( RowSet rowSet )
            throws DBException,
    ...
    public synchronized void rebuild()
    {
        ...
        if( rowSet != null && rowSet.size() > 0 )
        {
            try
            {
                ...
                // Anyway, the message line takes fontHeight plus 2 pixels for space
                // and separator line
    ...
    {
        if( rowSet != null && selectedIndex > -1 && selectedIndex < rowSet.size() )
        {
     ...
     */
    public void addToolbarItem( final int code, final String text, final String help )
    {
     ...
     * @param help help text
     */
    public void addToolbarItem( final int code, final Image image, final String help )
    {
        final ToolButton button = new ToolButton();
        button.action = code;
        button.help = help != null ? ResourceManager.getResource( help ) : null;
     ...
     * @param mode mode, one of PLAIN, DATE, TIME or MONEY
     */
    ...
    {
        final DataList.FieldDescription fd = (DataList.FieldDescription)
...
//                g.fillArc( 0, 0, getWidth(), height, 45, -45 );
    ...
    )
    {
        ...
        if( selected || selectedLine )
        {
            ...
            g.fillRoundRect( 0, y, getWidth(), currentMaxLines * lineHeight, 2, 2 );
        }
    ...
    protected String buildLabel( final Row row )
    {
        ...
        try
        {
            ...
            {
                String value = null;
                final DataList.FieldDescription field =
                        (DataList.FieldDescription) i.nextElement();
                final Object raw = field.mode != STATIC ?
                                   resolve( row, field.name ) :
                                   field.label;
                if( raw != null )
                {
                    switch( field.mode )
                    ...
                    if( value == null )
                    {
                        ...
                        {
                            value = ResourceManager.getResource(
                                    field.resource + raw.toString()
                            );
                        }
                        else
                        {
    ...
    public class FieldDescription
    {
        ...
        )
        {
            ...
            this.label = label != null ?
                         ResourceManager.getResource( label ) :
                         null;
            this.trueLabel = trueLabel != null ?
                             ResourceManager.getResource( trueLabel ) :
                             ResourceManager.getResource( "global.Yes" );
            this.falseLabel = falseLabel != null ?
                              ResourceManager.getResource( falseLabel ) :
    ...
    private void moveList( final int offset )
    {
        ...
        repaint();
        // make sure the view is kept visible in case a progress view has been shown
        Application.getManager().getDisplay().setCurrent( this );
    }
    ...
    public void handleEvent( Event e )
    {
        ...
        {
            font = Font.getFont( Font.FACE_PROPORTIONAL, Font.STYLE_PLAIN, Ui.getFontSize() );
            dirty = true;
            if( isShown() )
            {
                repaint();
