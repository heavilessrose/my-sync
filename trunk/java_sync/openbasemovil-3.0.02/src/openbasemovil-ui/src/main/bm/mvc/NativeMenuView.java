 ...
 * @version $Revision$
 */
public class NativeMenuView extends AbstractView implements MenuView {
	...
	public static NativeMenuView create(final Controller controller,
			final MenuDef menuDef, final String name) {
		final String title = ResourceManager.getResource(menuDef.getTitle());
		final NativeMenuView menu = new NativeMenuView(controller, title,
				menuDef.getId());
		final Vector items = menuDef.getItems();
		final int count = items.size();
		final boolean hasImageMap = menuDef.getImageMap() != null;
		if (hasImageMap) {
			loadImages(name, menuDef.getImageMap(), menuDef.getImageMapBig(),
					count);
		}
		for (int i = 0; i < count; i++) {
			...
			if (active) {
				final String text = ResourceManager
						.getResource(item.getTitle());
				if (hasImageMap) {
					menu.addOption(text, (Image) images.get(fix), item
							.getAction());
				} else {
	...
	static void loadImages(final String name, final String imageMap,
			final String imageMapBig, final int count) {
		final Display display = Application.getManager().getDisplay();
		final int bestSize = display.getBestImageWidth(Display.LIST_ELEMENT);
		int size;
		if (bestSize > 0) {
		...
		String imageSource = size == 24 && imageMapBig != null ? imageMapBig
				: imageMap;
		final Image all = Util.loadImage(display.getClass(), imageSource);
		if (all != null) {
			int x = 0;
			for (int i = 0; i < count; i++) {
				final Image image = Image.createImage(all, x, 0, size, size,
						Sprite.TRANS_NONE);
				x += size;
	...
	protected NativeMenuView(final Controller controller, final String title,
			final int viewId) {
		super(controller);
		this.viewId = viewId;
	...
	public void commandAction(final Command command,
		...
		if (!handleAction(command)) {
			final ControllerEvent event = AbstractController.buildEvent(this,
					command, (Choice) displayable);
			if (event.getCode() == ControllerEvent.LIST_ITEM) {
				final Integer code = (Integer) map.get(new Integer(event
						.getIndex()));
				if (code != null) {
	...
	protected int addOption(final String text, final Image image,
			final int eventCode) {
