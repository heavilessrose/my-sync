 ...
 * @version $Revision$
 */
public abstract class BrowserView extends AbstractView implements ScrollerView {
	...
	protected BrowserView(final Controller controller, final String title,
	...
	protected void addStringItem(final String id, final String label,
			final String text, final int layout) {
	...
	protected void setItemText(final String id, final String text) {
		values.put(id, text);
		((StringItem) ((Hashtable) items.get(state)).get(id)).setText(text);
	}

	private StringItem newStringItem(final String label, final String text,
			final int layout) {
	...
	private void initButtons() {
		previousCommand = new Command(ResourceManager
				.getResource("global.Previous"), Command.ITEM, 100);
		nextCommand = new Command(ResourceManager.getResource("global.Next"),
				Command.ITEM, 100);
	...
	private ImageItem newButton(final Image image, final Command command) {
		final ImageItem next = new ImageItem(null, image, Item.LAYOUT_LEFT,
				null);
	...
	private Image getImage(final String resource, final String alt) {
		Image image;
		try {
			image = Image.createImage(getClass().getResourceAsStream(resource));
		} catch (IOException e) {
	...
	public boolean isShown() {
		final Displayable current = Application.getManager().getDisplay()
				.getCurrent();
		// noinspection MethodCallInLoopCondition
		for (Enumeration i = forms.elements(); i.hasMoreElements();) {
