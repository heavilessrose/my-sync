 ...
 * A form is identified by it's name, and that name is also the first part of
 * all the properties associated to it.<br/>
 ...
 * <dt>readOnly</dt>
 * <dd>If the form is a readonly one, can be &quot;true&quot; or
 * &quot;false&quot;</dd>
 * <dt>itemCount</dt>
 * <dd>The number of items in the form</dd>
 * <dt>saveOnAccept</dt>
 * <dd>If the data should be actually saved in the table or just the row
 * updated, by default true. Only has meaning when the view is not readOnly and
 * not browsable.</dd>
 * </dl>
 * Each item is defined using the form name, followed by the item index (from 0
 * to itemCount - 1), and then the item properties (required in bold):
 * <dl>
 * <dt><b>name</b></dt>
 * <dd>item name</dd>
 * <dt><b>type</b></dt>
 ...
 * <dt>constraints</dt>
 * <dd>constraints for editable fields (text, money, percent, integer,
 * fixedpoint)</dd>
 * <dt>appearance</dt>
 ...
 * @version $Revision$
 */
public class DataFormView extends AbstractView implements
		RowSetBrowser.Listener, ItemStateListener, DataboundView {
	private static final Log log = LogFactory.getLog("DataFormView");

	public static final String[] NOKIA_FULL_KEYBOARD = new String[] { "e61",
			"e70" };
	 ...
	 *            view name
	 * @param browsable
	 *            if it will be used with a RowSet
	 * @param formDef
	 *            form definition
	 */
	public DataFormView(final Controller controller, final String name,
			final boolean browsable, final FormDef formDef) {
	 ...
	 *            if it will be used with a RowSet
	 * @param readOnly
	 *            if the form should be created as readonly overriding
	 *            definition
	 * @param formDef
	 *            form definition
	 */
	public DataFormView(final Controller controller, final String name,
			final boolean browsable, final boolean readOnly,
			final FormDef formDef) {
		this(controller, name, browsable, new Boolean(readOnly), formDef);
	}
	 ...
	 *             if no space left
	 */
	public void bind(final RowSet rowSet) throws RSException, DBException,
	 ...
	 * 
	 * @param index
	 *            item index as declared in the configuration file.
	 * @return item
	 */
	...
	public TextField getTextField(final int index) {
		return (TextField) ((Form) displayable).get(browsable ? index + 1
				: index);
	}
	...
	public StringItem getStringItem(final int index) {
		return (StringItem) ((Form) displayable).get(browsable ? index + 1
				: index);
	}
	...
	public ImageItem getImageItem(final int index) {
		return (ImageItem) ((Form) displayable).get(browsable ? index + 1
				: index);
	}
	...
	public DateField getDateField(final int index) {
		return (DateField) ((Form) displayable).get(browsable ? index + 1
				: index);
	}
	...
	public ChoiceGroup getChoiceGroup(final int index) {
		return (ChoiceGroup) ((Form) displayable).get(browsable ? index + 1
				: index);
	}
	...
	private void build(final Boolean readOnlyOverride) throws IOException {
	...
	public void cancel() {
		try {
			...
			final Row row = browsable ? browser.getRowSet().getCurrent()
					: this.row;
			rebind(row);
		} catch (Exception e) {
			log.error(e);
	...
	public void save() {
		try {
			final Row row = browsable ? browser.getRowSet().getCurrent()
					: this.row;
			// noinspection MethodCallInLoopCondition
			for (final Enumeration i = changedItems.elements(); i
					.hasMoreElements();) {
				final Item item = (Item) i.nextElement();
				final FieldDefinition field = (FieldDefinition) boundItems
						.get(item);
	...
	private class FormDefinition {
		...
		public FormDefinition(final String name,
				final Boolean readOnlyOverride, final FormDef def) {
			this.name = name;
			this.id = def.getId();
			...
			saveOnAccept = def.getSaveOnAccept() == null
					|| def.getSaveOnAccept().booleanValue();
			defaultButtons = def.getDefaultButtons() == null
	...
	private class FieldDefinition {
		...
		public int getMaxValue() {
			return def.getItemType() == ItemDef.GAUGE ? ((GaugeDef) def)
					.getMaxValue() : 0;
		}

		public int getChoiceType() {
			return def.getItemType() == ItemDef.CHOICE ? ((ChoiceDef) def)
					.getChoiceType() : 0;
		}
		...
		private Item createButton() throws IOException {
			Item item;
			final ButtonDef buttonDef = (ButtonDef) def;

			if (buttonDef.getImage() != null) {
				final Image img = Image.createImage(getClass()
						.getResourceAsStream(buttonDef.getImage()));
				item = new ImageItem(null, img, Item.BUTTON, null);
			} else {
				item = new StringItem(null, ResourceManager
						.getResource(buttonDef.getLabel()), Item.BUTTON);
			}
			Command cmd = null;
			...
			if (cmd == null) {
				cmd = new Command(ResourceManager.getResource(buttonDef
						.getLabel()), Command.ITEM, 1);
			}
			addAction(new Action(getName(), cmd, buttonDef.getCode(), item),
					true);
		...
		private Item createItem(final int constraints) {
			...
			Item item;
			final String label = ResourceManager
					.getResource(textDef.getLabel())
					+ (textDef.getLabelExtra() != null ? textDef
							.getLabelExtra() : "");
			if (readOnly) {
				item = new StringItem(label, textDef.getText(), textDef
						.getAppearance());
			} else {
				item = new TextField(label, textDef.getText(), size,
						constraints);
			}
		...
		private Item createChoice() {
			...
			final String label = ResourceManager.getResource(choiceDef
					.getLabel())
					+ (choiceDef.getLabelExtra() != null ? choiceDef
							.getLabelExtra() : "");

			final ChoiceGroup item = new ChoiceGroup(label, choiceDef
					.getChoiceType());
			if (choiceDef.isWrap()) {
			...
			for (int i = 0; i < count; i++) {
				...
				item.append(ResourceManager.getResource(option.getLabel()),
						null);
			}
			return item;
		}
		...
		private Item createSpacer() {
			...
			final Spacer item = new Spacer(spacerDef.getMinWidth(), spacerDef
					.getMinHeight());
			item.setLayout(def.getLayout());
			return item;
		}
		...
		private Item createImage() {
			...
			final String label = ResourceManager.getResource(imageDef
					.getLabel())
					+ (imageDef.getLabelExtra() != null ? imageDef
							.getLabelExtra() : "");
			return new ImageItem(label, null, def.getLayout(), ResourceManager
					.getResource(imageDef.getAltText()), imageDef
					.getAppearance());
		}

		private Item createGauge() {
			final GaugeDef gaugeDef = (GaugeDef) def;
			final String label = ResourceManager.getResource(gaugeDef
					.getLabel())
					+ (gaugeDef.getLabelExtra() != null ? gaugeDef
							.getLabelExtra() : "");
			final Gauge item = new Gauge(label, !readOnly, gaugeDef
					.getMaxValue(), 0);
			item.setLayout(def.getLayout());
			return item;
		}
		...
		private Item createDate(final int type) {
			...
			final String label = ResourceManager
					.getResource(dateDef.getLabel())
					+ (dateDef.getLabelExtra() != null ? dateDef
							.getLabelExtra() : "");
			final Item item;
			if (!readOnly) {
		...
		private void bindPercent(final Row row, final Item item) {
			...
			if (raw != null) {
				if (raw instanceof FixedPoint) {
					value = ((FixedPoint) raw).toString(((FixedPointDef) def)
							.getDecimals())
							+ (readOnly ? "%" : "");
				} else {
					value = "";
		...
		private void bindFixedPoint(final Row row, final Item item,
				final boolean money) {
			final Object raw = resolve(row, bind);
		...
		private void bindImage(final Row row, final Item item) {
			...
			if (raw != null) {
				if (raw instanceof byte[]) {
					((ImageItem) item).setImage(Image.createImage((byte[]) raw,
							0, ((byte[]) raw).length));
				} else {
		...
		public Object getValue(final Item item) {
			...
			case ItemDef.MONEY:
				return FixedPoint.parse(((TextField) item).getString().replace(
						',', '.'), '.');
			...
			case ItemDef.INTEGER:
				return new Integer(Integer.parseInt(((TextField) item)
						.getString()));
