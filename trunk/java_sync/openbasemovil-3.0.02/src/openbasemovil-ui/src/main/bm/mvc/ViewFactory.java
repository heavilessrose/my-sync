...
public final class ViewFactory {
	/**
	 * When a view is first created the ViewFactory sends an Event with the view
	 * attached with the key &quot;view&quot; so listeners can add or remove
	 * features from the view.<br/>
	...
	public static void init(final int indexOrder, final String resourceFile) {
		...
		ViewFactory.index = new Index(COMPILED_VIEWS, indexOrder,
				Index.KT_STRING, true, false);
	 ...
	 * @return the view
	 */
	public static SettingsView getSettingsView(final String viewKey,
			final String name) {
	 ...
	 * @return the view
	 */
	public static MenuView getMenu(final String viewKey, final String name) {
		final Hashtable views = ViewFactory.views;
		if (views.containsKey(viewKey)) {
			...
			try {
				...
				if (menuDef.getType() == MenuDef.NATIVE) {
					menu = NativeMenuView.create(getController(menuDef
							.getController()), menuDef, name);
				} else {
					menu = IconMenuView.create(getController(menuDef
							.getController()), menuDef);
				}
	 ...
	 * @return the view
	 */
	public static ListBrowser getListBrowser(final String viewKey,
			final String name) {
		final Hashtable views = ViewFactory.views;
		if (views.containsKey(viewKey)) {
			...
			sendWait();
			ListBrowser browser;
			final boolean nativeView = viewDef.getType() != null ? viewDef
					.getType().byteValue() == ListBrowserDef.NATIVE
					: isListBrowserDefaultNative();
			if (nativeView) {
				browser = new ListBrowserView(getController(viewDef
						.getController()), ResourceManager.getResource(viewDef
						.getTitle()), viewDef.getId(),
						viewDef.getPageSize() != null ? viewDef.getPageSize()
								.intValue() : 10);
			} else {
				final DataListView dlv = new DataListView(getController(viewDef
						.getController()), ResourceManager.getResource(viewDef
						.getTitle()), viewDef.getId());
				...
				if (viewDef.getSelectedColor() != null) {
					dataList.setSelectedColor(viewDef.getSelectedColor()
							.intValue());
				}
				...
				if (viewDef.getSelectedBackground() != null) {
					dataList.setSelectedBackground(viewDef
							.getSelectedBackground().intValue());
				}
			...
			for (int i = 0; i < fieldCount; i++) {
				...
				if (browser instanceof ListBrowserView) {
					...
					browser.addField(field.getName(), field.getLabel(), field
							.getMode() != null ? field.getMode().byteValue()
							: DataList.PLAIN, field.getTrueLabel(), field
							.getFalseLabel(), field.getResource());
				}
			}
			...
			if (browser instanceof ListBrowserView) {
				((ListBrowserView) browser).setWrap(viewDef.getWrap() != null
			...
			if (viewDef.getFontFace() != null) {
				font = Font.getFont(viewDef.getFontFace().intValue(), viewDef
						.getFontStyle() != null ? viewDef.getFontStyle()
						.intValue() : Font.STYLE_PLAIN,
						viewDef.getFontSize() != null ? viewDef.getFontSize()
								.intValue() : Font.SIZE_SMALL);
			} else if (viewDef.getFontStyle() != null) {
				font = Font.getFont(Font.FACE_PROPORTIONAL, viewDef
						.getFontStyle().intValue(),
						viewDef.getFontSize() != null ? viewDef.getFontSize()
								.intValue() : Font.SIZE_SMALL);
			...
			for (int i = 0; i < speedOptionCount; i++) {
				final SpeedOptionDef option = (SpeedOptionDef) speedOptions
						.elementAt(i);
				...
				if (active) {
					if (browser instanceof ListBrowserView) {
						browser.addSpeedOption(optionText, code);
					} else {
						ViewFactory.loadIconStore(option.getIconSet());
						if (option.getIconSet() != null) {
							browser.addSpeedOption(code, option.getIconSet(),
									option.getIcon() != null ? option.getIcon()
											.intValue() : i, option.getHelp());
						} else {
							browser.addSpeedOption(code, optionText, option
									.getHelp());
						}
					}
				}
	 ...
	 *            view definition name
	 * @param browsable
	 *            if a DataForm is browsable
	 * @param readOnly
	 *            if a DataForm is readOnly
	 * @return the view
	 */
	public static DataFormView getDataForm(final String name,
		...
		if (views.containsKey(name)) {
			...
			return getDataForm(name, name, browsable, readOnly,
					getFormDef(name));
		}
	}
	...
	public static DataFormView getBrowsableDataForm(final String name) {
		if (views.containsKey(name)) {
			...
			return getDataForm(name, name, true, def.isReadOnly(), def);
		}
	}
	...
	public static DataFormView getDataForm(final String name) {
		if (views.containsKey(name)) {
			...
			final FormDef def = getFormDef(name);
			return getDataForm(name, name, false, def.isReadOnly(), def);
		}
	}
	 ...
	 * @return the view
	 */
	public static DataFormView getDataForm(final String viewKey,
			final String name, final boolean browsable, final boolean readOnly) {
		final Hashtable views = ViewFactory.views;
		if (views.containsKey(viewKey)) {
			return (DataFormView) views.get(viewKey);
		} else {
			return getDataForm(viewKey, name, browsable, readOnly,
					getFormDef(name));
		}
	}

	private static DataFormView getDataForm(final String viewKey,
			final String name, final boolean browsable, final boolean readOnly,
			final FormDef def) {
		sendWait();
		final DataFormView form = new DataFormView(getController(def
				.getController()), name, browsable, readOnly, def);
		configureView(viewKey, name, form, def);
		System.gc();
		return form;
	}

	private static void configureView(final String viewKey, final String name,
			final View view, final ViewDef def) {
		addActions(view, def);
	...
	private static void addAttachments(final View view, final ViewDef def) {
		...
		for (int i = 0; i < count; i++) {
			final AttachmentDef attachment = (AttachmentDef) attachments
					.elementAt(i);
			...
			if (type != null) {
				...
				if (type.equals("int")) {
					...
					value = new Boolean(rawValue != null
	...
	private static void addActions(final View view, final ViewDef def) {
		...
		for (int i = 0; i < count; i++) {
			...
			if (command == null) {
				command = new Command(ResourceManager.getResource(actionDef
						.getLabel()), forItem ? Command.ITEM : actionDef
						.getType() != null ? actionDef.getType().intValue()
						: Command.SCREEN,
						actionDef.getOrder() != -1 ? actionDef.getOrder()
								: i + 1);
			}
			if (view instanceof DataFormView && forItem) {
				final Item item = ((DataFormView) view).getItem(actionDef
						.getFor());
				final Action action = new Action(actionName, command, code,
						item, actionDef.getDefault() != null
								&& actionDef.getDefault().booleanValue());
				view.addAction(action, active);
			} else {
				view.addAction(new Action(actionName, command, code, null,
		...
		if (id != null) {
			if (!IconStore.hasIconSet(id)) {
				try {
					...
					IconStore.addIconSet(iconSetDef.getSource(), id, iconSetDef
							.getWidth(), iconSetDef.getHeight());
				} catch (Exception e) {
