    ...
    implements EventListener
{
    ...
    private int         color;
    private Font        helpFont    = Font.getFont( Font.FACE_PROPORTIONAL, Font.STYLE_PLAIN, Ui.getFontSize() );
    private Font        listFont    = Font.getFont( Font.FACE_PROPORTIONAL, Font.STYLE_BOLD, Ui.getFontSize() );
    private Font        gridFont    = Font.getFont( Font.FACE_PROPORTIONAL, Font.STYLE_BOLD, Font.SIZE_LARGE );
...
//    private int selectedBorder;
    ...
    private synchronized void calcList()
    {
        ...
        int selectedHeight = lineHeight + ( (helpFont.getHeight() +1) * helpLines );
...
//                );
    ...
    private synchronized void calcGrid()
    {
        ...
        for( int i = 0; i < rows && count < itemCount; i++ )
        {
            ...
            {
                zones[ index - firstVisible ] = new Rectangle(
                        x,
                        y,
                        itemWidth,
                        itemHeight
        ...
        switch( action )
        {
            ...
            case FIRE:
                listener.itemSelected(
                        (MenuItem) items.elementAt( selectedIndex )
                );
                break;
        }
    }
    ...
    public void handleEvent( Event event )
    {
        ...
        {
            helpFont = Font.getFont( Font.FACE_PROPORTIONAL, Font.STYLE_PLAIN, Ui.getFontSize() );
            listFont = Font.getFont( Font.FACE_PROPORTIONAL, Font.STYLE_BOLD, Ui.getFontSize() );
            dirty = true;
            if( isShown() )
            {
                repaint();
