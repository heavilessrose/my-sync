...
public class Dialog extends Canvas {
	...
	private static Font buttonFont = Font.getFont(Font.FACE_PROPORTIONAL,
			Font.STYLE_BOLD, Font.SIZE_SMALL);
	private static Font titleFont = Font.getFont(Font.FACE_PROPORTIONAL,
			Font.STYLE_BOLD, Font.SIZE_LARGE);
	private static Font textFont = Font.getFont(Font.FACE_PROPORTIONAL,
	...
	public static void setSelectedButtonBackground(
			final int selectedButtonBackground) {
	 ...
	 */
	public Dialog(final String title, final String message,
			final int dialogType, final AlertType alertType) {
		init(title, message, dialogType, alertType);
	}

	private void init(final String title, final String message,
			final int dialogType, final AlertType alertType) {
	...
	private int paint(final Graphics g, final String[] text, final int y,
			final Font font, final int color, final boolean center) {
	...
	protected void paint(Graphics graphics) {
		...
		final String[] titleLines = Tools.splitInLines(title, titleFont,
				titleFont, getWidth() - 2, maxLines, true);
		...
		final String[] messageLines = Tools.splitInLines(message, textFont,
				textFont, getWidth() - 2, maxLines, true);
		...
		if (dialogType == OK) {
			...
			g.drawString(ResourceManager.getResource("global.Ok"), 1, buttonY,
					Graphics.LEFT | Graphics.BASELINE);
		} else {
			g.setColor(selected == 0 ? selectedButtonBackground
					: buttonBackground);
			g.fillRect(0, getHeight() - buttonHeight, getWidth() / 2,
					buttonHeight);

			g.setColor(selected == 0 ? selectedButtonColor : buttonColor);
			g.setFont(buttonFont);
			g.drawString(ResourceManager.getResource("global.Yes"), 1, buttonY,
					Graphics.LEFT | Graphics.BASELINE);

			g.setColor(selected == 1 ? selectedButtonBackground
					: buttonBackground);
			g.fillRect(getWidth() / 2, getHeight() - buttonHeight,
					getWidth() / 2, buttonHeight);
			...
			g.drawString(no, buttonX, buttonY, Graphics.LEFT
					| Graphics.BASELINE);
		}
