...
public final class Model
{
                   ...
                   SerializationException
    {
        if( database == null )
        {
            ...
            if( database.getVersionMajor() == 0 )
            {
                ...
                passwords.addColumn(
                        FLD_PASS_CATEGORY_ID,
                        Constants.FT_LONG,
                        0,
                        false,
                        TBL_CATEGORIES
                );
                passwords.addColumn( FLD_PASS_USER_NAME, Constants.FT_STRING, 32 );
    ...
    public static RowSet getRootCategories()
            throws DBException,
                   SerializationException,
                   RecordStoreFullException
    {
        return database.getTable( TBL_CATEGORIES )
                .find( FLD_CAT_PARENT_ID, null );
    }
    ...
    {
        return database.getTable( TBL_CATEGORIES )
                .find( FLD_CAT_PARENT_ID, parentId );
    }
    ...
    public static RowSet findEntriesByCategory( final long id )
            throws DBException,
                   ...
                   RecordStoreFullException
    {
        return database.getTable( TBL_PASSWORDS )
                .find( FLD_PASS_CATEGORY_ID, new Long( id ) );
    }

    public static Row findCategoryById( final Long parentId )
            throws DBException,
                   SerializationException,
                   RecordStoreFullException,
                   RSException
    {
        return lookUpEntity(
                database.getTable( TBL_CATEGORIES ),
                parentId.longValue()
        );
    }
    ...
    public static void removeCategoryTree( final Row category )
            throws DBException,
                   ...
                   RSException
    {
        final RowSet childCategories = getChildCategories(
        ...
        {
            final RowSet childEntries = findEntriesByCategory(
                    category.getId()
            );
                   ...
                   RSException
    {
        ...
        row.setField(
                FLD_PASS_PASSWORD,
                cipher.encrypt( clearPassword.getBytes() )
        );
        row.save();
    }
}
