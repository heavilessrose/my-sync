...
public class Database {
	...
	/**
	 * List of TableInfo objects, in the right order to send updates to the
	 * server.
	 */
	 ...
	 *             if no space left
	 */
	public static Database create(final String name) throws DBException,
	 ...
	 * Takes into account nested calls to open, calling close on the recordstore
	 * when the open count reaches 0.
	 * 
	 * @throws RSException
	 *             on errors
	 ...
	 *             if no space left
	 */
	public void drop() throws DBException, RSException,
			RecordStoreFullException {
	 ...
	 *             on storage errors
	 */
	public void sanityCheck() throws DBException, RecordStoreFullException,
			RSException {
		// noinspection MethodCallInLoopCondition
	 ...
	 *             on errors
	 */
	public void start() throws DBException, RecordStoreFullException,
			SerializationException, RSException {
		if (!started) {
			try {
				open();
				...
				event.setPhase(ResourceManager
						.getResource("db.start.startSystem"));
				ProgressEvent.dispatch(event);

				event
						.setPhase(ResourceManager
								.getResource("db.start.Starting"));
				...
				event.setPhase(ResourceManager
						.getResource("db.start.startSystem"));
				event.setMessage(ResourceManager.getResource("global.Done"));
				ProgressEvent.dispatch(event);
				log.info("Started");
			} finally {
				close();
	 ...
	 * 
	 * @param event
	 *            progress event, when called from the server it should be null
	 * @throws RecordStoreFullException
	 *             if no more space is left on record store
	 * @throws DBException
	 *             on errors
	 * @throws RSException
	 *             on recordstore errors
	 */
	public void softStart(final ProgressEvent event) throws DBException,
			RecordStoreFullException, RSException {
		try {
			final int count = tableInfo.size();
			int current = 0;
			final String eventMessage = event != null ? event.getMessage()
					: null;
			if (event != null) {
			...
			for (Enumeration enu = tableInfo.elements(); enu.hasMoreElements();) {
				...
				if (event != null) {
					current++;
					event
							.setMessage(eventMessage + " " + current + "/"
									+ count);
					event.increment();
					event.dispatch();
				}
	 ...
	 *             on storage errors
	 */
	public void createTable(final Table table) throws DBException,
	 ...
	 *            table information
	 * @throws SerializationException
	 *             on errors serializing table info
	 * @throws RecordStoreFullException
	 *             if no space left
	 * @throws RSException
	 *             on errors adding to persistent storage
	 * @throws DBException
	...
	void startTable(final Table table) throws RecordStoreFullException,
			DBException, RSException {
		final Hashtable[] maps = buildTableFieldMap(table.tableInfo
				.getFieldInfo());
		final Hashtable indexMap = buildTableIndexMap(table.tableInfo
				.getIndexInfo());
		table.parent = this;
	...
	public Table startTable(final TableInfo info)
		...
		final Table table = new Table(this, info, (TableStatus) tableStatus
				.get(info.getName()), maps[0], maps[1], indexMap);
		// Sanity check, only if a full start is beign done
		if (table.getTableStatus() != null) {
			if (table.getTableStatus().isLocked()) {
				table.getTableStatus().setDamaged(true);
	...
	private Hashtable buildTableIndexMap(final IndexInfo[] tableIndexInfo) {
		...
		if (tableIndexInfo != null) {
			...
			for (int i = 0; i < indexCount; i++) {
				...
				final Index index = new Index(idx.getName(), idx.getOrder(),
						idx.getKeyType(), idx.isCaseSensitive());
	 ...
	 *             if no space left
	 */
	private void loadStructure() throws DBException, RSException,
			RecordStoreFullException {
		...
		try {
			...
			try {
				...
				log.error(e);
				ErrorLog.addError("Database", "loadStructure", null, null, e);
			...
			event.setSource(this);
			event.setCurrentValue(0);
			event.setPhase(ResourceManager.getResource("db.start.startSystem"));
			event.setMaxValue(new Integer(2));
			ProgressEvent.dispatch(event);
			...
			event.setCurrentValue(0);
			event.setPhase(ResourceManager.getResource("db.start.loadData"));
			...
			for (int i = 0; i < length; i++) {
				...
				if (recordId > 1) {
					...
					if (recordSize > data.length) {
						try {
							data = Tools.secureAlloc(recordSize
									+ Constants.GROWTH_FACTOR);
						} catch (J2MEException e) {
							log.error(e);
							ErrorLog.addError("Database", "loadStructure",
									null, null, e);
							throw new DBException(
									Constants.ERR_DB_LOAD_STRUCTURE, e);
						}
					}
	...
	private void loadHeader() {
		...
		try {
			...
			header.deserialize(new SerializerInputStream(
	 ...
	 * @noinspection FieldRepeatedlyAccessedInMethod
	 */
	void updateTableStatus(final TableStatus status) throws DBException,
			RecordStoreFullException, RSException {
		try {
			final ByteArrayOutputStream baos = new ByteArrayOutputStream();
			...
			log.error(e);
			ErrorLog.addError("Database", "updateTableStatus",
					new Object[] { status }, null, e);
			throw new DBException(Constants.ERR_DB_UPDATE_STATUS,
					"db.updateTableStatus()", e);
		} finally {
			close();
		}
	}
	 ...
	 * @noinspection FieldRepeatedlyAccessedInMethod
	 */
	void updateTableInfo(final TableInfo info) throws DBException,
			RecordStoreFullException, RSException {
		try {
			final ByteArrayOutputStream baos = new ByteArrayOutputStream();
			final SerializerOutputStream dos = new SerializerOutputStream(baos);
			...
			log.error(e);
			ErrorLog.addError("Database", "updateTableInfo",
					new Object[] { info }, null, e);
			throw new DBException(Constants.ERR_DB_UPDATE_STATUS,
					"db.updateTableInfo()", e);
		} finally {
			close();
		}
	}

	synchronized void saveHeader() throws DBException, RSException,
			RecordStoreFullException {
		open();
		...
		try {
			...
			ErrorLog.addError("Database", "saveHeader", null, null, e);
			log.error(e);
	 ...
	 *             on storage errors
	 */
	public void setVersionMajor(final short major) throws DBException,
			RecordStoreFullException, RSException {
	 ...
	 *             on storage errors
	 */
	public void setVersionMinor(final short minor) throws DBException,
			RecordStoreFullException, RSException {
		header.setVersionMinor(minor);
		saveHeader();
	}

	void resetHeader() throws RSException, DBException,
			RecordStoreFullException {
		header = new DatabaseHeader();
		saveHeader();
	}
	 ...
	 *             on errors
	 */
	public void pack() throws DBException, SerializationException,
			RecordStoreFullException, RSException, InvalidRecordIDException {
		...
		for (int i = 0; i < size && !ControlledTask.isCancelled(); i++) {
			final TableInfo tableInfo = (TableInfo) tableInfoList.elementAt(i);
			pe.setMessage(ResourceManager.getResource("tableNames."
					+ tableInfo.getName()));
			pe.dispatch();
	 ...
	 * Currently does not repair the tables, but the indexes that are marked as
	 * damaged. Tables are not usually damaged, but indexes are more complex and
	 * can be damaged more easily if the phone runs out of battery or the like.
	 * 
	 * @throws DBException
	 *             on database errors
	 * @throws SerializationException
	 ...
	 *             on storage errors
	 */
	public void repair() throws DBException, SerializationException,
			RecordStoreFullException, RSException {
		final ProgressEvent pe = new ProgressEvent(this);
		final int size = tableInfoList.size();
		pe.setAnimate(false);
	...
	/**
	 * Drop a table from the database, not just the files of the table but the
	 * table definition from the database structure.
	 * 
	 * @param table
	 *            table to drop
	 * @throws RSException
	 ...
	 *             if no space left
	 */
	public void dropTable(final Table table) throws RecordStoreFullException,
			RSException {
		table.drop();
