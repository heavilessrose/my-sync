 ...
 * @version $Revision$
 */
public class ScriptingRow extends NativeClass {
	/*
	 * public Date getDate( final String name ) public Date getDate( final int
	 * index ) public byte[] getBlob( final String name ) public byte[] getBlob(
	 * final int index )
	 * 
	 * public byte getStatus() public long getId() public int getVersion()
	 * public Integer getRecordId() public boolean isSent() public Table
	 * getParent() public String getString( final String name ) public String
	 * getString( final int index ) public int getInt( final String name )
	 * public int getInt( final int index ) public long getLong( final String
	 * name ) public long getLong( final int index ) public short getShort(
	 * final String name ) public short getShort( final int index ) public
	 * FixedPoint getFixedPoint( final String name ) public FixedPoint
	 * getFixedPoint( final int index ) public boolean getBoolean( final String
	 * name ) public boolean getBoolean( final int index ) public synchronized
	 * void setField( final String name, final Object value ) public
	 * synchronized void setField( final int index, final Object value ) public
	 * boolean isDirty() public int getFieldCount() public String getFieldName(
	 * final int index ) public boolean equals( final Object o ) public int
	 * hashCode() public String toString() public Row cloneRow() public void
		...
		if (id != -1) {
			...
			if (row != null) {
				try {
					...
					case GET_RECORD_ID:
						return Conversor.toInstance(getVirtualMachine(), row
								.getRecordId());

					case IS_SENT:
						return Conversor.toInstance(getVirtualMachine(), row
								.isSent() ? CoreConstants.TRUE
								: CoreConstants.FALSE);
					...
					case GET_FIELD: {
						...
						if (locator != null) {
							if (locator instanceof String) {
								...
								value = row.getField(((Integer) locator)
										.intValue());
							}
						}
						return Conversor.toInstance(getVirtualMachine(), value);
					}

					case IS_DIRTY:
						return Conversor.toInstance(getVirtualMachine(), row
								.isDirty() ? CoreConstants.TRUE
								: CoreConstants.FALSE);
					...
					case GET_FIELD_NAME:
						return Conversor.toInstance(getVirtualMachine(), row
								.getFieldName(Conversor.toInteger(
					...
					case EQUALS:
						return Conversor
								.toInstance(
										getVirtualMachine(),
										row.equals(args[0].get("value")) ? CoreConstants.TRUE
												: CoreConstants.FALSE);
					...
					case CLONE_ROW: {
						...
						final Instance i = getVirtualMachine().newInstance(
								"Row");
						i.setAttachment("row", row);
						i.setAttachment("parent", target
