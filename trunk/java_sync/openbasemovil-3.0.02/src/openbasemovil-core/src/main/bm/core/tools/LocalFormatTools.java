 ...
 * <code>format.month.<i>MonthNumber</i></code> where <i>MonthNumber</i> ranges
 * from 1 to 12<br/>
 * Day names are taken from the ResourceManager using the key
 * <code>format.weekday.<i>DayNumber</i></code> where <i>DayNumber</i> ranges
 * from 1 to 7 as returned by Calendar.MONTH, beign 1 Sunday, 2 Monday, etc.<br/>
...
public final class LocalFormatTools {
	...
	private static final String[] TOKENS = { "d", "dd", "ddd", "dddd", "M",
			"MM", "MMM", "MMMM", "yy", "yyyy", "h", "hh", "m", "mm", "s", "ss" };
	private static final int[] FORMAT = { Calendar.DAY_OF_MONTH,
			Calendar.DAY_OF_MONTH, Calendar.DAY_OF_WEEK, Calendar.DAY_OF_WEEK,
			Calendar.MONTH, Calendar.MONTH, Calendar.MONTH, Calendar.MONTH,
			Calendar.YEAR, Calendar.YEAR, Calendar.HOUR_OF_DAY,
			Calendar.HOUR_OF_DAY, Calendar.MINUTE, Calendar.MINUTE,
			Calendar.SECOND, Calendar.SECOND };

	/**
	 * Get a fixed local date, adding to the current time the GMT offset for the
	 * default TimeZone.
	 * 
	 * @return local date
	 */
	 ...
	 * seconds suitable for showing an ellapsed time, i.e. not showing hours and
	 * minutes if they're zero.
	 * 
	 * @param milliseconds
	 *            time ellapsed in milliseconds
	 * @param units
	 *            if true use unit letters (h,m,s), if false just separate using
	 *            colons
	 * @return formatted time string, using two digits for minutes and seconds
	 */
	public static String formatEllapsedTime(final long milliseconds,
			final boolean units) {
	...
	/**
	 * Format a date using the given format string, adjusting the time to local
	 * time.
	 * 
	 * @param date
	 *            date
		...
		for (int i = 0; i < count; i++) {
			if (tokens[i].equals(token)) {
				...
				case Calendar.YEAR:
					if (token.length() == 2) {
						element = element % 100;
						return element < 10 ? "0" + element : Integer
								.toString(element);
					} else {
						return value;
					}
				...
				case Calendar.MONTH:
					...
					case 3:
						return ResourceManager.getResource(
								"format.month." + value).substring(0, 3)
								.toLowerCase();

					default:
						return ResourceManager.getResource("format.month."
								+ value);
					}
				...
				case Calendar.DAY_OF_WEEK:
					...
					case 3:
						return ResourceManager.getResource(
								"format.weekday." + value).substring(0, 3)
								.toLowerCase();

					default:
						return ResourceManager.getResource("format.weekday."
								+ value);
					}
