...
public class Store {
	...
	public static boolean useCompression() {
		if (useCompression == null) {
			final long rsSizeLimit = DeviceInfo.getDeviceInfo()
					.getRsSizeLimit();
			useCompression = new Boolean(rsSizeLimit > 0
					&& rsSizeLimit < 1000000);
		}
	...
	/**
	 * Get a store. Two calls with the same name are guaranteed to get the same
	 * object.<br/>
	 * If the store does not really exists, a new store representation is
	 * returned but nothing will be actually created until something is written
	 * to the Store.
	 * 
	 * @param name
	 *            store name
	 * @param multiplexer
	 *            if not yet created, the multiplexer to use, at least 1
	 * @return store
	 */
	 ...
	 *             if no more space available on record store
	 */
	public synchronized int addRecord(final byte[] data) throws RSException,
	 ...
	 *             if no more space available on record store
	 */
	public synchronized int addRecord(final byte[] data, final int offset,
			final int length) throws RSException, RecordStoreFullException {
		final StoreInfo info = this.info;
		final Log log = Store.log;
		// log.debug( "addRecord( " + (data != null) + ", " + offset + ", " +
		// length + " )" );
		...
		try {
			...
			log.error(e);
			ErrorLog.addError(CLASS_NAME, "addRecord", new Object[] { data,
					new Integer(offset), new Integer(length) }, info.name, e);
	...
	private void makeCurrent(final int recordId, final boolean adding)
		...
		if (currentRecord == null || currentRecord.getRecordId() != recordId) {
			// log.debug( "change current from " + (currentRecord != null ?
			// currentRecord.getRecordId() : 0) + " to " + recordId );
			...
			try {
				...
				if (!adding) {
					ErrorLog.addError(CLASS_NAME, "makeCurrent", new Object[] {
							new Integer(recordId), new Boolean(adding) },
							"Invalid record", e);
					throw e;
				} else {
	...
	private void loadRecord(final int recordId, final boolean adding)
		...
		try {
			...
			final SerializerInputStream in = new SerializerInputStream(
					useCompression() ? (InputStream) new ZInputStream(bais)
							: bais);
			if (currentRecord == null) {
				currentRecord = new Record(recordId, info.multiplexer);
			} else {
			...
			if (!adding) {
				ErrorLog.addError(CLASS_NAME, "loadRecord",
						new Object[] { new Integer(recordId) },
						"mapped record id not found (Store: " + info.name
								+ ", mpx: " + info.multiplexer + ")", e);
				throw new InvalidRecordIDException(
						Constants.ERR_STR_LOAD_RECORD, e);
			}
	 ...
	 */
	public synchronized void flush() throws RSException,
			RecordStoreFullException {
		if (currentRecord != null && currentRecord.isDirty()) {
			...
			if (useCompression()) {
				...
				zos = new ZOutputStream(baos, DeviceInfo.getDeviceInfo()
			...
			try {
				...
				if (currentRecord.getRecordId() > 0
						&& currentRecord.getRecordId() < rs.getNextRecordID()) {
					// log.debug( "flush: exsiting record " + currentRecord );
					rs.setRecord(currentRecord.getRecordId(), data, 0,
							data.length);
				} else {
					// log.debug( "flush: new record " + currentRecord );
					currentRecord.setRecordId(rs
				...
				log.error(e);
				ErrorLog.addError(CLASS_NAME, "flush",
						new Object[] { currentRecord }, info.name, e);
	...
	/**
	 * Close the underlying record store.<br/>
	 * Keeps track of nested calls so record store is only closed when no
	 * previous calls to open remain, so it can be safely called from any
	 * method.
	 * 
	 * @throws RSException
	 *             on rs errors
	 */
	public synchronized void close() throws RSException {
		...
		if (openCount > 0) {
			...
			if (openCount == 0) {
				try {
					...
					ErrorLog.addError(CLASS_NAME, "close", null, info.name, e);
					throw new RSException(Constants.ERR_STR_CLOSE, e);
				} finally {
					this.rs = null;
					if (listener != null) {
	 ...
	 * Keeps track of nested calls so record store is only opened one, so it can
	 * be safely called from any method.
	 * 
	 * @throws RSException
	 *             on errors
	 * @throws RecordStoreFullException
	 *             if no space left
	 */
	public synchronized void open() throws RSException,
			RecordStoreFullException {
		open(true);
	}
	 ...
	 *            create underlyin record store if necessary
	 * @throws RSException
	 *             on errors
	 * @throws RecordStoreFullException
	 *             if no space left
	...
	public synchronized void shutdown() throws RecordStoreFullException,
			RSException {
		flush();
		final RecordStore rs = this.rs;
		if (rs != null) {
	 ...
	 * 
	 * @throws RSException
	 *             on record store exceptions
	 * @throws RecordStoreFullException
	 *             if no space left
	 * @noinspection FieldRepeatedlyAccessedInMethod
	 * */
	public synchronized void drop() throws RSException,
			RecordStoreFullException {
	...
	public synchronized void deleteRecord(final int recordId)
			throws RSException, InvalidRecordIDException,
			RecordStoreFullException {
		// final Log log = Store.log;
		try {
			...
			if (!currentRecord.isNull(locator.offset)) {
				...
				if (currentRecord.size == 0
						&& currentRecord.getRecordId() < rs.getNextRecordID() - 1) {
				...
				ErrorLog.addError(CLASS_NAME, "deleteRecord",
						new Object[] { new Integer(recordId) },
						"Invalid record in valid chunk", null);
				throw new InvalidRecordIDException(
						Constants.ERR_STR_DELETE_RECORD, Integer
								.toString(recordId));
			}
		} catch (javax.microedition.rms.InvalidRecordIDException e) {
			ErrorLog.addError(CLASS_NAME, "deleteRecord",
					...
					e);
			throw new InvalidRecordIDException(Constants.ERR_STR_DELETE_RECORD,
					e);
		} catch (RecordStoreException e) {
			ErrorLog.addError(CLASS_NAME, "deleteRecord",
					new Object[] { new Integer(recordId) },
					"RecordStoreException (@rs.deleteRecord): "
							+ locator.toString(), e);
			throw new RSException(Constants.ERR_STR_DELETE_RECORD, e);
		} catch (SerializationException e) {
			ErrorLog.addError(CLASS_NAME, "deleteRecord",
					new Object[] { new Integer(recordId) },
					"SerializationException (@rs.deleteRecord): "
							+ locator.toString(), e);
			throw new RSException(Constants.ERR_STR_DELETE_RECORD, e);
		} finally {
			close();
	...
	public synchronized byte[] getRecord(final int recordId)
		...
		try {
			...
			log.error(e);
			ErrorLog.addError(CLASS_NAME, "getRecord",
					new Object[] { new Integer(recordId) }, info.name, e);
			throw new RSException(Constants.ERR_STR_DELETE_RECORD, e);
		} finally {
			close();
		}
	...
	public synchronized int getRecord(final int recordId, final byte[] buffer,
			final int offset) throws RSException, InvalidRecordIDException {
	...
	public synchronized int getRecordSize(final int recordId)
		...
		try {
			...
			if (currentRecord.isNull(locator.offset)) {
				throw new InvalidRecordIDException(
						Constants.ERR_STR_GET_RECORD_SIZE, Integer
								.toString(recordId));
			} else {
			...
			log.error(e);
			throw new RSException(Constants.ERR_STR_GET_RECORD_SIZE, Integer
					.toString(recordId));
		} finally {
			close();
		}
	...
	public StoreEnumeration enumerateRecords() throws RSException,
			RecordStoreFullException {
		try {
			open();
			return new StoreEnumeration(this, rs.enumerateRecords(null, null,
					false));
		} catch (RecordStoreFullException e) {
			log.error(e);
			throw e;
	...
	public synchronized static void setBurstMode(final boolean burstMode)
			throws RecordStoreFullException, RSException {
	...
	/**
	 * Packs used space by copying records to a new record store, droping
	 * current and copying back to this one. This method is needed to solve bugs
	 * on some phones that miss space on setRecord or do not recover space on
	 ...
	 * @noinspection FieldRepeatedlyAccessedInMethod
	 */
	public synchronized void packCopy() throws RSException,
			RecordStoreFullException {
		...
		try {
			...
			log.error(e);
			ErrorLog.addError(CLASS_NAME, "packCopy", null, info.name, e);
			...
			if (pack != null) {
				...
				try {
					RecordStore.deleteRecordStore(info.name + "_bak");
				} catch (Exception e) {
				}
			}
		}
	 ...
	 * @noinspection FieldRepeatedlyAccessedInMethod
	 */
	public synchronized void setRecord(final int recordId, final byte[] data,
			final int offset, final int length) throws RSException,
			InvalidRecordIDException, RecordStoreFullException {
		try {
			// log.debug( "setRecord( " + recordId + ", " + (data != null) +
			// ", " + offset + ", " + length + " )" );
			open();
			final Store.RecordLocator locator = this.locator;
			locator.translate(recordId);
			...
			if (!currentRecord.isNull(locator.offset)) {
				...
				ErrorLog.addError(CLASS_NAME, "setRecord", new Object[] {
						new Integer(recordId), data },
						"Invalid record in valid chunk", null);
				throw new InvalidRecordIDException(
						Constants.ERR_STR_SET_RECORD, Integer
								.toString(recordId));
			}
		} catch (SerializationException e) {
			log.error(e);
			ErrorLog.addError(CLASS_NAME, "setRecord", new Object[] {
					new Integer(recordId), data, new Integer(offset),
					new Integer(length) }, info.name, e);
			throw new RSException(Constants.ERR_STR_DELETE_RECORD, e);
		} finally {
			close();
		}
	 ...
	 * There's no need to call this method unless you call stopStoreSystem
	 * first.
	 */
	...
	private class RecordLocator {
		...
		public void translate(final int id) {
			...
			if (multiplexer > 1) {
				...
				// log.debug( "translation( " + multiplexer + "): " + recordId +
				// ", " + offset );
			} else {
				recordId = id;
				offset = 0;
		...
		public String toString() {
			return "RecordLocator{" + "multiplexer=" + multiplexer
					+ ", recordId=" + recordId + ", offset=" + offset + '}';
		}
	}

	public interface Listener {
		/**
		 * Called by the Store when the underlying record store is really opened
		 * and listener should perform a set up.
		 */
		void rsOpen();

		/**
		 * Called by the Store when the underlying record store is really closed
		 * and listener should perform clean up.
		 */
		void rsClose();
	}
