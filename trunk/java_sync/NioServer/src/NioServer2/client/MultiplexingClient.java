 ...
 */
package NioServer2.client;

import NioServer2.handlers.*;
import NioServer2.io.SelectorThread;
...
/**
 * A simple test client for the I/O Multiplexing based server. This class
 * simulates several clients using an imaginary request- reply protocol to
 * connect to a server. Several connections can be established at the same time.
 * Each one, generates a random packet to simulate a request, sends it to the
 * server and waits for the answer before sending the next packet.
 * 
 * This implementation is based on I/O Multiplexing, so it should be able to
 * handle several thousand of connections. Using Redhat Linux 9.0 I was able to
 * establish about 10.000 connections before running out of memory (the system
 * had only 256Mb of RAM).
 * 
 * @author Nuno Santos
 */
public class MultiplexingClient implements ConnectorListener,
	...
	/**
	 * Creates a new packet with a size chosen randomly between MIN_SIZE and
	 * MAX_SIZE.
	 */
	...
	public static void main(String[] args) throws Exception {
		InetSocketAddress remotePoint = new InetSocketAddress(/*args[0]*/"127.0.0.1", Integer
				.parseInt(/*args[1]*/"8888"));
		connectionCount = 1;
		if (args.length > 2) {
