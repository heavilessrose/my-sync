...
int ReadRawResp(int fd) {
	...
	if (len > 0) {
		DEBUGLOG(fprintf(LOG, "ReadReadRawResp:\n"));
...
Boolean sendsms(int fd, char *atcmd, char *pdu) {
	...
	for (i=0; i<tries; i++) {
		...
		/*2.0系统与3.0系统返回的字符串不同, 此处改为不去判断返回的字符串, 直接进行下面的操作*/
...
{
	char csms[MAX_SMS_LENGTH] = {0};
	strcpy(csms, asms);
	//Winks_Unicode2UtfString(asms, csms, sizeof(csms));
	
//	NSAutoreleasePool * pool = [[NSAutoreleasePool alloc] init];
//	NSString *str;
//	size_t len = wcslen((const wchar_t*)asms);
//	str = [NSString stringWithCharacters: (const unichar *)asms length: len];
//	strcpy(csms, [str UTF8String]);
	...
	if (aphone == NULL || csms == NULL || code == NULL) {
		DEBUGLOG(printf("check phone/number err\n"));
		return -1;
	}
	...
	if (smscount==0) {
		result = -1;
		DEBUGLOG(printf("CalcSMSSendCount err\n"));
		goto __EXIT__;
	};
	// 群发的号码数
	...
	DEBUGLOG(printf("country code = %s\n", code));
	
	DEBUGLOG(printf("encodingSMSC() success : %s\n", esmsc));
	...
	DEBUGLOG(printf("send to phonenumber: %s\n", aphone));
	DEBUGLOG(printf("send content: %d %d %d %d %d %d\n", asms[0], asms[1], asms[2], asms[3], asms[4], asms[5]));
...
void encodingSMSC(char *smsc, char *encodedsmsc) {
...
void encodingPhone(int index, char *encodedphone) {
	...
	DEBUGLOG(printf("encodedphone: %s\n", encodedphone));
}
