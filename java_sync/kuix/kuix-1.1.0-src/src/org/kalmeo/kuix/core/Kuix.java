...
public final class Kuix {
	 ...
	 * 
	 * @param display
	 *            the {@link Display} instance. Set this value to
	 ...
	 * 
	 * @param okLabelRenderer
	 *            the renderer (xml input stream) used as ok label
	 * @param cancelLabelRenderer
	 *            the renderer (xml input stream) used as cancel label
	 * @param yesLabelRenderer
	 *            the renderer (xml input stream) used as yes label
	 * @param noLabelRenderer
	 *            the renderer (xml input stream) used as no label
	 */
	public static void customizeAlertLabels(
			ByteArrayInputStream okLabelRenderer,
	...
	public static PopupBox showPopupBox(String xmlFilePath,
		...
		return showPopupBox(getXmlResourceInputStream(xmlFilePath),
				dataProvider);
	}
	...
	/**
	 * Create and display a {@link PopupBox}. This method is a full feature of
	 * all {@link PopupBox} helpers like alert, splash.
	 * 
	 * @param styleClass
	 *            The {@link PopupBox} style class
	 * @param duration
	 *            the duration of the {@link PopupBox}
	 * @param content
	 *            the content could be a {@link Widget} or a {@link String}
	 * @param firstLabel
	 *            the label or widget of the first button
	 * @param firstAction
	 *            action of the first button
	 * @param secondLabel
	 *            the label or widget of the second button
	 * @param secondAction
	 *            action of the second button
		...
		if (Kuix.getCanvas() != null) {
			...
			if (secondLabel != null) {
				...
				if (firstLabel instanceof ByteArrayInputStream) {
					...
					secondMenuItem
							.add(new Text().setText((String) secondLabel));
				}
	 ...
	 * 
	 * @param duration
	 *            the duration of the splash (in ms)
	 * @param content
	...
	public static PopupBox splash(int duration, Widget content,
		...
		return showPopupBox(KuixConstants.SPLASH_STYLE_CLASS, duration,
				content, null, null, null, null, onCloseAction);
	}
	 ...
	 * 
	 * <pre>
	 * alert(&quot;Hello world&quot;, KuixConstants.ALERT_OK | KuixConstants.ALERT_INFO, &quot;doOk&quot;,
	 * 		null, null, null);
	 * alert(&quot;Is it rainning ?&quot;, KuixConstants.ALERT_YES | KuixConstants.ALERT_NO
	 * 		| KuixConstants.ALERT_QUESTION, null, null, &quot;doYes&quot;, &quot;doNo&quot;);
	 * </pre>
	 * 
	 * <b>Caution :</b><br>
	 * - <code>KuixConstants.ALERT_OK</code> and
	 * <code>KuixConstants.ALERT_YES</code> are always placed on the first
	 * menuItem and then couldn't be displaye together.<br>
	 * - <code>KuixConstants.ALERT_CANCEL</code> and
	 * <code>KuixConstants.ALERT_NO</code> are always placed on the second
	 * menuItem and then couldn't be displaye together.<br>
	 * 
	 * @param message
	 *            the text message to display
	 * @param options
	 *            the options {@see KuixConstants}
	 * @param okAction
	 *            the ok onAction name
	 * @param yesAction
	 *            the yes onAction name
	 * @param noAction
	 *            the no onAction name
	 * @param cancelAction
	 *            the cancel onAction name
	 * @return The {@link PopupBox} instance
	 */
	public static PopupBox alert(String message, int options,
			String firstAction, String secondAction) {
	 ...
	 */
	public static PopupBox alert(String message, int options) {
		return alert(message, options, null, null);
	}
	 ...
	 *            the message to display
	 * @param throwable
	 *            the {@link Throwable} to get message or class name
	 * @return The {@link PopupBox} instance
	 */
	 ...
	 * <code>rootWidget</code>. Initial <code>rootWidget</code> content is
	 * removed.
	 * 
	 * @param rootWidget
	 * @param inputStream
	 ...
	 * @param dataProvider
	 * @param append
	 *            if <code>false</code> loaded content replace current
	 *            <code>rootWidget</code>'s content.
	 * @param mergeRootWidget
	 *            if <code>true</code> and if loaded content's root widget tag
	 *            equals <code>rootWidget</code>'s tag the given
	 ...
	 * <code>myStyle.css</code>) the default css folder location is automaticaly
	 * added and the path become : <code>/css/myStyle.css</code>. Absolute paths
	 * are kept.
	 * 
	 * @param cssFilePath
	 */
	public static void loadCss(String cssFilePath) {
		if (cssFilePath != null) {
			if (!cssFilePath.startsWith("/")) {
				cssFilePath = new StringBuffer(
						KuixConstants.DEFAULT_CSS_RES_FOLDER).append(
						cssFilePath).toString();
			}
	...
	public static InputStream getXmlResourceInputStream(String xmlFilePath) {
		if (xmlFilePath != null) {
			if (!xmlFilePath.startsWith("/")) {
				xmlFilePath = new StringBuffer(
						KuixConstants.DEFAULT_XML_RES_FOLDER).append(
						xmlFilePath).toString();
			}
			// Use frameHandler.getClass() because of a Object.class bug
			InputStream inputStream = frameHandler.getClass()
		...
		throw new IllegalArgumentException("Unknow xmlFilePath : "
				+ xmlFilePath);
	}
	 ...
	 * 
	 * @param clazz
	 *            The {@link Class} where the <code>getResourceAsStream()</code>
	 *            function is called.
	 * @param path
	 *            Path of the resource file
	 * @return The corresponding {@link ByteArrayInputStream}, or
	 *         <code>null</code> if an error occure.
	 */
	...
	/**
	 * Parse the XML <code>inputStream</code> to build the corresponding widget
	 * tree.
	 * 
	 * @param rootWidget
	 * @param inputStream
	...
	private static Widget parseXml(final Widget rootWidget,
		...
		if (inputStream != null) {
			...
			final Widget[] rootWidgetHolder = (rootWidget == null) ? new Widget[1]
					: null;
			...
			try {
				LightXmlParser.parse(inputStream,
						...
						new LightXmlParserHandler() {
							 ...
							 * 
							 * @see
							 * com.kalmeo.util.xml.DefaultHandler#startElement
							...
							public void startElement(String name,
								...
								if (tag.startsWith("_")) {
									...
									if (path.isEmpty() && mergeRootWidget
											&& rootWidget != null) {
									...
									if (newWidget == null) {
										...
										if (newWidget == null
												&& attributes != null
												&& attributes
														.containsKey(KuixConstants.PACKAGE_ATTRIBUTE)) {

											// Try to construct a custom widget

											// Extract package attribute and construct class name
											String packageName = (String) attributes
													.get(KuixConstants.PACKAGE_ATTRIBUTE);
											...
											try {
												...
												e.printStackTrace();
												throw new IllegalArgumentException(
														"Custom widget creation exceptiond : "
																+ className);
											}
											if (customWidgetInstance instanceof Widget) {
												...
												throw new IllegalArgumentException(
														"Invalid custom widget : "
																+ className);
											}

											// Remove package attribute to continue
											attributes
													.remove(KuixConstants.PACKAGE_ATTRIBUTE);
									...
									if (attributes != null) {
										...
										while (enumeration.hasMoreElements()) {
											...
											attributeValue = convertParsePropertyStringValues((String) attributes
													.get(key));
							...
							public void characters(String characters,
								...
								if (characters.trim().length() > 0
									...
									// Check #inc statment
									if (characters
											.startsWith(KuixConstants.INCLUDE_KEYWORD_PATTERN)) {
										String fileName = null;
										String dataProviderProperty = null;
										boolean mergeRootWidgetParameter = true;
										String rawParams = StringUtil
												.extractRawParams(
										...
										if (rawParams != null) {
											...
											if (st.hasMoreElements()) {
												...
												fileName = convertParsePropertyStringValues(st
														.nextToken().trim());
												if (fileName.length() != 0) {

													// DataProvider ?
													if (st.countTokens() >= 1) {
														dataProviderProperty = st
																.nextToken()
																.trim();
														if (KuixConstants.NULL_KEYWORD
																.equals(dataProviderProperty)) {
													...
													// Merge root widget ?
													if (st.countTokens() >= 1) {
														mergeRootWidgetParameter = BooleanUtil
																.parseBoolean(st
																		.nextToken()
																		.trim());
													}
													...
													if (inputStream != null) {
														try {
															if (usedAttribute != null) {
																...
																byte[] rawData = new byte[inputStream
																		.available()];
																inputStream
																		.read(rawData);
																...
																if (dataProviderProperty != null
																	...
																	if (dataProviderProperty
																			.startsWith(KuixConstants.PARSE_PROPERTY_START_PATTERN)) {
																		String property = dataProviderProperty
																				.substring(
									...
									if (usedWidget
										...
										if (!isCDATA
											...
											if (characters
													.startsWith(KuixConstants.BIND_PROPERTY_START_PATTERN)) {
												property = characters
														.substring(
										...
										if (!isCDATA) {
											...
											characters = convertParsePropertyStringValues(characters
													.trim());
							 ...
							 * 
							 * @see
							 * com.kalmeo.util.xml.DefaultHandler#endElement
							 * (java.lang.String)
							 */
							public void endElement(String name) {
								if (name.startsWith("_")) {
									...
									Widget parentWidget = path.isEmpty() ? (mergeRootWidget ? null
											: rootWidget)
											: (Widget) path.lastElement();
									boolean internal = !internalWidgets
											.isEmpty()
											&& internalWidgets.lastElement() == currentWidget;
									if (internal) {
										internalWidgets.pop();
									}
							 ...
							 * 
							 * @see
							 * com.kalmeo.util.xml.DefaultHandler#endDocument()
							 */
							...
							/**
							 * Convert the parse property variables and replace
							 * them by their string values.
							 * <p>
							 ...
							 * </p>
							 * <p>
							 * <code>${var1}</code> is transform to
							 * <code>Hello</code>
							 * </p>
							 * <p>
							 * <code>${var1|Nothing}</code> is transform to
							 * <code>Hello</code>
							 * </p>
							 * <p>
							 * <code>${var2|Nothing}</code> is transform to
							 * <code>Nothing</code>
							 * </p>
							 * 
							 * @param rawData
							...
							private String convertParsePropertyStringValues(
								...
								int posStart = rawData
										.indexOf(KuixConstants.PARSE_PROPERTY_START_PATTERN);
								if (posStart != -1) {
									...
									if (posEnd != -1) {
										StringBuffer buffer = new StringBuffer(
												rawData.substring(0, posStart));
										String property = rawData.substring(
												posStart + 2, posEnd);
										String propertyValue = null;
										int posPipe = property
										...
										return buffer
												.append(
														convertParsePropertyStringValues(rawData
																.substring(posEnd + 1)))
												.toString();
									}
								}
							 ...
							 * 
							 * @param rawData
							 * @return a list of all extracted bind properties
							 *         or <code>null</code> if not bind property
							 *         is defined in the input String.
							 */
							private String[] extractBindProperties(
										...
										if (posEnd != -1) {
											...
											String propertyDefinition = rawData
													.substring(posStart + 2,
															posEnd);
											int posPipe = propertyDefinition
													.indexOf(KuixConstants.PROPERTY_ALTERNATIVE_SEPRATOR_PATTERN);
											if (posPipe == -1) {
												properties
														.addElement(propertyDefinition);
											} else {
												properties
														.addElement(propertyDefinition
																.substring(0,
								...
								if (properties != null) {
									String[] propertiesArray = new String[properties
											.size()];
	...
	/**
	 * Parse a CSS <code>inputStream</code> and register extracted {@link Style}
	 * to the styles list.
	 * 
	 * @param inputStream
	 * @throws IOException
		...
		if (style != null) {
			...
			if (registredStyle != null) {
				...
				for (StyleProperty property = (StyleProperty) properties
						.getFirst(); property != null; property = (StyleProperty) property
						.getNext()) {
	...
	/**
	 * Returns the list of {@link Style} associated to the <code>widget</code>
	 * or <code>null</code> if no style was found.
	 * 
	 * @param widget
					...
					while (styleSelector != null) {
						...
						while (currentWidget != null && !isCompatible) {
							...
							if (styleSelector.hasTag()) {
								...
								// Tag
								if (!isCompatible
										&& currentWidget.getTag() != null
										&& currentWidget.getTag().equals(
								...
								// Inherited tag
								if (!isCompatible
										&& currentWidget.getInheritedTag() != null
							...
							// Pseudo class
							if (styleSelector.hasPseudoClass()
									&& pseudoClasses != null) {
								for (int i = pseudoClasses.length - 1; i >= 0; --i) {
									for (int j = styleSelector
											.getPseudoClasses().length - 1; j >= 0; --j) {
										if (pseudoClasses[i]
												.equals(styleSelector
														.getPseudoClasses()[j])) {
											isCompatible = true;
	 ...
	 * (obtained from "microedition.locale" system property). The initialization
	 * method is called automatically when a call to
	 * {@link #getMessage(java.lang.String)} method is attempted for the first
	 * time.
	 * 
	 * You can call this method explicitly to see whether there was any problem
	 * with initialization of the i18n support. Method returns a status of the
	 * successfulness.
	 * 
	 ...
	 * 
	 * @param locale
	 *            locale which will be used to determine which message file from
	 *            bundle will be used
	 * @return true if the intialization was succesfull, false if there was any
	 *         problem.
	 */
	 ...
	 * 
	 * @param messageBundle
	 *            full custom messages bundle path
	 * @param locale
	 *            locale which will be used to determine which message file from
	 *            bundle will be used
	...
	public static boolean loadI18nBundle(String messageBundle) {
		...
		if (messageBundle != null && !messageBundle.startsWith("/")) {
			messageBundle = new StringBuffer(
					KuixConstants.DEFAULT_I18N_RES_FOLDER)
					.append(messageBundle).toString();
		}
			...
			if ((locale != null) && (locale.length() > 1)) {
				...
				inputStream = clazz
						.getResourceAsStream(new StringBuffer(prefix).append(
								'.').append(locale).append(suffix).toString());
				if (inputStream == null) {
		...
		if (pattern != null && pattern.startsWith("%") && pattern.endsWith("%")) {
			...
			if ((argStartIndex = pattern.indexOf('(', 1)) != -1) {
				...
				if (argEndIndex > argStartIndex + 1) {
					StringTokenizer args = new StringTokenizer(pattern
							.substring(argStartIndex + 1, argEndIndex), ",");
			...
			return Kuix.getMessage(pattern.substring(1, keyEndIndex),
					argsValues);
		}
		return pattern;
	}
	 ...
	 * 
	 * @param inStream
	 *            stream from which the messages are read
	 * @throws IOException
	 *             if there is any problem with reading the messages
	 */
	 ...
	 * 
	 * @param in
	 *            InputStreamReader used to read the line
	 * @throws IOException
	 *             if there is any problem with reading
