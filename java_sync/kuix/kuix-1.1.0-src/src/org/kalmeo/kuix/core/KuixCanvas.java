...
/**
 * This is the Kuix implementation of the J2ME {@link GameCanvas}. This canvas
 * intercept all user events (keyboard and pointer)
 * 
 * @author bbeaulant
 */
public final class KuixCanvas extends GameCanvas {
	...
	public KuixCanvas(KuixInitializer initializer, boolean fullscreen) {
		...
		if (initializer.getMIDlet() != null) {
			String debugInfosKey = initializer.getMIDlet().getAppProperty(
					KuixConstants.KUIX_DEBUG_INFOS_KEY_APP_PROPERTY);
			if (debugInfosKey != null) {
				byte[] shortcuts = Kuix.getConverter().convertShortcuts(
						debugInfosKey);
				if (shortcuts != null) {
	 ...
	 * 
	 * @param debugInfosKuixKeyCode
	 *            the debugInfosKuixKeyCode to set
	 */
	...
	/**
	 * @param debugInfosEnabled
	 *            the debugInfosEnabled to set
	 */
	...
	protected void initialize() {
		...
		if (initializer.getMIDlet() != null) {
			String locale = initializer.getMIDlet().getAppProperty(
					KuixConstants.KUIX_LOCALE_APP_PROPERTY);
			if (locale != null) {
		...
		String customDisplayBounds = initializer.getMIDlet().getAppProperty(
				KuixConstants.KUIX_DESKTOP_BOUNDS_APP_PROPERTY);
		...
		if (platformName == KuixConstants.PLATFORM_BLACKBERRY) {
			final Command softKeyLeftCommand = new Command(Kuix
					.getMessage(KuixConstants.SOFT_LEFT_I18N_KEY), Command.OK,
					1);
			final Command softKeyRightCommand = new Command(Kuix
					.getMessage(KuixConstants.SOFT_RIGHT_I18N_KEY), Command.OK,
					2);
			...
			setCommandListener(new CommandListener() {
				...
				public void commandAction(Command c, Displayable d) {
					if (c == softKeyLeftCommand) {
						processKeyEvent(KuixConstants.KEY_PRESSED_EVENT_TYPE,
								SOFT_KEY_LEFT_DEFAULT);
						return;
					}
					if (c == softKeyRightCommand) {
						processKeyEvent(KuixConstants.KEY_PRESSED_EVENT_TYPE,
								SOFT_KEY_RIGHT_DEFAULT);
						return;
					}
				}
						...
						if (!keyEvents.isEmpty()) {
							synchronized (this) {
								for (int i = 0; i < keyEvents.size(); ++i) {
									int[] keyEvent = ((int[]) keyEvents
											.elementAt(i));
									FocusManager focusManager = desktop
						...
						if (!pointerEvents.isEmpty()) {
							synchronized (this) {
								for (int i = 0; i < pointerEvents.size(); ++i) {
									int[] pointerEvent = ((int[]) pointerEvents
											.elementAt(i));
									FocusManager focusManager = desktop
											.getCurrentFocusManager();
									if (focusManager != null
	 ...
	 * 
	 * @see
	 * javax.microedition.lcdui.game.GameCanvas#paint(javax.microedition.lcdui
	 * .Graphics)
	 */
	public void paint(Graphics g) {
		...
		if (initialized) {
			...
			// Draw image
			if (image != null) {
				g.drawImage(image, (getWidth() - imageWidth) / 2, (getHeight()
						- imageHeight - textHeight) / 2, 0);
			}
	 ...
	 * 
	 * @param x
	 *            x coordinate of the repaint region
	 * @param y
	 *            y coordinate of the repaint region
	 * @param width
	 *            width of the repaint region
	 * @param height
	 *            height of the repaint region
	 */
	 ...
	 * <code>repaintNextFrame</code> is automaticaly call.
	 */
	...
	private void forceRepaint() {
		...
		if (repaintRegionOnly) {
			imageBufferGraphics.setClip(repaintRegion.x, repaintRegion.y,
					repaintRegion.width, repaintRegion.height);
		}
		...
		if (unpaintableWidgets == null || unpaintableWidgets.isEmpty()) {
			if (repaintRegionOnly) {
				flushGraphics(repaintRegion.x, repaintRegion.y,
						repaintRegion.width, repaintRegion.height);
			} else {
				flushGraphics();
			}
			...
			if (repaintRegionOnly) {
				flushRegion(repaintRegion.x, repaintRegion.y,
						repaintRegion.width, repaintRegion.height);
			} else {
	...
	private void flushRegion(int xMin, int yMin, int xMax, int yMax) {
		...
		if (top != 0) {
			if (left != 0) {
				flushRegion(xMin, yMin, xMin + left, yMin + top);
			}
			flushRegion(xMin + left, yMin, xMax - right, yMin + top);
			if (right != 0) {
				flushRegion(xMax - right, yMin, xMax, yMin + top);
			}
		}

		// Center
		if (left != 0) {
			flushRegion(xMin, yMin + top, xMin + left, yMax - bottom);
		}
		if (right != 0) {
			flushRegion(xMax - right, yMin + top, xMax, yMax - bottom);
		}

		// Bottom
		if (bottom != 0) {
			if (left != 0) {
				flushRegion(xMin, yMax - bottom, xMin + left, yMax);
			}
			flushRegion(xMin + left, yMax - bottom, xMax - right, yMax);
			if (right != 0) {
				flushRegion(xMax - right, yMax - bottom, xMax, yMax);
			}
		}
	 ...
	 */
	private boolean intersect(Widget widget, int xMin, int yMin, int xMax,
			int yMax) {
		return (xMax > widget.getX() && yMax > widget.getY()
				&& widget.getX() + widget.getWidth() > xMin && widget.getY()
				+ widget.getHeight() > yMin);
	}
			...
			while (st.hasMoreTokens()) {
				...
				for (int dy = -1; dy <= 1; ++dy) {
					for (int dx = -1; dx <= 1; ++dx) {
						g.drawString(debugInfo, x + dx, y + dy, Graphics.TOP
								| Graphics.LEFT);
					}
				}
	...
	public String getDebugInfos() {
		...
		buffer.append("Kuix ").append(KuixConstants.VERSION).append(" (")
				.append(Worker.instance.getFrameDuration()).append("ms)")
				.append("\ndisplay : ").append(getWidth()).append("x").append(
						getHeight()).append("\nfps : ").append(fps).append(
						"\nplatform : ").append(getPlatformName()).append(
						"\ntotalMemory : ").append(totalMemory).append(
						"\nfreeMemory : ").append(freeMemory).append('\n')
	...
	protected void pointerDragged(int x, int y) {
		if (pointerDragged || Math.abs(x - pointerPressedX) > 5
				|| Math.abs(y - pointerPressedY) > 5) {
			pointerDragged = true;
	...
	private String getPlatform() {
		...
		try {
			...
			if (currentPlatform.indexOf("Nokia") != -1) {
				...
				return KuixConstants.PLATFORM_SONY_ERICSSON;
					...
					|| currentPlatform.indexOf("j2me") != -1) { // SunMicrosystems = WTK 2.5.x, j2me = J2ME Platforme SDK 3.0
		...
		try {
			if (getKeyName(SOFT_KEY_LEFT_MOTOROLA).toUpperCase().indexOf(
					SOFT_WORD) > -1) {
				return KuixConstants.PLATFORM_MOTOROLA;
			}
		} catch (Throwable e) {
			try {
				if (getKeyName(SOFT_KEY_LEFT_MOTOROLA1).toUpperCase().indexOf(
						SOFT_WORD) > -1) {
					return KuixConstants.PLATFORM_MOTOROLA;
				}
			} catch (Throwable e1) {
				try {
					if (getKeyName(SOFT_KEY_LEFT_MOTOROLA2).toUpperCase()
							.indexOf(SOFT_WORD) > -1) {
						return KuixConstants.PLATFORM_MOTOROLA;
					}
				} catch (Throwable e2) {
	...
	private int getLeftSoftkeyCode() {
		...
		try {
			if (platformName.equals(KuixConstants.PLATFORM_MOTOROLA)) {
				...
				try {
					softkeyLeftMoto = getKeyName(SOFT_KEY_LEFT_MOTOROLA)
							.toUpperCase();
				} catch (Exception e) {
				}
				String softkeyLeftMoto1 = "";
				try {
					softkeyLeftMoto1 = getKeyName(SOFT_KEY_LEFT_MOTOROLA1)
							.toUpperCase();
				} catch (Exception e) {
				}
				String softkeyLeftMoto2 = "";
				try {
					softkeyLeftMoto2 = getKeyName(SOFT_KEY_LEFT_MOTOROLA2)
							.toUpperCase();
				} catch (Exception e) {
				}
				if (softkeyLeftMoto.indexOf(SOFT_WORD) >= 0
						&& softkeyLeftMoto.indexOf("1") >= 0) {
					return SOFT_KEY_LEFT_MOTOROLA;
				} else if (softkeyLeftMoto1.indexOf(SOFT_WORD) >= 0
						&& softkeyLeftMoto1.indexOf("1") >= 0) {
					return SOFT_KEY_LEFT_MOTOROLA1;
				} else if (softkeyLeftMoto2.indexOf(SOFT_WORD) >= 0
						&& softkeyLeftMoto2.indexOf("1") >= 0) {
					return SOFT_KEY_LEFT_MOTOROLA2;
				} else if (softkeyLeftMoto.indexOf(SOFT_WORD) >= 0
						&& softkeyLeftMoto.indexOf("LEFT") >= 0) {
					return SOFT_KEY_LEFT_MOTOROLA;
				} else if (softkeyLeftMoto1.indexOf(SOFT_WORD) >= 0
						&& softkeyLeftMoto1.indexOf("LEFT") >= 0) {
					return SOFT_KEY_LEFT_MOTOROLA1;
				} else if (softkeyLeftMoto2.indexOf(SOFT_WORD) >= 0
						&& softkeyLeftMoto2.indexOf("LEFT") >= 0) {
					return SOFT_KEY_LEFT_MOTOROLA2;
				}
				...
				String leftKeySiemensName = getKeyName(SOFT_KEY_LEFT_SIEMENS)
						.toUpperCase();
				if (leftKeySiemensName.indexOf(SOFT_WORD) >= 0) {
					...
					.equals(KuixConstants.PLATFORM_SONY_ERICSSON)) {
	...
	private int getRightSoftkeyCode() {
		...
		try {
			if (platformName.equals(KuixConstants.PLATFORM_MOTOROLA)) {
				...
				try {
					rightSoftMoto1 = getKeyName(SOFT_KEY_LEFT_MOTOROLA1)
							.toUpperCase();
				} catch (Exception e) {
				}
				String rightSoftMoto = "";
				try {
					rightSoftMoto = getKeyName(SOFT_KEY_RIGHT_MOTOROLA)
							.toUpperCase();
				} catch (Exception e) {
				}
				String rightSoftMoto2 = "";
				try {
					rightSoftMoto2 = getKeyName(SOFT_KEY_RIGHT_MOTOROLA1)
							.toUpperCase();
				} catch (Exception e) {
				}
				if (rightSoftMoto.indexOf(SOFT_WORD) >= 0
						&& rightSoftMoto.indexOf("2") >= 0) {
					return SOFT_KEY_RIGHT_MOTOROLA;
				} else if (rightSoftMoto1.indexOf(SOFT_WORD) >= 0
						&& rightSoftMoto1.indexOf("2") >= 0) {
					return SOFT_KEY_RIGHT_MOTOROLA;
				} else if (rightSoftMoto2.indexOf(SOFT_WORD) >= 0
						&& rightSoftMoto2.indexOf("2") >= 0) {
					return SOFT_KEY_RIGHT_MOTOROLA1;
				} else if (rightSoftMoto.indexOf(SOFT_WORD) >= 0
						&& rightSoftMoto.indexOf("RIGHT") >= 0) {
					return SOFT_KEY_LEFT_MOTOROLA;
				} else if (rightSoftMoto1.indexOf(SOFT_WORD) >= 0
						&& rightSoftMoto1.indexOf("RIGHT") >= 0) {
					return SOFT_KEY_RIGHT_MOTOROLA1;
				} else if (rightSoftMoto2.indexOf(SOFT_WORD) >= 0
						&& rightSoftMoto2.indexOf("RIGHT") >= 0) {
					return SOFT_KEY_RIGHT_MOTOROLA;
				}
				...
				String rightSoftSiemens = getKeyName(SOFT_KEY_RIGHT_SIEMENS)
						.toUpperCase();
	...
	private int getDeleteKeyCode() {
		try {
			if (platformName.equals(KuixConstants.PLATFORM_NOKIA)) {
				...
				if (getKeyName(DELETE_KEY_SAMSUNG).toUpperCase().indexOf(
						"CLEAR") >= 0) {
	 ...
	 * 
	 * @param keyCode
	 *            This code is sent by platform to canvas and redirected here
