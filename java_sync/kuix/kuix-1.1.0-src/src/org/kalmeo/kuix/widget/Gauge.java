...
public class Gauge extends FocusableWidget {
	...
	public void setValue(int value) {
		...
		if (value != this.value) {
			...
			if (increment != 0) {
				value = MathFP.mul(MathFP.ceil(MathFP.div(value, increment)),
						increment);
			}
	...
	protected void doLayout() {
		...
		int barWidth = MathFP.mul(MathFP.toFP(getWidth() - insets.left
				- insets.right), value);
	...
	public boolean processKeyEvent(byte type, int kuixKeyCode) {
		if (isFocusable()
				&& (type == KuixConstants.KEY_PRESSED_EVENT_TYPE || type == KuixConstants.KEY_REPEATED_EVENT_TYPE)) {
			switch (kuixKeyCode) {
	...
	public boolean processPointerEvent(byte type, int x, int y) {
		if (isFocusable()
				&& (type == KuixConstants.POINTER_PRESSED_EVENT_TYPE || type == KuixConstants.POINTER_DRAGGED_EVENT_TYPE)) {
			...
			setValue(MathFP.div(MathFP.toFP(x), MathFP.toFP(getWidth()
