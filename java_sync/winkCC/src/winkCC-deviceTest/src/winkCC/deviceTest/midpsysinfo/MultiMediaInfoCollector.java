 ...
 * @author Mark Schrijver <mark.schrijver@mobillion.nl> (additional system
 *         properties)
 */
public class MultiMediaInfoCollector extends InfoCollector {
	...
	public void collectInfos(MIDPSysInfoMIDlet midlet, Display display) {
		addInfo("supports.video.capture: ", getInfo(System
				.getProperty("supports.video.capture")));
		addInfo("video.encodings: ", getInfo(System
				.getProperty("video.encodings")));
		addInfo("video.snapshot.encodings: ", getInfo(System
				.getProperty("video.snapshot.encodings")));
		addInfo("audio.encoding: ", getInfo(System
				.getProperty("audio.encoding")));
		addInfo("supports.audio.capture: ", getInfo(System
				.getProperty("supports.audio.capture")));
		addInfo("supports.recording: ", getInfo(System
				.getProperty("supports.recording")));
		addInfo("supports.mixing: ", getInfo(System
		...
		try {
			...
			addInfo("microedition.media.version: ", getInfo(System
					.getProperty("microedition.media.version")));
			addInfo("streamable.contents: ", getInfo(System
					.getProperty("streamable.contents")));

			String[] supportedProtocols = ProtocolsInfo
					.getSupportedProtocols(null);

			if (supportedProtocols != null) {
				for (int i = 0; i < supportedProtocols.length; i++) {
					String protocol = supportedProtocols[i];
					String[] supportedContentTypes = ProtocolsInfo
