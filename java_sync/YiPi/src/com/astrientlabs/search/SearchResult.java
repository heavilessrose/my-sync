...
public class SearchResult
{
    ...
    public void writeTo(OutputStream os, ProgressListener listener) throws IOException
    {
        ...
        {      
            ...
            {
                TimeoutConnection timeoutConnection = new TimeoutConnection(c,System.currentTimeMillis()+(1000*90));
                try
                {
                ...
                while ( (r = is.read(buffer)) != -1 && !cancel)
                {
                    total += r;
                    os.write(buffer,0,r);
                    if ( listener != null ) listener.update(this,total);
                }
            }
            else
            {
        ...
        {
            Logger.instance.log("searchResult","File write failure: " + title + ": " + url);
    ...
    public Image getImage(String iURL, int bufferSize, int width, int height) throws IOException
    {
        cancel = false;
        HttpConnection c = null;
        InputStream is = null;
        ...
        try
        {   
            String imageUrl = "http://www.hostj2me.com/imageproxy?key=astrient:1e4e4d:-1985900&f=JPEG&u=" + Strings.URLEncode(iURL) + "&w=" + width + "&h=" + height;
            c = (HttpConnection) Connector.open(imageUrl,Connector.READ);
            
            if ( c.getResponseCode() == HttpConnection.HTTP_OK )
            {
                
                TimeoutConnection timeoutConnection = new TimeoutConnection(c,System.currentTimeMillis()+(1000*45));
                try
                {
                    Network.setTimeout(timeoutConnection);
                    is = c.openInputStream();
            ...
            {
                valid = false;
                throw new IOException("HTTP ERR: " + c.getResponseCode() + " " + c.getResponseMessage());
            }          
        }
        finally
        {
