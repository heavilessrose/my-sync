...
public class MiniPushParser {
	...
	public void parse(InputStream is, MiniPushHandler handler, int bufferSize)
			throws IOException {
		cancel = false;
		...
		while (!cancel && ((i = is.read(buffer)) != -1)) {
			for (int j = 0; j < i; j++) {
				...
				if (mode == MODE_VALUE && b == '<') {
					...
					startHandled = false;
					if (value.length() > 0)
						handler.handleText(tag.toString(), value.toString());
					value.setLength(0);
					tag.setLength(0);
				} else if (b == '<') {
					startHandled = false;
					...
					mode = MODE_TAG;
					handler.handleAttribute(tag.toString(), attribute
