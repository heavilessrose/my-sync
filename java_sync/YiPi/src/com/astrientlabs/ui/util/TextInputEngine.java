...
public class TextInputEngine implements Runnable {
	...
	public TextInputEngine(TextInputListener textInputListener,
			int idleTimeout, int keyTimeout, int cursorFlashRate,
			int maxTextChars) {
		...
		if (platform != null) {
			if (platform.startsWith("NokiaE61")
					|| platform.startsWith("NokiaE62")) {
		...
		if (keyMap == null) {
			...
			keyMap
					.put(String.valueOf(Canvas.KEY_STAR).intern(),
							".,?!\"-()@/:_;+&%*=<>\u00A3\u20AC$\u00A5\u00A4[]{}\\~^`\u00A1\u00BF\u00A7#| ");
		}
	...
	public boolean keyPressed(int keyCode) {
		...
		if ((chars = keyMap.get(String.valueOf(keyCode).intern())) != null) {
			if (mode == MODE_SINGLETAP
					|| (lastKeyCode != keyCode || ((lastEventTimestamp + keyTimeout) < System
							.currentTimeMillis()))) {
				lastKeyCode = keyCode;
				charIndex = 0;
				...
				if (caretPosition == buffer.length()
						&& buffer.length() < maxTextChars) {
					buffer.append(' ');
				}
			}

			if (uppercase) {
				buffer.setCharAt(caretPosition, Character.toUpperCase(chars
						.charAt((charIndex++) % chars.length())));
			} else {
				buffer.setCharAt(caretPosition, chars.charAt((charIndex++)
						% chars.length()));
			}
	...
	public void run() {
		if (!running && hasFocus) {
			try {
				...
				if (mode == MODE_SINGLETAP
						|| currentTs >= (lastEventTimestamp + keyTimeout)) {
					if (!cursorMoved && buffer.length() < maxTextChars) {
						cursorMoved = true;
						caretPosition++;
						if (caretPosition == buffer.length())
