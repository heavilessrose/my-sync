...
{   
    ...
    {
        if ( ! (focus instanceof PopupWidget) ) this.prevFocus = focus;
        this.popup = popup;
        this.showPopup = true;
        this.focus = popup;
        repaint();
    ...
    protected void drawHeader(Graphics g)
    {
        ...
        g.drawImage(Images.TITLE, width>>1, padding, Graphics.HCENTER | Graphics.TOP);           
    }
    
    protected void drawFooter(Graphics g)
    {
        g.setColor(Colors.FOOTER_BGCOLOR);
        g.fillRect(startX,startY+height-footerHeight,getWidth(),footerHeight+1);
        ...
        {
            popup.paint(g,width-popup.widgetWidth-2,height-footerHeight-popup.widgetHeight-1-2,getWidth()-2,getHeight()-2,(focus==popup));
        }
    }
    ...
    {
        Widget w;
        for ( int i = 0, stop = children.size(), y = startY + headerHeight; i < stop && startY < height; i++ )
        {
            w = (Widget)children.elementAt(i);
            w.paint(g,startX,y,getWidth(),height-footerHeight,(focus==w));
            if ( w.xyFloat == Widget.POSITION_RELATIVE ) y += (w.widgetHeight);
        }
    }
    ...
    {
        PopupWidget popup = new PopupWidget(this,(int) (getWidth()*.90), (int) (getHeight()*.60),Colors.POPUP_BG);
    ...
    {
        setError(( Strings.isNull(t.getMessage()) ) ? TextMap.instance.get("error.0") :  TextMap.instance.get("error.0") + "\n\n" + t.getMessage());
    }
    
    public void setError(String text)
    {
        PopupWidget popup = new PopupWidget(this,(int) (getWidth()*.90), (int) (getHeight()*.60),Colors.POPUP_ERROR_BG);
        popup.setText(text);
        popup.modal = false;
        setPopup(popup);
    }
