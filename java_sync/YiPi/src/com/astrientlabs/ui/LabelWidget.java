...
public class LabelWidget extends Widget {
	...
	public void paint(Graphics g, int x, int y, int maxX, int maxY,
		...
		g.drawString(text, x + padding, y + padding, Graphics.LEFT
				| Graphics.TOP);

		if (hasFocus) {
			g.setStrokeStyle(Graphics.DOTTED);
			g.setColor(Colors.HIGHLIGHT_LINE);
			g.drawLine(x + padding, y + padding + Fonts.TEXT.getHeight(), x
					+ padding + Fonts.TEXT.stringWidth(text), y + padding
					+ Fonts.TEXT.getHeight());
