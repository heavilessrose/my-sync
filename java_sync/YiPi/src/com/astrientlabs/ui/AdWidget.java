...
{   
    ...
    public void paint(Graphics g, int x, int y, int maxX, int maxY, boolean hasFocus)
	{
        ...
        if ( ad != null )
        {
            g.drawImage(ad.getImage(),x+(getWidth()>>1),y + (widgetHeight>>1),Graphics.VCENTER | Graphics.HCENTER);
        }
        ...
        g.drawString(message,x+(widgetWidth>>1),y+widgetHeight-padding,Graphics.HCENTER|Graphics.BOTTOM);
    }
    ...
    public void run()
    {
        ...
        try
        {
            running = true;
            //must keep running
            if ( ad == null || (adChangeTs+ad.getDisplayTime() < System.currentTimeMillis()) )
            {
                ad = AdCache.instance.next(this.widgetWidth,this.widgetHeight-Fonts.TEXT.getHeight()-(2*padding));
