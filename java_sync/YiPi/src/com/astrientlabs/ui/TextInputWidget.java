...
{   
    ...
    public TextInputWidget(Window parent, String prompt, String text, TextInputListener textInputListener)
    {
        super(parent);
        ...
        textInputEngine = new TextInputEngine(textInputListener, 1300, 800, 400, 100);
    ...
    public void paint(Graphics g, int x, int y, int maxX, int maxY, boolean hasFocus)
	{
        ...
        //draw icon
        //g.drawImage(icon,x+padding,y + (widgetHeight>>1),Graphics.VCENTER|Graphics.LEFT);
        g.drawString(name,x+padding,y+padding+borderThickness,Graphics.LEFT|Graphics.TOP);    
     
        //draw text
        caretX = x + padding + g.getFont().stringWidth(name) + padding + padding;
        ...
        {
            g.drawString(paintText,caretX,y+padding+borderThickness,Graphics.LEFT|Graphics.TOP);        
        }
        ...
        if ( hasFocus && ((flicker++ & 1) == 0) )
        {
            ...
            if ( caretPos > 0 && ( paintText != null) )
            {
                ...
                {
                    caretX += g.getFont().stringWidth(paintText.substring(0,caretPos));   
                }
            }
            
            g.drawString("_",caretX,y+padding+borderThickness,Graphics.LEFT|Graphics.TOP);   
        }
    }
    ...
    public void keyPressed(int keyCode)
    {
        if ( !textInputEngine.keyPressed(keyCode) )
        {
            ...
            {
                case Canvas.LEFT : textInputEngine.delete(); break;
                case Canvas.RIGHT : textInputEngine.moveCursor(true); break;
                case 505 : textInputEngine.delete(); break;
                default : super.keyPressed(keyCode);
            }      
        }
    }  
