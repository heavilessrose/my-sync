...
public class SearchResultWidget extends Widget {
	...
	public void paint(Graphics g, int x, int y, int maxX, int maxY,
		...
		// draw image
		if (image == null) {
			g.drawImage(Images.PICTURE, x + padding, y + (widgetHeight >> 1),
					Graphics.LEFT | Graphics.VCENTER);
		} else {
			g.drawImage(image, x + padding, y + (widgetHeight >> 1),
					Graphics.LEFT | Graphics.VCENTER);
			g.setColor(0xFFFFFF);
			g.drawRect(x + padding, y + (widgetHeight >> 1)
					- (image.getHeight() >> 1), image.getWidth() - 1, image
					.getHeight() - 1);
		}

		g.setColor(textColor);
		g.drawString(Strings
				.getText(searchResult.getTitle(), maxTextLen, "..."), x
				+ padding + thumbnailWidth + padding + 1, y + padding
				+ borderSize, Graphics.TOP | Graphics.LEFT);
		g.drawString(Strings.getText(searchResult.getSizeString(), maxTextLen,
				"..."), maxX - padding, y + padding + borderSize, Graphics.TOP
				| Graphics.RIGHT);
		g.drawString(Strings.getText(searchResult.getUrl(), maxTextLen, "..."),
				x + padding + thumbnailWidth + padding + 1, y + borderSize
						+ padding + g.getFont().getHeight() + padding,
	...
	public void setImage(Image image) {
		...
		widgetHeight = (2 * padding)
				+ Math.max(image.getHeight(), 2 * Fonts.TEXT.getHeight() + 2
						* padding);
		if (parent.isShown())
