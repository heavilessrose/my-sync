...
public class DownloadImageJob implements Runnable, ProgressListener {
	...
	public String getDownloadsDir() {
		...
		if (location == null) {
			...
			String dir;
			for (Enumeration e = FileSystemRegistry.listRoots(); e
					.hasMoreElements();) {
				dir = "file:///" + e.nextElement() + "yipi/";
				Logger.instance.log("dl", dir);
				try {
					dirConnection = (FileConnection) Connector.open(dir,
							Connector.READ_WRITE);
	...
	public void update(Object who, long position) {
		if (who == searchResult) {
			if ((position - lastPos) > antiFlood) {
				buffer.setLength(0);
				buffer.append(searchResult.getTitle()).append(" ").append(
						Strings.getSizeString(position)).append("/").append(
						Strings.getSizeString(searchResult.getSize()));
