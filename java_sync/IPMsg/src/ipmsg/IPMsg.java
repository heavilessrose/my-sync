...
public class IPMsg implements IPMComListener {
	...
	private long getCryptoCaps() {
		return IPMSG_RSA_512 | IPMSG_RSA_1024 | IPMSG_RC2_40 | IPMSG_RC2_128
				| IPMSG_BLOWFISH_128;
	}

	String makeKey(IPMComEvent ipme) {
		IPMPack tmppack = ipme.getPack();
		String key = ipme.getIPMAddress() + ":" + tmppack.getUser() + ":"
				+ tmppack.getHost();
		return key;
	}
	...
	public void receive(IPMComEvent ipme) {
		...
		case (int) IPMSG_ANSENTRY:
			if (publickey != null)
				IPMSend.send(dsock, makePack(IPMSG_GETPUBKEY, Long.toString(
						getCryptoCaps(), 16).toUpperCase(), false), ipme
						.getIPMAddress());
		case (int) IPMSG_BR_ABSENCE:
			...
			if ((opt & IPMSG_DIALUPOPT) != 0
				...
				dialupmember.put(ipme.getIPMAddress().toString(), ipme
						.getIPMAddress());
		...
		case (int) IPMSG_BR_EXIT:
			...
			ie = new IPMEvent(this, IPMEvent.UPDATELIST_EVENT, new Date(System
					.currentTimeMillis()), ipme);
			invokeListener(ie);
			break;
		case (int) IPMSG_SENDMSG:
			if ((opt & IPMSG_SENDCHECKOPT) != 0)
				IPMSend.send(dsock, makePack(IPMSG_RECVMSG | IPMSG_AUTORETOPT,
						new Long(tmppack.getNo()).toString(), false), ipme
						.getIPMAddress());
			if (new Boolean(pref.getProperty("absenceState")).booleanValue()
				...
				try {
					...
					if (!tmpmsg.equals(""))
						IPMSend.send(dsock, makePack(IPMSG_SENDMSG
								| IPMSG_AUTORETOPT, tmpmsg, false), ipme
								.getIPMAddress());
				} catch (MissingResourceException ex) {
				}
			if (!userlist.containsKey(makeKey(ipme))) {
				nickname = pref.getProperty("nickName");
				IPMSend.send(dsock, makePack(IPMSG_BR_ENTRY | IPMSG_AUTORETOPT
						| getEntryOpt(), nickname, true), ipme.getIPMAddress());
			}
			if ((opt & IPMSG_ENCRYPTOPT) != 0)
				tmppack.setExtra(decryptMessage(tmppack.getExtra()));
			ie = new IPMEvent(this, IPMEvent.RECEIVEMSG_EVENT, new Date(System
					.currentTimeMillis()), ipme);
			invokeListener(ie);
			break;
		...
		case (int) IPMSG_READMSG:
			ie = new IPMEvent(this, IPMEvent.READMSG_EVENT, new Date(System
					.currentTimeMillis()), ipme);
			invokeListener(ie);
			break;
		case (int) IPMSG_DELMSG:
			ie = new IPMEvent(this, IPMEvent.DELETEMSG_EVENT, new Date(System
					.currentTimeMillis()), ipme);
			invokeListener(ie);
			break;
		case (int) IPMSG_GETPUBKEY:
			IPMSend.send(dsock, makePack(IPMSG_ANSPUBKEY, answerPublicKey(ipme
					.getPack()), false), ipme.getIPMAddress());
			break;
		case (int) IPMSG_ANSPUBKEY:
			receivePublicKey(ipme);
			break;
		case (int) IPMSG_GETINFO:
			IPMSend.send(dsock, makePack(IPMSG_SENDINFO, pref
					.getProperty("version")
					+ " ("
					...
					+ " "
					+ System.getProperty("os.version") + ")", false), ipme
					.getIPMAddress());
			break;
		}
	}
	...
	IPMPack makePack(long com, String extra, boolean groupflag) {
		if (groupflag)
			return new IPMPack(1, getSerial(), user, host, com, extra, pref
	...
	IPMAddress[] getBroadcastAddr() {
		...
		try {
			...
			for (int i = 0; i < strbroadcasts.length; i++)
				for (int j = 0; j < ports.length; j++)
					try {
						tmpaddr[i * ports.length + j] = new IPMAddress(
								ports[j], InetAddress
			...
			for (int i = 0; i < ports.length; i++)
				try {
					tmpaddr[i] = new IPMAddress(ports[i], InetAddress
		...
		for (int i = 0; i < dialup; i++)
			tmpaddr[i + tmpaddr.length - dialup] = (IPMAddress) enum
					.nextElement();
		return tmpaddr;
	}

	synchronized void invokeListener(IPMEvent tmpevent) {
		for (Enumeration enum = ipmlistener.elements(); enum.hasMoreElements();) {
	...
	private static String bytesToString(byte[] b) {
		...
		for (int i = 0; i < b.length; i++) {
			...
			strbuf
					.append(Integer.toString((int) (tmpb / 16), 16)
							.toUpperCase());
			strbuf
					.append(Integer.toString((int) (tmpb % 16), 16)
							.toUpperCase());
	...
	byte[] stringToBytes(String src) {
		...
		for (int i = 0; i < src.length(); i += 2) {
			int b = Integer.parseInt(src.substring(i,
	...
	String decryptMessage(String src) {
		...
		try {
			...
			c = Cipher.getInstance(cipher_name);
			IvParameterSpec iv = new IvParameterSpec(new byte[8]);
			SecretKeySpec sks1 = new SecretKeySpec(skey, pref.getProperty(
					"Cipher2").substring(0,
	...
	void receivePublicKey(IPMComEvent ipme) {
		if (publickey == null)
			return;
		try {
			StringTokenizer token = new StringTokenizer(ipme.getPack()
					.getExtra(), ":");
	...
	long setupEncryption() {
		...
		try {
			try {
				KeyFactory kf = KeyFactory.getInstance("RSA");
				RSAPublicKeySpec pubspec = makePublicKeySpec(pref
						.getProperty("publicKey"));
				publickey = kf.generatePublic(pubspec);
				RSAPrivateKeySpec privspec = makePrivateKeySpec(pref
				...
				pref.setProperty("publicKey",
						makeRSAPublicKeyString((RSAPublicKey) publickey));
				pref.setProperty("privateKey",
	...
	void makeBroadcastRecvSockets() throws SocketException {
		...
		proxy = new IPMProxy(this, proxyaddr, new Boolean(pref
				.getProperty("proxyBroadcastAll")).booleanValue());
		for (int i = 0; i < ports.length; i++) {
		...
		public void run() {
			...
			while (true) {
				...
				outer: while (true) {
					...
					IPMComEvent ipmce = new IPMComEvent(this, ipmpe
							.getToIPMAddress().getPort(), ipmpe.getPack(),
	...
	public IPMsg() {
		...
		try {
			try {
				localaddr = InetAddress.getByName(pref
						.getProperty("localAddress"));
			} catch (MissingResourceException ex) {
	...
	public void sendMsg(IPMAddress[] addrs, String msg, long flag) {
		if (addrs == null) {
			IPMPack ipmp = makePack(IPMSG_SENDMSG | IPMSG_BROADCASTOPT | flag,
					msg, false);
			BroadcastSend bs = new BroadcastSend(dsock, ipmp,
					getBroadcastAddr());
		} else if (addrs.length == 1) {
			if (publickeys.containsKey(addrs[0].toString())) {
				try {
					msg = encryptMessage((CryptoInfo) publickeys.get(addrs[0]
							.toString()), msg);
					flag |= IPMSG_ENCRYPTOPT;
				} catch (Exception ex) {
				}
			}
			IPMPack ipmp = makePack(IPMSG_SENDMSG | IPMSG_SENDCHECKOPT | flag,
					msg, false);
			...
			ns.addIPMComListener(new IPMComListener() {
				public void receive(IPMComEvent ipmce) {
					...
					IPMEvent ie = new IPMEvent(this,
							IPMEvent.CANTSENDMSG_EVENT, new Date(System
									.currentTimeMillis()), ipmce);
					invokeListener(ie);
				}
			});
			...
			for (int i = 0; i < addrs.length; i++) {
				...
				if (publickeys.containsKey(addrs[i].toString())) {
					try {
						tmpmsg = encryptMessage((CryptoInfo) publickeys
								.get(addrs[i].toString()), msg);
	...
	public void sendReadMsg(IPMEvent ipme) {
		IPMPack ipmp = makePack(IPMSG_READMSG | IPMSG_AUTORETOPT, new Long(ipme
				.getPack().getNo()).toString(), false);
		IPMSend.send(dsock, ipmp, ipme.getIPMAddress());
	}

	public void sendDeleteMsg(IPMEvent ipme) {
		IPMPack ipmp = makePack(IPMSG_DELMSG | IPMSG_AUTORETOPT, new Long(ipme
				.getPack().getNo()).toString(), false);
		IPMSend.send(dsock, ipmp, ipme.getIPMAddress());
	}
