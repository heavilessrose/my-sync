...
public abstract class AbstractSession implements Session {
	...
	public enum SessionStatus {
		READING, WRITING, IDLE, INIT, CLOSING
	}

	protected volatile SessionStatus sessionStatus;

	public SelectionKey selectionKey;

	protected ByteBuffer in;

	public static final Log log = LogFactory.getLog(DefaultTCPSession.class);
	...
	protected void onRead() {
		this.sessionStatus = SessionStatus.READING;
		readFromBuffer();
	...
	protected synchronized void onWrite() {
		...
		try {
			if (this.sessionStatus.equals(SessionStatus.WRITING)) // 用户可能正在调用flush方法
				return;
			if (this.sessionStatus.equals(SessionStatus.READING) // 不允许读写并行
					&& !handleReadWriteConcurrently)
				return;
			selectionKey.interestOps(selectionKey.interestOps()
					& ~SelectionKey.OP_WRITE);
			this.sessionStatus = SessionStatus.WRITING;
	...
	public void close() {
		if (closed)
			return;
		closed = true;
		this.sessionStatus = SessionStatus.CLOSING;
		try {
	...
	public AbstractSession(SelectableChannel sc, SelectionKey sk,
		...
		this.sessionStatus = SessionStatus.INIT;
		this.handleReadWriteConcurrently = handleReadWriteConcurrently;
	}
	...
	public synchronized void flush() throws IOException, InterruptedException {
		...
		if (this.sessionStatus.equals(SessionStatus.IDLE)
	...
	private boolean isReadingAndAllowWriteConcurrently() {
		return (this.sessionStatus.equals(SessionStatus.READING) && handleReadWriteConcurrently);
	}
}
