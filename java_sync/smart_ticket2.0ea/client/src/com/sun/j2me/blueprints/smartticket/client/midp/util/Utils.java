...
final public class Utils {
	 ...
	 */
	public static ChoiceGroup newPopUp(UIController uiController, String label, int start, int end) {
	...
	public static ChoiceGroup newMonthPopUp(UIController uiController) {
		return newPopUp(uiController, uiController.getString(UIConstants.MONTH),
				UIConstants.MONTH_START, UIConstants.MONTH_END);
	}

	public static ChoiceGroup newYearPopUp(UIController uiController) {
		ChoiceGroup yearList = new ChoiceGroup(uiController.getString(UIConstants.YEAR),
				Choice.POPUP);

		/* temporary set 6 years onward */
		for (int i = UIConstants.CREDIT_CARD_START_YEAR; i < UIConstants.CREDIT_CARD_START_YEAR + 6; i++) {
	...
	public static ChoiceGroup newCardPopUp(UIController uiController) {
		ChoiceGroup cardList = new ChoiceGroup(uiController.getString(UIConstants.CARD_TYPE),
				Choice.POPUP);
	...
	public static void setExpirationDate(UIController uiController, String exprDate,
			ChoiceGroup monthList, ChoiceGroup yearList) {
	...
	public static boolean setChoiceGroupEntry(UIController uiController, int preference,
	...
	public static String showTimeToDate(UIController uiController, int[] showTime) {
		// Day.
	...
	public static String showTimeToHourMinute(UIController uiController, int[] showTime) {
	...
	public static void appendTheaterList(UIController uiController, Choice cg, Theater[] theaters,
		...
		for (int i = 0; i < theaters.length; i++) {
			...
			for (int j = 0; j < localTheaters.length; j++) {
				if (theaters[i].getPrimaryKey().equals(localTheaters[j].getPrimaryKey())) {
					try {
						if (System.currentTimeMillis() > theaters[i].getTheaterSchedule()
								.getExpirationDate()) {
							imageIndex = UIConstants.ICON_IDX_SCHEDULE_EXPIRED;
						} else {
			...
			cg.append(theaters[i].getName() + " [" + theaters[i].getZipCode() + "]", uiController
					.getImage(imageIndex));
		}
	}
