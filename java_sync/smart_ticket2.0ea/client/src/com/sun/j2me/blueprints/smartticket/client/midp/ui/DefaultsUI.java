...
public class DefaultsUI extends Form implements CommandListener {
	...
	public void init(Preferences preferences) {
		...
		Utils.setChoiceGroupEntry(uiController, preferences.getFavoriteDay(), favoriteDayList);
		Utils.setChoiceGroupEntry(uiController, preferences.getFavoriteSeat(), favoriteSeatList);
		Utils.setChoiceGroupEntry(uiController, preferences.getUpdateMovieSchedule(),
				updateMovieList);
		Utils.setChoiceGroupEntry(uiController, preferences.getListMovie(), listMovieList);
	}
	...
	private void createItems() {
		firstZipField = new TextField(uiController.getString(UIConstants.FIRST_ZIP), "", 10,
				TextField.ANY);
		secondZipField = new TextField(uiController.getString(UIConstants.SECOND_ZIP), "", 10,
				TextField.ANY);
		numOfTicketsField = new TextField(uiController
				.getString(UIConstants.DEFAULT_NUM_OF_TICKETS), "", 3, TextField.NUMERIC);
		suggestedOrderMsg = new StringItem(uiController.getString(UIConstants.SUGGESTED_ORDER_MSG),
				null);
		updateMovieList = new ChoiceGroup(
				uiController.getString(UIConstants.UPDATE_MOVIE_SCHEDULE), Choice.POPUP);
		listMovieList = new ChoiceGroup(uiController.getString(UIConstants.LIST_MOVIE),
				Choice.POPUP);
		// initialize those pop up lists
		favoriteSeatList = Utils.newPopUp(uiController, uiController
				.getString(UIConstants.PREFERRED_SEAT), UIConstants.PREF_SEAT_START,
				UIConstants.PREF_SEAT_END);
		favoriteDayList = Utils.newPopUp(uiController, uiController
				.getString(UIConstants.FAVORITE_DAY), UIConstants.PREF_DATE_START,
				UIConstants.PREF_DATE_END);

		updateMovieList.append(uiController.getString(UIConstants.AUTOMATICALLY), null);
		updateMovieList.append(uiController.getString(UIConstants.WHEN_I_SIGN_IN), null);
		updateMovieList.append(uiController.getString(UIConstants.MANUALLY), null);
		listMovieList.append(uiController.getString(UIConstants.SUGGESTED_ORDER), null);
		listMovieList.append(uiController.getString(UIConstants.ALPHAETICALLY), null);
	}
