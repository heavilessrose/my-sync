...
/** 
 * 感知用户和程序之间的所有可能的交互.含有用于每个可能动作的方法.
 * "动作方法"经常启动两个新的线程，一个用于执行后台的动作，另一个用于向用户显示进度条.
 */
public class UIController {
	...
	public static class EventIds {
		...
		public static final byte EVENT_ID_MOVIE_RATINGS_RESOLUTION_REQUESTED = 16;
	...
	private static final String[] iconPaths = { "/icons/g.png", "/icons/pg.png", "/icons/r.png",
			"/icons/q.png", "/icons/fullstar.png", "/icons/halfstar.png", "/icons/remote.png",
	...
	public void init() throws ApplicationException {
		...
		Alert alert = new Alert(null, null, icons[UIConstants.ICON_IDX_SPLASH], null);
	...
	public void showInfoAlert(String title, String message, Displayable d) {
		Alert alert = new Alert((title == null) ? // getString(UIMessageCodes.SMART_TICKET)
		"Smart Ticket"
				: title);
	...
	private void signOn(Runnable authorizedRunnable) {
		...
		if (isLoggedIn) {
			...
			if (preferences.isSilentlyLoggingIn() && userName != null && password != null) {
	 ...
	 * runnable.
	 */
	public void signOn(final Runnable authorizedRunnable, final String userName,
			...
			public void run() {
				try {
					...
					if (authorizedRunnable instanceof EventDispatcher) {
						showErrorAlert(ae, ((EventDispatcher) authorizedRunnable).fallbackUI);
					} else {
	...
	public void accountSetupSaved(AccountInfo accountInfo, Preferences preferences) {
	...
	public void manageTheaterSchedulesRequested() {
		runWithProgress(new EventDispatcher(EventIds.EVENT_ID_MANAGETHEATERSCHEDULESREQUESTED,
				mainMenuUI), getString(UIConstants.PROCESSING), false);
	}
	...
	public void rateMyMoviesSyncRequested() {
		runWithProgress(new EventDispatcher(EventIds.EVENT_ID_MOVIE_RATINGS_SYNC_REQUESTED,
				rateMyMoviesUI), getString(UIConstants.PROCESSING), false);
	}

	public void rateMyMoviesSaveRequested(MovieRating[] movieRatings) {
		selectedMovieRatings = movieRatings;

		runWithProgress(new EventDispatcher(EventIds.EVENT_ID_MOVIE_RATINGS_SAVE_REQUESTED,
				rateMyMoviesUI), getString(UIConstants.PROCESSING), false);
	}

	public void rateMyMoviesDeleteRequested() {
		runWithProgress(new EventDispatcher(EventIds.EVENT_ID_MOVIE_RATING_DELETE_REQUESTED,
				rateMyMoviesUI), getString(UIConstants.PROCESSING), false);
	}
	...
	public void rateMyMoviesResolutionRequested(MovieRating[] movieRatings) {
		...
		runWithProgress(new EventDispatcher(EventIds.EVENT_ID_MOVIE_RATINGS_RESOLUTION_REQUESTED,
				rateMyMoviesUI), getString(UIConstants.PROCESSING), false);
	}
	...
	public void selectSeatsSelected(TheaterSchedule.MovieSchedule movieSchedule, int[] showTime) {
	...
	public void seatsSelected(Seat[] seats) {
		 ...
		 * getString(UIConstants.PROCESSING), false);
		 */
		signOn(new EventDispatcher(EventIds.EVENT_ID_SEATSSELECTED, seatingPlanUI));
	}

	public void signOnConfirmed() {
		confirmTicketUI.init(selectedTheater.getName(), selectedMovie.getTitle(), selectedShowTime,
				selectedSeats);
	...
	class EventDispatcher extends Thread {
		...
		/** 在模型层调用合适的方法(在新线程中执行)完成事件请求.
		 * 当这些方法的最后一次调用返回时，控制器初始化下一个UI画面并显示出来 */
		public void run() {
			try {
				switch (taskId) {
				...
				case EventIds.EVENT_ID_MANAGETHEATERSCHEDULESREQUESTED: {
					...
					if (theaterList == null) {
						showErrorAlert(getString(UIConstants.NO_ZIP_ERROR), mainMenuUI);
					} else {
						manageTheaterSchedulesUI
								.init(theaterList, model.getPersistedTheaters(null));
				...
				case EventIds.EVENT_ID_CHOOSEMOVIEREQUESTED: {
					...
					if (accountInfo == null) {
						...
						Theater[] theaterList = getTheaterList();
						Theater[] persistedTheaters = model.getPersistedTheaters(null);
						Theater[] cachedTheaters = model.getCachedTheaters(null);
						/* merge the persisted and cached theater list */
						Theater[] theaters = new Theater[persistedTheaters.length
								+ cachedTheaters.length];

						System.arraycopy(persistedTheaters, 0, theaters, 0,
								persistedTheaters.length);
						System.arraycopy(cachedTheaters, 0, theaters, persistedTheaters.length,
								cachedTheaters.length);
				...
				case EventIds.EVENT_ID_LOOKUPTHEATERSCHEDULE: {
					...
					TheaterSchedule theaterSchedule = selectedTheater.getTheaterSchedule();
				...
				case EventIds.EVENT_ID_SELECTSEATSSELECTED: {
					SeatingPlan seatingPlan = selectedMovieSchedule
							.getSeatingPlan(selectedShowTime);
					String movieName = selectedMovie.getTitle();

					seatingPlanUI.init(selectedTheater.getName(), movieName, seatingPlan,
							selectedShowTime);
				...
				case EventIds.EVENT_ID_SEATSSELECTED: {
					reservation = model.reserveSeats(selectedTheater.getPrimaryKey(), selectedMovie
							.getPrimaryKey(), selectedShowTime, selectedSeats);
				...
				case EventIds.EVENT_ID_PURCHASEREQUESTED: {
					...
					purchaseCompleteUI.init(reservation.getId(), selectedTheater.getName(),
				...
				case EventIds.EVENT_ID_RATEMYMOVIESREQUESTED: {
					...
					MovieRating[] nonDeletedMovieRatings = new MovieRating[movieRatings.length];
					int j = 0;

					for (int i = 0; i != movieRatings.length; i++) {
						if (movieRatings[i].getModificationStatus() != Synchronizable.STATUS_DELETED) {
				...
				case EventIds.EVENT_ID_MOVIE_RATINGS_SYNC_REQUESTED: {
					progressObserverUI.init(getString(UIConstants.SYNC_MOVIE_RATINGS), false);

					int strategyId = preferences.getConflictResolutionStrategyId();
					...
					if (strategyId != Synchronizable.STRATEGY_ASK_USER) {
						...
						run();
						showInfoAlert(getString(UIConstants.SYNC_SUCCESSFUL), rateMyMoviesUI);
					} else {
						MovieRating[][] movieRatingPairs = model.synchronizeMovieRatings();
	...
	private void createCommands() {
		exitCommand = new Command(getString(UIConstants.EXIT), Command.EXIT, 1);
		mainMenuCommand = new Command(getString(UIConstants.MAIN_MENU), Command.OK, 3);
		mySettingsCommand = new Command(getString(UIConstants.MY_SETTINGS), Command.OK, 3);
	...
	private Theater[] getTheaterList() throws ApplicationException {
		...
		if (zipCode2.length() != 0) {
			...
			Theater[] theaters = new Theater[theaterList.length + theaterList2.length];

			System.arraycopy(theaterList, 0, theaters, 0, theaterList.length);
			System.arraycopy(theaterList2, 0, theaters, theaterList.length, theaterList2.length);
