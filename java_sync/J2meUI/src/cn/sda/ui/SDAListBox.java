 ...
 * @version 1.0
 */
public class SDAListBox extends SDABaseControl {
    //边框
    private int borderStyle = SDAConsts.bsFixedSingle;
    //隐藏选定内容
    private boolean hideSelection = true;
    //滚动边框(多行)
    private int scrollBars = SDAConsts.srVertical;
    ...
    //增加项
    public SDAListItem addItem(String itemString, Image image, boolean isChecked) {
        return internalAddItem(itemString, image, isChecked);
    }

    private SDAListItem internalAddItem(String itemString, Image image, boolean isChecked) {
        SDAListItem listitem = new SDAListItem();
    ...
    private Image internalGetItemImage(int itemIndex) {
        if (itemList.size() > itemIndex) {
            SDAListItem listitem = (SDAListItem) itemList.elementAt(itemIndex);
    ...
    public SDAListItem getItem(String itemValue) {
        return internalGetItem(itemValue);
    }

    public SDAListItem getItem() {
        return (SDAListItem) itemList.elementAt(itemIndex);
    }

    public SDAListItem getItem(int Index) {
        return (SDAListItem) itemList.elementAt(Index);
    }

    private SDAListItem internalGetItem(String itemValue) {
        SDAListItem item = null;
        SDAListItem result = null;
        for (int i = 0; i < itemList.size(); i++) {
            item = (SDAListItem) itemList.elementAt(i);
