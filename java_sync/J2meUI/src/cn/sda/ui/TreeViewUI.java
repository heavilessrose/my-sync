 ...
 * @version 1.0
 */
public class TreeViewUI extends BaseControl {
    //边框
    ...
    private TreeNodeUI CurNode = null;
    //绝对行
    ...
    private int p_len = 0;
    private TreeNodeUI p_node = null;
    //事件
    ...
    protected void setTreeNodesIndex() {
        ...
        for (int i = 0; i < TreeNodes.size(); i++) {
            node = (TreeNodeUI) TreeNodes.elementAt(i);
            node.setIndex(i);
    ...
    private void setTreeNodeIndex(TreeNodeUI node) {
        TreeNodeUI cnode = null;
        ...
        for (int i = 0; i < node.getChildsCount(); i++) {
            cnode = (TreeNodeUI) node.getChild(i);
    ...
    //增加项
    public TreeNodeUI addChild(TreeNodeUI parent, String nodeString) {
        return internalAddChild(parent, nodeString);
    }

    private TreeNodeUI internalAddChild(TreeNodeUI parent, String nodeString) {
        TreeNodeUI listitem = new TreeNodeUI();
    ...
    public void addChild(TreeNodeUI parent, TreeNodeUI node) {
        if (parent == null) {
    ...
    private void internalDeleteChild(int nodeIndex) {
        TreeNodeUI node = getNodeFromItemIndex(nodeIndex);
        if (node.hasParent()) {
    ...
    private void getChildNode(Vector vt, TreeNodeUI node) {
        vt.addElement(node);
        TreeNodeUI cnode = null;
        if (node.hasChild()) {
            for (int i = 0; i < node.getChildsCount(); i++) {
    ...
    private void getChildNum(TreeNodeUI node) {
        p_len++;
        TreeNodeUI cnode = null;
        if (node.hasChild()) {
            for (int i = 0; i < node.getChildsCount(); i++) {
    ...
    //画节点
    private void paintNodes(Graphics g, TreeNodeUI node) {
        getMaxLineInfo();
        //根据传递的SDATreeNode来画
        //int HBarWidth = getHBarWidth();
        ...
        if (node.equals(CurNode)) {
            //画背景
            g.setColor(ConstsUI.clActiveCaption);
    ...
    //获取向下画的线长度
    private void getVLineLength(TreeNodeUI node) {
    ...
    //根据位置获取Node
    private TreeNodeUI getNodeFromItemIndex(int index) {
        TreeNodeUI node = null;
        int clickLine = index;
        p_len = -1;
        p_node = null;
    ...
    private void calClickLine(TreeNodeUI node, int lineID) {
        p_len++;
        if (p_len == lineID) {
            p_node = node;
            return;
    ...
    private void getNodeNum(TreeNodeUI node) {
        p_len++;
    ...
    protected void doPointerPressed(int x, int y) {
        ...
        if (InClientRect(posx, posy, 0, 0, HBarWidth, VBarHeight)) {
            ...
            TreeNodeUI node = getNodeFromItemIndex(itemIndex);
            if (node != null) {
                CurNode = node;
                doSelectChange();
                doSelectNode();
