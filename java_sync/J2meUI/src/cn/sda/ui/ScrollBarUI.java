 ...
 */
public class ScrollBarUI extends BaseControl {
	...
	private void doPointerPress(int x, int y) {
		...
		if (InClientRect(posx, posy, 0, 0, thisWidth, thisHeight)) {
			if (InClientRect(posx, posy, 0, 0,
					kind == ConstsUI.ktHorizontal ? arrowWidth : thisWidth,
					kind == ConstsUI.ktHorizontal ? thisHeight : arrowWidth)) {
				if (value > minValue) {
					value -= increment;
					doScrollChange();
				}
					...
					posx,
					posy,
					kind == ConstsUI.ktHorizontal ? thisWidth - arrowWidth : 0,
					kind == ConstsUI.ktHorizontal ? 0 : thisHeight
							- arrowWidth,
					kind == ConstsUI.ktHorizontal ? arrowWidth : thisWidth,
					kind == ConstsUI.ktHorizontal ? thisHeight : arrowWidth)) {
				if (value < maxValue) {
