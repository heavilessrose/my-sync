 ...
 * @version 1.0
 */
public class BaseEdit extends BaseControl implements InputInterface {
	// 边框
	...
	// 对齐方式
	private int oldAlignType = ConstsUI.alignLeft;
	// 输入法
	private int imeType = InputPanel.imNone;
	// 文字变动
	...
	private void doKeyUp(int keyCode) {
		...
		// 删除（F2）
		if (key.equals(ConstsUI.KEY_SOFT2) || key.equals(ConstsUI.KEY_CLEAR)) {
			if (form.Application.inputPanel.numChars != null) {
				form.Application.inputPanel.numChars = null;
		...
		// ESC
		if (key.equals(ConstsUI.KEY_SOFT1)) {
		...
		if (key.equals(ConstsUI.KEY_SELECT) || key.equals(ConstsUI.KEY_ENTER)) {
			if (form.Application.inputPanel.numChars != null) {
				...
				if (form.Application.inputPanel.ImeType == InputPanel.imLowerCase
						|| form.Application.inputPanel.ImeType == InputPanel.imUpperCase) {
	...
	private void doGetFocus() {
		...
		textAlign = ConstsUI.alignLeft;
		// 输入法
		if (imeType != InputPanel.imNone) {
