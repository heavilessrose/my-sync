...
public class DataSetUI {
	...
	public class Row {
		...
		private void internalSetValueByFieldName(String fieldName,
			...
			for (int i = 0; i < Fields.size(); i++) {
				fd = (Field) Fields.elementAt(i);
				if ((fd.FiledName.equals(fieldName))
						&& (fd.FiledType != DataSetUI.FieldAudoID)) {
		...
		private void internalSetValueByFieldIndex(int fieldIndex,
			...
			if ((fieldIndex > -1) && (fieldIndex < valueList.size())) {
				...
				if (fd.FiledType != DataSetUI.FieldAudoID) {
	...
	private void refreshDataControl() {
		if (enableControls) {
			for (int i = 0; i < dataControlList.size(); i++) {
				BaseControl ctrl = (BaseControl) dataControlList
						.elementAt(i);
				if (ctrl != null) {
					ctrl.paint();
				}
	...
	// 需要刷新的表格
	protected void setWatchControl(BaseControl control, boolean setWatch) {
		if (setWatch) {
