...
public abstract class BaseControl {
	...
	protected BaseControl parent = null;
	...
	// 对齐方式
	protected int dock = ConstsUI.dsNone;
	// event
	...
	private class Align {
		...
		public BaseControl referControl = null;
		public BaseControl[] arrayControl = null;
	...
	public BaseControl() {
		...
		this.originTop = 0;
		this.visible = true;
		this.alignment = ConstsUI.alignCenter;
		this.ctl3d = false;
		this.enabled = true;
		this.tabStop = true;
		this.backColor = ConstsUI.clBtnFace;
		this.foreColor = ConstsUI.clBlack;
	...
	protected void DrawText(Graphics g, int LeftSpace, int RightSpace,
		...
		if (textWidth < clientWidth) {
			if (this.alignment == ConstsUI.alignCenter) {

				dtX += (clientWidth - textWidth) / 2;
			}
			if (this.alignment == ConstsUI.alignRight) {
	...
	public final Vector FindControls(Class AClass) {
		...
		for (int i = ctrlList.size() - 1; i >= 0; i--) {
			BaseControl ctrl = (BaseControl) ctrlList.elementAt(i);
	...
	protected final BaseControl FindControl(int x, int y) {
		if (!this.visible || !this.enabled) {
			return null;
		}
		BaseControl result = this;
		for (int i = ctrlList.size() - 1; i >= 0; i--) {
			BaseControl ctrl = (BaseControl) ctrlList.elementAt(i);
	...
	protected final void PaintChilds() {
		...
		for (int i = 0; i < ctrlList.size(); i++) {
			ctrl = (BaseControl) ctrlList.elementAt(i);
	...
	protected void SetForm(FormUI form) {
		this.form = form;
		if (ctrlList.isEmpty()) {
			return;
		}
		for (int i = 0; i < ctrlList.size(); i++) {
			((BaseControl) (ctrlList.elementAt(i))).SetForm(form);
		}
	}

	public void AddControl(BaseControl control) {
		if (control instanceof MainMenuUI) {
			return;
		}
	...
	public final void RemoveControl(BaseControl control) {
		if (ctrlList.contains(control)) {
	...
	// 查找下一个焦点组件
	protected BaseControl FindNextFocusedCtrl(BaseControl focusedCtrl,
			boolean Reverse) {
		BaseControl result = null;
		if (ctrlList.isEmpty()) {
			return null;
		}
		...
		if (focusedCtrl == null) {
			...
			while (Reverse ? (index >= endIndex) : (index <= endIndex)) {
				BaseControl ctrl = (BaseControl) (ctrlList
						.elementAt(index));
				if (ctrl.CanFocused()) {
					result = ctrl;
					break;
			...
			if (result == null && !(this instanceof FormUI)
					&& this.parent != null) {
	...
	protected void setChildsDock() {
		...
		for (int i = 0; i < ctrlList.size(); i++) {
			...
			if (ctrl.dock == ConstsUI.dsBottom) {
				ctrl.left = 0;
				ctrl.top = dockbottom - ctrl.getHeight();
				ctrl.width = getWidth();
				dockbottom -= ctrl.getHeight();
				if (ctrl instanceof SpliterUI) {
					if (oldctrl != null) {
						((SpliterUI) ctrl).setChangeControl(oldctrl);
					}
				}
				oldctrl = ctrl;
			}
	...
	// 加入布局
	public void setAlignment(BaseControl referControl,
			BaseControl[] arrayControl, int alignType, int paramValue) {
	...
	public void setAlignment(BaseControl referControl,
		...
		internalSetAlignment(referControl,
				new BaseControl[] { arrayControl }, alignType, paramValue);
	}

	private void internalSetAlignment(BaseControl referControl,
			BaseControl[] arrayControl, int alignType, int paramValue) {
	...
	protected void setChildsAlign() {
		...
		BaseControl referControl = null;
		BaseControl[] arrayControl = null;
		BaseControl ctrl = null;
		int alignType = -1;
		int paramValue = 0;
		...
		for (int j = 0; j < AlignmentList.size(); j++) {
			...
			// 相对于参照组件水平居中
			if (alignType == ConstsUI.amhInCenter) {
				refLeft = 0;
				refTop = 0;
			...
			} else if (alignType == ConstsUI.amvInCenter) { // 相对于参照组件垂直居中
				refLeft = 0;
				refTop = 0;
				for (int i = 0; i < arrayControl.length; i++) {
					ctrl = arrayControl[i];
			...
			} else if (alignType == ConstsUI.amParenLeftSpace) {
				refLeft = 0;
				refTop = 0;
				for (int i = 0; i < arrayControl.length; i++) {
					ctrl = arrayControl[i];
			...
			} else if (alignType == ConstsUI.amParentRightSpace) {
				refLeft = 0;
				refTop = 0;
				for (int i = 0; i < arrayControl.length; i++) {
					ctrl = arrayControl[i];
			...
			} else if (alignType == ConstsUI.amParentTopSpace) {
				refLeft = 0;
				refTop = 0;
				for (int i = 0; i < arrayControl.length; i++) {
					ctrl = arrayControl[i];
			...
			} else if (alignType == ConstsUI.amParentBottomSpace) {
				refLeft = 0;
				refTop = 0;
				for (int i = 0; i < arrayControl.length; i++) {
					ctrl = arrayControl[i];
				...
				for (int i = 0; i < arrayControl.length; i++) {
					...
					if (ctrl != referControl) {
						// 左对齐
						if (alignType == ConstsUI.amhleft) {
							if (ctrl.dock == ConstsUI.dsNone) {
								ctrl.setLeft(refLeft);
							}
						}
						// 右对齐
						if (alignType == ConstsUI.amhRight) {
							if (ctrl.dock == ConstsUI.dsNone) {
								ctrl.setLeft(refLeft
						...
						// 平移居中对齐
						if (alignType == ConstsUI.amhCenter) {
							if (ctrl.dock == ConstsUI.dsNone) {
								ctrl.setLeft(refLeft
						...
						// 上对齐
						if (alignType == ConstsUI.amvTop) {
							if (ctrl.dock == ConstsUI.dsNone) {
								ctrl.setTop(refTop);
							}
						}
						// 下对齐
						if (alignType == ConstsUI.amvBottom) {
							if (ctrl.dock == ConstsUI.dsNone) {
								ctrl.setTop(refTop
						...
						// 垂直移动居中对齐
						if (alignType == ConstsUI.amvCenter) {
							if (ctrl.dock == ConstsUI.dsNone) {
								ctrl.setTop(refTop
						...
						// 水平平移等距排列
						if (alignType == ConstsUI.amhSpaceEqually) {
							if (ctrl.dock == ConstsUI.dsNone) {
						...
						// 垂直移动等距排列
						if (alignType == ConstsUI.amvSpaceEqually) {
							if (ctrl.dock == ConstsUI.dsNone) {
						...
						// 等宽
						if (alignType == ConstsUI.amWidthEqually) {
							if (ctrl.dock == ConstsUI.dsNone) {
								ctrl.setWidth(refWidth);
							}
						}
						// 等高
						if (alignType == ConstsUI.amHeightEqually) {
							if (ctrl.dock == ConstsUI.dsNone) {
	...
	public final void setFoucsed() {
		if (CanFocused() && !isFoucsed()) {
			...
			BaseControl ctrl = this.form.focusControl;
