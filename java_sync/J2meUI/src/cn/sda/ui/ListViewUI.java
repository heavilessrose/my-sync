 ...
 * @version 1.0
 */
public class ListViewUI extends BaseControl {
    //边框
    ...
    //选中的项目的颜色
    private int itemFocusedRectColor = ConstsUI.clBtnShadow;
    private int itemFocusedFontColor = ConstsUI.clActiveCaption;
    //列
    ...
    //但前选中项
    private ListViewItemUI curItem = null;
    //最大列，最大行
    ...
    private void paintItems(Graphics g) {
        ...
        ListViewItemUI item = null;
        for (int i = 0; i < itemList.size(); i++) {
            item = (ListViewItemUI) itemList.elementAt(i);
    ...
    //处理项目
    public ListViewItemUI addItem() {
        ListViewItemUI item = new ListViewItemUI();
        itemList.addElement(item);
        repaintControl();
        return item;
    }

    public ListViewItemUI addItem(String itemString) {
        ListViewItemUI item = new ListViewItemUI();
    ...
    public ListViewItemUI addItem(String itemString, Image image, Image selectedImage) {
        ListViewItemUI item = new ListViewItemUI();
        item.setItemString(itemString);
        item.setImage(image);
    ...
    public void addItem(ListViewItemUI item) {
    ...
    public void removeItem(ListViewItemUI item) {
        if (itemList.contains(item)) {
    ...
    public ListViewItemUI getCurItem() {
        return curItem;
    }
    ...
    public void setItemIndex(int itemIndex) {
        if (itemIndex > -1 && itemIndex < itemList.size()) {
            curItem = (ListViewItemUI) itemList.elementAt(itemIndex);
        } else {
            curItem = null;
        }
    }
    ...
    private void getItemByPos(int x, int y) {
        ...
        ListViewItemUI item = getItemByColRow(col, row);
        if (item != null) {
            curItem = item;
        }
    }

    private ListViewItemUI getItemByColRow(int col, int row) {
        ListViewItemUI item = null;
        int index = row * colCount + col;
        if ((index < itemList.size()) && index > -1) {
            item = (ListViewItemUI) itemList.elementAt(index);
        }
        return item;
    }
    //处理事件的执行
