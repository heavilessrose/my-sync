 ...
 * @version 1.0
 */
public class SDAGrid extends SDABaseControl {
    //数据集合
    private SDADataSet DataSet = null;
    private SDADataSet oldDataSet = null;
    //各列宽度
    ...
    //滚动条颜色
    private int FixedColor = SDAConsts.clBtnFace;
    //选中后字体颜色
    private int selectedFontColor = SDAConsts.clWhite;
    //选中行后，行的背景色
    private int selectedBackColor = SDAConsts.clFocusShadow;
    private int lostFocusSelectedBackColor = SDAConsts.clGray;
    //统计行背景色
    private int footRowColor = SDAConsts.clWhite;
    //内部变量
    //左边指针显示的宽度
    ...
    //排序方式
    private int sortType = SDADataSet.sortAsc;
    //单元格单击事件
    ...
    public class Column {
        ...
        //标题文字对齐方式
        private int titleAlignType = SDAConsts.alignLeft;
        //内容文字对齐方式
        private int dataAlignType = SDAConsts.alignLeft;
        //统计内容对齐方式
        private int footAlignType = SDAConsts.alignLeft;
        //统计内容格式
        ...
        if (isVisible()) {
            ...
            if (DataSet != null) {
                ...
                for (int i = StartCol; i < getColumnsCount(); i++) {
                    ...
                    if (ct.titleAlignType == SDAConsts.alignLeft) {
                        drawString(g, ct.Title, V + 1, 1);
                    } else if (ct.titleAlignType == SDAConsts.alignCenter) {
                    ...
                    if (ct.FieldName.equals(sortFieldName)) {
                        g.setColor(borderColor);
                        if (sortType == SDADataSet.sortAsc) {
                            //升序
                    ...
                    if (showFootRow) {
                        ...
                        if (ct.footAlignType == SDAConsts.alignLeft) {
                            drawString(g, ct.footValue, V + 1, thisHeight - barwidth - rowHeight + 1);
                        } else if (ct.footAlignType == SDAConsts.alignCenter) {
                ...
                for (int i = StartRow;
                    ...
                    for (int j = StartCol;
                        ...
                        if (ct.dataAlignType == SDAConsts.alignLeft) {
                            drawString(g, drawstr, V + 2, (i - StartRow + 1) * rowHeight + 1);
                        } else if (ct.dataAlignType == SDAConsts.alignCenter) {
    ...
    //读取数据填充各列表
    public void setDataSet(SDADataSet DataSet) {
        interalSetDataSet(DataSet);
    }

    private void interalSetDataSet(SDADataSet DataSet) {
        //设置观察
        if (DataSet == null) {
            if (oldDataSet != null) {
    ...
    private void doPointerPressed(int x, int y) {
        ...
        if (InClientRect(posx, posy, FixedWidth, 0, thisWidth - barwidth - FixedWidth, rowHeight)) {
            ...
            if (DataSet.getField(cl.FieldName).getFiledType() != SDADataSet.FieldAudoID) {
                if (sortType == SDADataSet.sortAsc) {
                    sortType = SDADataSet.sortDesc;
                } else {
                    sortType = SDADataSet.sortAsc;
                }
