 ...
 */
package NioServer2.handlers;

import NioServer2.io.ProtocolDecoder;
 ...
 * where
 * 
 * - <STX> is ascii 02. - <ETX> is ascii 03. - <asciiData> is any valid
 * character.
 * 
 * @author Nuno Santos
 */
final public class SimpleProtocolDecoder implements ProtocolDecoder {
	...
	/**
	 * Size of the buffer used to reconstrut the packet. Should be big enough to
	 * hold an entire packet.
	 */
	...
	public ByteBuffer decode(ByteBuffer socketBuffer) throws IOException {
		...
		while (socketBuffer.hasRemaining()) {
			...
			try {
				...
				e.printStackTrace();
				throw new IOException("Packet too big. Maximum size allowed: "
						+ BUFFER_SIZE + " bytes.");
			}
			pos++;
