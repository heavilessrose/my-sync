 ...
 * Net class stores user authentication information, so authenticated calls
 * can pick it directly from here instead of requiring more parameters.<br/>
...
public class Net
{
    ...
    public static final int PKT_MIN_FREE_MEMORY = PKT_BIG * 3;  // Minimum memory to set PKT_BIG
    ...
    public static final long PKTIS_WAIT_TIME[]  = new long[] { 5000, 10000, 20000, 30000 };
    ...
    /**
     * This encryption level means that no encryption is performed, this is
     * the default level.
     */
    ...
    /**
     * Encrypt everything using a custom encryption algorithm. This causes
     * some overhead.
     */
     ...
     * Nevertheless, for some small pieces of data compression might not be used,
     * for example for login in. If you set encryption level to SENSITIVE, this
     * kind of information will be encrypted using a custom algorithm. Currently
     * Blowfish is the algorithm used, it's a strong encryption algorithm with
     * a low foot print in memory and a good speed.<br/>
     ...
     * This method first makes a ping to the bmServer ping service, if that's
     * ok it returns NET_STATUS_OK.<br/>
