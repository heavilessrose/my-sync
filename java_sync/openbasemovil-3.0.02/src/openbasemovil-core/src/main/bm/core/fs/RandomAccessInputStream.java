...
public class RandomAccessInputStream extends InputStream {
	 ...
	 * The records store must have been created with a
	 ...
	 *             on errors
	 */
	public RandomAccessInputStream(final RecordStore rs) throws FSException {
		this.rs = rs;
		init();
	}

	private void init() throws FSException {
		readHeader();
		currentRecordId = 2;
		currentOffsset = 0;
		if (size > 0) {
	 ...
	 *             on errors
	 */
	public RandomAccessInputStream(final String name) throws FSException {
		try {
	...
	private void readHeader() throws FSException {
		try {
	 ...
	 * 
	 * @param offset
	 *            offset, if the offset is invalid the position is moved to EOF.
	 * @throws FSException
	 *             on errors
	 */
	public void seek(final long offset) throws FSException {
		try {
