...
/**
 * A Background task, that registers it self on Event so no progress events are
 * thrown under this task.<br/>
 * It also provides control to stop every BackgroundTask, so application is not
 * held up by a background task when the user is trying to exit the application.
 * 
 * @author <a href="mailto:narciso@elondra.com">Narciso Cerezo</a>
 * @version $Revision: 6 $
 */
public abstract class ControlledTask extends Thread {
	/**
	...
	public static synchronized void cancelAll() {
		if (allTasks.size() > 0) {
			...
			pe.setMessage("");
			final String message = ResourceManager
					.getResource("task.closingAll");
			pe.setTitle(message);
			pe.setPhase(message);
			pe.dispatch();
	...
	/**
	 * Check if the current executing thread is a ControlledTask, and if so if
	 * it has been signaled for cancel.
	 * 
	...
	/**
	 * If the maximum time for the thread to cancel is reached, die is called to
	 * try to stop it. The subclass should do whatever is possible to end its
	 * execution, for example, if it's blocked in an InputStream for an external
	 * peripheral, it could try to close the stream so an exception is trhown at
