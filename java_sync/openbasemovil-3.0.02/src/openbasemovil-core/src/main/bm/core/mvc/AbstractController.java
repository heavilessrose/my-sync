...
public abstract class AbstractController implements Controller {
	...
	public void handle(final ControllerEvent event) {
		synchronized (this) {
			final ControllerTask task = new ControllerTask(this, event,
					lastTask);
			lastTask = task;
			task.start();
		}
	...
	/**
	 * Utility method that hides a possibly hanging progress view trying to show
	 * the previous view or the main view if not.
	 */
	public static void hideProgressView() {
		...
		if (currentView != null) {
			if (currentView instanceof EventListener
					&& Event.isRegistered(ProgressEvent.PROGRESS,
	 ...
	 *            command
	 */
	public static void commandAction(final View parent, final Command command) {
	 ...
	 * @return event
	 */
	public static ControllerEvent buildEvent(final View parent,
	 ...
	 */
	public static ControllerEvent buildEvent(final View parent,
			final Command command, final Choice choice) {
