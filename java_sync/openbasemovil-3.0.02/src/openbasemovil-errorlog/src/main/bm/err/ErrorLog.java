...
public class ErrorLog {
	...
	public static String getErrors() throws J2MEException {
		...
		try {
			...
			for (int i = 0; i < count; i++, index++) {
				final ByteArrayInputStream bais = new ByteArrayInputStream(rs
						.getRecord(index));
	...
	public static synchronized void addError(final String className,
			final String method, final Object[] args, final String message,
			final Exception e) {
		RecordStore rs = null;
		try {
			rs = RecordStore.openRecordStore(RS_NAME, true);
			baos.reset();
			sos.writeString(LocalFormatTools.format(new Date(),
					"yyyyMMdd hhmmss")
	...
	private static String buildMessage(final String className,
			final String method, final Object[] args, final String message,
			final Exception e) {
		final StringBuffer msg = new StringBuffer();
		msg.append(className).append('.').append(method).append('(');
		if (args != null) {
		...
		if (e != null) {
			msg.append('\t').append(
					e instanceof J2MEException ? ((J2MEException) e).getTrace()
