...
public class Node implements Serializable {
	...
	/**
	 * Split the node in two.<br/>
	 * This can be done only when the node is full. The middle key will go up to
	 * the parent, the left keys will remain in this node, and the right keys
	 * will go to a new &quot;brother&quote; node at the right of this one. If
	 * the node is the root node the split is always done into two new nodes,
	 * left and right of the root one which will keep just the middle key.
	 * 
	 ...
	 */
	void split() throws DBException, SerializationException,
			...
			if (parent == null) {
				...
				if (!leaf) {
					for (int i = 0; i <= order; i++) {
						...
						final Node rightChild = index.getNode(this,
								children[order + i + 1]);
	...
	void shift(final int index) {
		...
		if (!leaf) {
			System.arraycopy(children, index, children, index + 1, keyCount
					- index + 1);
		}
	...
	void appendNode(final Node neighbour) {
		System.arraycopy(neighbour.keys, 0, keys, order, neighbour
				.getKeyCount());
		if (!neighbour.isLeaf()) {
			System.arraycopy(neighbour.children, 0, children, order, neighbour
					.getKeyCount() + 1);
		}
		keyCount = 2 * order;
		index.addToSave(this);
	 ...
	 *             on errors
	 */
	NodeKey extractNodeKey(final int keyIndex) throws DBException {
		...
		if (!leaf) {
			System.arraycopy(children, keyIndex + 1, children, keyIndex,
					keyCount - keyIndex);
	 ...
	 *             on errors
	 */
	Node getChildNode(final int keyIndex) throws DBException {
		return index.getNode(this, children[keyIndex]);
	}

	int size() throws DBException {
		int size = keyCount;
		if (!leaf) {
			for (int i = 0; i <= keyCount; i++) {
	...
	public String toString() {
		...
		final int keyCount = this.keyCount;
		final StringBuffer buffer = new StringBuffer("Node{").append(
				"keyCount=").append(keyCount).append(",leaf=").append(leaf)
				.append(",recordId=").append(recordId);
		buffer.append(",keys={");
		for (int i = 0; i < keyCount; i++) {
			buffer.append("key[").append(i).append("]=").append(
					keys[i].getKey()).append(", ").append(keys[i].getData());
			if (i < keyCount - 1) {
				buffer.append(",");
			}
		}
		...
		if (!leaf) {
			...
			for (int i = 0; i <= keyCount; i++) {
				buffer.append("child[").append(i).append("]=").append(
						children[i]);
				if (i < keyCount) {
					buffer.append(",");
				}
