...
public class IndexInfo implements Serializable {
	 ...
	 * 
	 * @param order
	 *            index order, if below MIN_ORDER then MIN_ORDER is used
	 * @param name
	 *            index name
	 * @param fieldName
	 *            field over wich the index will be constructed
	 * @param keyType
	 *            key type, one of KT_LONG, KT_STRING or KT_FULL_TEXT
	 * @param caseSensitive
	 *            if the keys are considered case sensitive
	 */
	public IndexInfo(final int order, final String name,
			final String fieldName, final byte keyType,
	 ...
	 *            index name
	 * @param fieldNames
	 *            fields over wich the index will be constructed
	 * @param keyType
	 *            key type, one of KT_LONG, KT_STRING or KT_FULL_TEXT
	 * @param caseSensitive
	 *            if the keys are considered case sensitive
	 */
	public IndexInfo(final int order, final String name,
			final String[] fieldNames, final byte keyType,
			final boolean caseSensitive) {
		this.order = order < MIN_ORDER ? MIN_ORDER : order;
		this.name = name.toLowerCase();
	...
	/*
	 * ( IndexInfo: {byte: version} {int: order} {String: name} {byte:
	 * fieldCount} {String: fieldName}+ {byte: indexType} [ if( indexType ==
	 * KT_STRING ) {boolean: caseSensitive} ] )
	 */
	...
	public String toString() {
		...
		toString.append("IndexInfo{" + "order=").append(order).append(
				", name='").append(name).append("'" + ", fieldNames='");
		if (fieldNames != null) {
			toString.append("[");
			final int length = fieldNames.length;
			for (int i = 0; i < length; i++) {
		...
		toString.append("'" + ", keyType=").append(keyType).append(
				", caseSensitive='").append(caseSensitive).append(
				"', fieldExpression='").append(fieldExpression).append("'}");
	...
	public boolean equals(final Object o) {
		...
		if (name != null ? !name.equals(indexInfo.name)
				: indexInfo.name != null) {
			return false;
		}
