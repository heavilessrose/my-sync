 ...
 * @version $Revision$
 */
public class ScriptingTable extends NativeClass {
	/*
	 * Pendientes de implementar
	 * 
	 * 
	 * public int[] findRaw( final String fieldName, final Object value ) public
	 * int[] findRawFuzzy( final String fieldName, final String value ) public
	 * TableStatus getTableStatus() public void rebuildModifiedIndex() public
	 * TableInfo getTableInfo() public void unmarkSentRecords() public long
	 * getSize() public long getTotalSize() public long getCacheHits() public
	 * long getCacheMisses() public long getAverageFindTime() public RowSet
	 * getModifiedRecords() public FieldInfo getFieldInfo( final int index )
	 * public FieldInfo getFieldInfo( final String name )
	 * 
	 * public synchronized void save( final Row row ) public void remove( final
	 * Row row )
	 */
		...
		if (id != -1) {
			...
			if (table != null) {
				try {
					switch (id) {
					case GET_NAME:
						return Conversor.toInstance(getVirtualMachine(), table
								.getName());
					...
					case FETCH:
						return sendRow(table.fetch(Conversor.toInteger(
								args[0].get("value")).intValue()), target);
					...
					case CREATE_INDEX:
						table.createIndex(Conversor.toString(args[0]),
								Conversor.toInteger(args[1]).intValue(),
								Conversor.mapToStringArray(args[2]), Conversor
										.toInteger(args[3]).byteValue(),
								Conversor.toBoolean(args[4]));
						break;

					case FIND:
						return sendRowSet(table.find(Conversor
								.toString(args[0]), args[1].get("value")),
								target);
					...
					case FIND_FUZZY:
						return sendRowSet(table
								.findFuzzy(Conversor.toString(args[0]),
										Conversor.toString(args[1])), target);
					...
					case HAS_FIELD:
						return Conversor
								.toInstance(
										getVirtualMachine(),
										table.hasField(Conversor
												.toString(args[0])) ? CoreConstants.TRUE
												: CoreConstants.FALSE);
					}
				} catch (Exception e) {
