 ...
 * to handle custom commands and control different aspects of the browsing,
 * such as stablishing the page size.
 *
 * @author <a href="mailto:narciso@elondra.org">Narciso Cerezo</a>
 * @version $Revision: 1.10 $
 */
public class ListBrowserView
    extends AbstractView
    implements ListBrowser
{
    /**
     * Default page size.
     */
    ...
    /**
     * Vector of field names in the order they should be used to build the labels.
     */
     ...
     * @param help    help text
     */
    public void addSpeedOption(
            int code, String iconSet, int index, String help
    )
    {
        speedOptionsText.addElement( help );
        speedOptionsAction.addElement( new Integer( code ) );
    ...
    {
        ((List) displayable).setFitPolicy(
                wrap ? Choice.TEXT_WRAP_ON : Choice.TEXT_WRAP_OFF
        );
    }
    ...
    /**
     * Find a row by it's id, move the set to the appropriate page and make
     * that row selected.
     *
     * @param id row id
     * @throws DBException on errors
     ...
     * If the field name ends with a dollar sign, it means that the field is
     * a FixedPoint and should be rendered using toMoneyString instead of
     * toString.<br/>
     ...
     * If the field name ends with a pound sign, it means that the field is
     * a Date and should be rendered as a Time only.
    ...
    protected int getSelectedIndex()
    {
        ...
        if( list != null )
        {
            ...
            if( listSelectedIndex >= firstValid && listSelectedIndex <= lastValid )
            {
     ...
     * and if the subclass does not process the command, it processes it
     * to handle page and selection events.
     *
     ...
     * @throws bm.storage.RSException on errors
     */
    protected void onBeforeFillPage()
            throws DBException,
                   RecordStoreFullException,
                   RSException
    {
     ...
     * @throws bm.storage.RSException on errors
     */
    protected void onAfterFillPage()
            throws DBException,
                   RecordStoreFullException,
                   RSException
    {
