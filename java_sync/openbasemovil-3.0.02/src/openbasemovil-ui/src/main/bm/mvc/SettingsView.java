 ...
 */
public class SettingsView extends NativeMenuView {
	Hashtable options;
	String name;

	public static SettingsView create(final SettingsViewDef viewDef,
			final String name) {
		final SettingsView settingsView = new SettingsView(ResourceManager
				.getResource(viewDef.getTitle()), viewDef.getId());
	...
	private void build(final SettingsViewDef viewDef) {
		...
		if (viewDef.getImageMap() != null) {
			loadImages(name, viewDef.getImageMap(), viewDef.getImageMapBig(),
					count);
		}
		...
		for (int i = 0; i < count; i++) {
			...
			if (active) {
				...
				final String text = ResourceManager.getResource(option.title
						+ "." + getSetting(option.key, option.defaultValue));
				int index;
				if (viewDef.getImageMap() != null) {
					index = addOption(text, (Image) images.get(name + "." + i),
							i);
				} else {
	...
	private class SettingsController extends AbstractController {
		protected void process(ControllerEvent event) {
			try {
				...
				default:
					final Option option = (Option) options.get(new Integer(
							event.getIndex()));
					if (option != null) {
						final String value = new Boolean(!getSetting(
								option.key, option.defaultValue)).toString();
						Settings.setProperty(option.key, value);
						final List list = (List) displayable;
						final String text = ResourceManager
								.getResource(option.title + "." + value);
						list.set(event.getIndex(), text, (Image) images
								.get(name + "." + option.index));
					}
					show();
					break;
				}
