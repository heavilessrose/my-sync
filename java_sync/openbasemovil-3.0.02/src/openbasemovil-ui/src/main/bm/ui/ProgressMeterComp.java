...
public class ProgressMeterComp {
	...
	private static void createSlices() {
		...
		final InputStream source = comp.getClass().getResourceAsStream(
				"/images/progressBar.png");
		if (source == null) {
			ErrorHandler.handleError(comp, "/images/progressBar.png not found",
					new Exception());
		} else {
			try {
				final Image src = Image.createImage(source);
				final int height = src.getHeight();
				startSlice = Image.createImage(src, 0, 0, 1, height,
						Sprite.TRANS_NONE);
				emptySlice = Image.createImage(src, 1, 0, 1, height,
						Sprite.TRANS_NONE);
				filledSlice = Image.createImage(src, 2, 0, 1, height,
						Sprite.TRANS_NONE);
				endSlice = Image.createImage(src, 3, 0, 1, height,
						Sprite.TRANS_NONE);
			} catch (IOException e) {
	...
	public void paint(Graphics graphics, final int x0, final int y0, int w,
			int h) {
	...
	private void drawBar(final Graphics g, final int width, final int y) {
		...
		if (maxValue != Gauge.INDEFINITE) {
			final int filledWidth = maxValue != 0 ? (value * barWidth)
					/ maxValue : 0;
