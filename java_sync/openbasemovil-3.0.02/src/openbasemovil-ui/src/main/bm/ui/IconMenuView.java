 ...
 * @version $Revision$
 */
public class IconMenuView extends AbstractView implements MenuView,
		Menu.Listener {
	protected Hashtable map = new Hashtable(10);
	protected int viewId;
	protected String helpId;

	public static IconMenuView create(final Controller controller,
		...
		if (menuDef.getSelectedBackground() != null) {
			menu.setSelectedBackground(menuDef.getSelectedBackground()
					.intValue());
		}
		...
		if (menuDef.getFontFace() != null || menuDef.getFontStyle() != null
				|| menuDef.getFontSize() != null) {
			final Font font = Font.getFont(
					menuDef.getFontFace() != null ? menuDef.getFontFace()
							.intValue() : Font.FACE_PROPORTIONAL, menuDef
							.getFontStyle() != null ? menuDef.getFontStyle()
							.intValue() : Font.STYLE_PLAIN, menuDef
							.getFontSize() != null ? menuDef.getFontSize()
		...
		for (int i = 0; i < count; i++) {
			...
			if (active) {
				menu.addItem(itemDef.getTitle(),
						itemDef.getIconIndex() != null ? itemDef.getIconIndex()
								.intValue() : icon, itemDef
								.getSelectedIconIndex() != null ? itemDef
								.getSelectedIconIndex().intValue()
								: selectedIcon, itemDef.getAction(), itemDef
								.getHelp(), itemDef.getView());
			}
		}
	...
	protected IconMenuView(final Controller controller, final String title,
			final int viewId) {
		super(controller);
		this.viewId = viewId;
	...
	public void addItem(final String label, final int iconIndex,
		...
		((Menu) displayable).addItem(label, iconIndex, selectedIconIndex,
				action, view);
	}
