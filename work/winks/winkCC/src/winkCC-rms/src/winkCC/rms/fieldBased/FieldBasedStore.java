...
public class FieldBasedStore {
	...
	public FieldBasedStore(RecordStore rs) throws IOException,
	...
	public FieldBasedStore(RecordStore rs, int fieldListID) throws IOException,
			RecordStoreException {
	...
	// record ID.
	public synchronized int addRecord(Object[] fields) throws IOException,
			RecordStoreException {
		writeStream(fields);
	...
	private static FieldList loadFieldList(RecordStore rs, int fieldListID)
	...
	// field list.
	public synchronized Object[] readRecord(int recordID) throws IOException,
			RecordStoreException {
	...
	private void writeField(int type, Object value) throws IOException {
		switch (type) {
		case FieldList.TYPE_BOOLEAN:
	...
	// the field list.
	public synchronized void writeRecord(int recordID, Object[] fields)
			throws IOException, RecordStoreException {
		writeStream(fields);
		byte[] data = _bout.getByteArray();
	...
	private void writeStream(Object[] fields) throws IOException {
		int count = _fieldList.getFieldCount();
		...
		for (int i = 0; i < count; ++i) {
			writeField(_fieldList.getFieldType(i), (i < len ? fields[i] : null));
		}
	}
}
