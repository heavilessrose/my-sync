...
#if !defined(__linux__) & !defined(__unix__) & !defined(__sgi)
  #define WIN_DOS
#endif

// Dos or Windows platform
#if defined(WIN_DOS)
...
/* make sure the structure is packed on byte boundary */
  ...
  /*
  **  The following line generates a benign warning under MSC 7+
  **  when compiled without /Zp1.
  */

    #pragma pack(1)
#elif defined(__ZTC__) || defined (__SC__)
    #pragma ZTC align 1
#elif defined(__BORLANDC__) || defined(__TURBOC__) && (__TURBOC__ > 0x202)
    #pragma option -a-
#endif
...
// Needed for function template specializations
#if defined(_MSC_VER)
//#  define _FNC_INLINE template<>
#  define _FNC_INLINE inline
#else
#  define _FNC_INLINE inline
#endif
