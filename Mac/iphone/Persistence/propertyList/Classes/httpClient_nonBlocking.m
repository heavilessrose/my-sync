...
pthread_t createThread(){
	...
	int threadError = pthread_create(&posixThreadID, &attr, &pollThread, NULL);
	if(threadError != 0){
		printf("thread create error");
	}
	assert(!pthread_attr_destroy(&attr));
	printf("poll thread ID: %u \n", (unsigned int)pthread_self());
	//	assert(!pthread_attr_destroy(&attr));
	//	if (threadError != 0) {
	//		printf("threadError");
	//	}
...
void* pollThread(void *data){
	printf("start poll thread\n");
	if(maxfd == 0){
		printf("maxfd == 0");
		return NULL;
	}
	
	select(maxfd, &read_fds, &write_fds, &except_fds, NULL);// 永久等待
	printf("connected, select return\n");
