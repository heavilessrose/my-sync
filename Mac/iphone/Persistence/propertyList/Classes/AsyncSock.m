...
//				[self doSocketOpen:sock withCFSocketError:kCFSocketSuccess];
			break;
		case kCFSocketDataCallBack:// 服务器写到客户端
			NSLog(@"doCFSocketCallback: Incoming data ready");
			//NSLog(@"incoming data: %s", [(NSData*)data bytes]);
			[(NSData*)data writeToFile:[[propertyListViewController appDocumentsDir] stringByAppendingPathComponent:@"dd.txt"] atomically:YES];
			break;
